...
class EndpointTester:
            ...
            if method == "GET":
                async with self.session.get(url) as response:
                    ...
                    if response.status == expected_status:
                        ...
                        print(
                            f"✅ {endpoint} - Status: {response.status} ({test_result['response_time']:.3f}s)"
                        )
                    else:
                        print(
                            f"❌ {endpoint} - Expected: {expected_status}, Got: {response.status}"
                        )
    ...
    async def run_health_tests(self):
        ...
        # /healthz removed; use /api/health as canonical health endpoint
    ...
    async def run_prizepicks_tests(self):
        ...
        await self.test_endpoint(
            "/api/prizepicks/lineup/optimize", method="POST", data=lineup_data
        )
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        avg_response_time = (
            sum(r["response_time"] for r in self.test_results if r["response_time"])
            / total_tests
        )
    ...
    def print_summary(self):
        ...
        if summary["failed"] > 0:
            ...
            for failed_test in report["failed_tests"]:
                print(
                    f"  - {failed_test['method']} {failed_test['endpoint']}: {failed_test['error'] or f'Status {failed_test['actual_status']}'}"
                )
