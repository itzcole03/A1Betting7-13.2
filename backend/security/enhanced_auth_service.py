...
class EnhancedAuthService:
    ...
    def __init__(self):
        ...
        config_obj = (
            ...
            if hasattr(unified_config, "get_config")
            else unified_config
        )
        ...
        self.access_token_expire_minutes = (
            getattr(getattr(self.config, "api", object()), "jwt_expire_minutes", None)
            or 15
        )
        self.refresh_token_expire_days = getattr(
            getattr(self.config, "api", object()), "jwt_refresh_expire_days", 30
        )
    ...
    def _check_rate_limit(self, identifier: str) -> bool:
        ...
        if identifier in self._token_attempts:
            self._token_attempts[identifier] = [
                attempt
                for attempt in self._token_attempts[identifier]
    ...
    def create_access_token(
        ...
        # Prepare claims dict preferring Pydantic v2 model_dump when available
        try:
            if hasattr(claims, "model_dump") and callable(
                getattr(claims, "model_dump")
            ):
                claims_payload = claims.model_dump()
            else:
                claims_payload = claims.dict()
        except Exception:
            claims_payload = dict(getattr(claims, "__dict__", {}) or {})

        token = jwt.encode(claims_payload, self.secret_key, algorithm=self.algorithm)

        logger.info(f"Access token created for user {user_id}, expires at {expires_at}")
        return token

    def create_refresh_token(
        self, user_id: str, device_id: str = None, parent_token_id: str = None
        ...
        try:
            if hasattr(claims, "model_dump") and callable(
                ...
                refresh_claims_payload = claims.model_dump()
            else:
                refresh_claims_payload = claims.dict()
        except Exception:
            refresh_claims_payload = dict(getattr(claims, "__dict__", {}) or {})

        refresh_token = jwt.encode(
            refresh_claims_payload, self.secret_key, algorithm=self.algorithm
        )

        logger.info(
            f"Refresh token created for user {user_id}, rotation count: {rotation_count}"
        )
        return refresh_token, token_data

    def verify_token_with_skew(
        self, token: str, expected_type: str = "access", check_blacklist: bool = True
    ) -> TokenClaims:
        """
        ...
        try:
            ...
            if check_blacklist and self._is_token_blacklisted(jti):
                logger.warning(
                    f"Blacklisted token attempted: {jti[:8]}... by user {user_id}"
                )
                raise HTTPException(
            ...
            try:
                ...
                if exp_time and (now - exp_time) <= self.clock_skew_tolerance_seconds:
                    logger.info(
                        f"Token accepted within clock skew tolerance: {now - exp_time:.1f}s"
                    )
                else:
                    logger.warning(
                        f"Token expired beyond skew tolerance: {now - exp_time:.1f}s"
                    )
                    raise HTTPException(
                        status_code=status.HTTP_401_UNAUTHORIZED,
            ...
            logger.info(
                f"Token verified successfully for user {claims.sub}, type: {token_type}"
            )
            return claims
    ...
    def refresh_access_token(self, refresh_token: str) -> Tuple[str, str]:
        ...
        refresh_claims = self.verify_token_with_skew(
        ...
        if not token_data or token_data.is_revoked:
            logger.warning(
                f"Refresh token not found or revoked: {refresh_claims.jti[:8]}..."
            )
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
        ...
        if token_data.rotation_count >= 10:
            logger.warning(
                f"Refresh token rotation limit exceeded: {refresh_claims.jti[:8]}..."
            )
    ...
    def revoke_user_tokens(self, user_id: str, exclude_session: str = None) -> int:
        ...
        for token_id, token_data in self._refresh_tokens.items():
            if (
                ...
                and not token_data.is_revoked
                and token_id != exclude_session
            ):
    ...
    def get_token_info(self, token: str) -> Dict[str, Any]:
        ...
        try:
            ...
            exp_time = payload.get("exp")
            exp_datetime = (
                datetime.fromtimestamp(exp_time, timezone.utc) if exp_time else None
            )

            iat_time = payload.get("iat")
            iat_datetime = (
                datetime.fromtimestamp(iat_time, timezone.utc) if iat_time else None
            )
            ...
            return {
                ...
                "is_expired": (
                    exp_datetime < datetime.now(timezone.utc) if exp_datetime else None
                ),
