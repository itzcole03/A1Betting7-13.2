...
def success_response(
    ...
    """
    meta = ResponseMeta(processing_time_ms=processing_time_ms, request_id=request_id)

    # Prefer Pydantic v2 model_dump when available, fall back to dict()
    try:
        if hasattr(meta, "model_dump") and callable(getattr(meta, "model_dump")):
            meta_out = meta.model_dump()
        else:
            meta_out = meta.dict()
    except Exception:
        # Last resort: use __dict__ for serialization
        meta_out = dict(getattr(meta, "__dict__", {}) or {})
...
def error_response(
    ...
    meta = ResponseMeta(processing_time_ms=processing_time_ms, request_id=request_id)
    # Serialize error_detail and meta preferring model_dump()
    try:
        if hasattr(error_detail, "model_dump") and callable(
            getattr(error_detail, "model_dump")
        ):
            error_out = error_detail.model_dump()
        else:
            error_out = error_detail.dict()
    except Exception:
        error_out = dict(getattr(error_detail, "__dict__", {}) or {})
...
class ResponseBuilder:
    ...
    def error(
        self, error_code: str, message: str, details: Optional[str] = None
    ) -> Dict[str, Any]:
    ...
    async def wrapper(*args, **kwargs):
        ...
        try:
            ...
            return builder.error(
                e.error_code, e.message, str(e.details) if e.details else None
            )
