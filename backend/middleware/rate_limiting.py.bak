...
class RateLimitTracker:
    ...
    async def check_rate_limit(self, request: Request) -> Tuple[bool, Dict[str, Any]]:
        ...
        endpoint_count = await self.endpoint_counters[endpoint_counter_key].get_count(
            current_time
        )
        ...
        if minute_count >= minute_limit:
            ...
            rate_limit_info["retry_after"] = int(
                self.minute_counters[client_key].get_reset_time(current_time)
                - current_time
                + 1
            )
        ...
        if hour_count >= hour_limit:
            ...
            rate_limit_info["retry_after"] = int(
                self.hour_counters[client_key].get_reset_time(current_time)
                - current_time
                + 1
            )
            return False, rate_limit_info
        ...
        if endpoint_count >= endpoint_limit:
            ...
            rate_limit_info["retry_after"] = int(
                self.endpoint_counters[endpoint_counter_key].get_reset_time(
                    current_time
                )
                - current_time
                + 1
...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        try:
            ...
            unified_error_handler.handle_error(
                e, ErrorContext(endpoint="rate_limiting_middleware", method="dispatch")
            )
    ...
    def _add_rate_limit_headers(
        self, response: Response, rate_info: Dict, allowed: bool
    ):
        ...
        if allowed:
            ...
            response.headers["X-RateLimit-Limit-Minute"] = str(
                rate_info.get("minute_limit", 0)
            )
            response.headers["X-RateLimit-Remaining-Minute"] = str(
                max(
                    0,
                    rate_info.get("minute_limit", 0) - rate_info.get("minute_count", 0),
                )
            )
            response.headers["X-RateLimit-Limit-Hour"] = str(
                rate_info.get("hour_limit", 0)
            )
