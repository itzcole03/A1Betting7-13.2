...
class OptimizationRequest(BaseModel):
    ...
    objective: OptimizationObjective = Field(
        OptimizationObjective.MAXIMIZE_SHARPE, description="Optimization objective"
    )
    risk_level: RiskLevel = Field(
        RiskLevel.MODERATE, description="Risk tolerance level"
    )
...
class PortfolioOptimizationRequest(BaseModel):
    ...
    predictions: List[Dict[str, Any]] = Field(
        ..., description="List of predictions to optimize"
    )

    # Portfolio constraints
    max_allocation_per_bet: float = Field(
        0.25, ge=0, le=1, description="Max allocation per single bet"
    )
    min_allocation_per_bet: float = Field(
        0.01, ge=0, le=1, description="Min allocation per bet"
    )
    max_positions: int = Field(
        10, ge=1, le=50, description="Maximum number of positions"
    )
    ...
    max_drawdown: float = Field(
        0.15, ge=0, le=1, description="Maximum acceptable drawdown"
    )
    ...
    correlation_threshold: float = Field(
        0.7, ge=0, le=1, description="Max correlation between bets"
    )

    @field_validator("max_allocation_per_bet")
...
class KellyOptimizationRequest(BaseModel):
    ...
    predictions: List[Dict[str, Any]] = Field(
        ..., description="Predictions with win probabilities and odds"
    )

    # Kelly parameters
    fractional_kelly: float = Field(
        0.25, ge=0.1, le=1.0, description="Fractional Kelly multiplier"
    )
    max_kelly_allocation: float = Field(
        0.20, ge=0, le=1, description="Maximum Kelly allocation"
    )
...
class ArbitrageOptimizationRequest(BaseModel):
    ...
    min_profit_margin: float = Field(
        0.02, ge=0, le=1, description="Minimum profit margin"
    )
    max_stake_per_arb: Decimal = Field(
        ..., gt=0, description="Maximum stake per arbitrage"
    )
    total_capital: Decimal = Field(..., gt=0, description="Total available capital")

    # Execution constraints
    sportsbook_limits: Optional[Dict[str, Decimal]] = Field(
        None, description="Sportsbook betting limits"
    )
...
class PortfolioOptimization(BaseModel):
    ...
    allocations: Dict[str, float] = Field(
        ..., description="Bet ID to allocation mapping"
    )
    ...
    quantum_advantage: Optional[float] = Field(
        None, description="Quantum optimization advantage"
    )
    entanglement_score: Optional[float] = Field(
        None, description="Quantum entanglement score"
    )
    ...
    optimization_time_ms: float = Field(
        ..., description="Optimization time in milliseconds"
    )
...
class KellyRecommendation(BaseModel):
    ...
    # Kelly calculations
    kelly_recommendations: Dict[str, Dict[str, float]] = Field(
        ..., description="Kelly recommendations per bet"
    )
    fractional_kelly_used: float = Field(
        ..., description="Fractional Kelly multiplier applied"
    )

    # Aggregate metrics
    total_kelly_allocation: float = Field(..., description="Total Kelly allocation")
    expected_growth_rate: float = Field(
        ..., description="Expected logarithmic growth rate"
    )

    # Risk metrics
    probability_of_ruin: float = Field(..., description="Probability of ruin estimate")
    time_to_double: Optional[float] = Field(
        None, description="Expected time to double bankroll"
    )
...
class ArbitrageAnalysis(BaseModel):
    ...
    opportunities: List[Dict[str, Any]] = Field(
        ..., description="Analyzed arbitrage opportunities"
    )
    total_opportunities: int = Field(..., description="Total number of opportunities")

    # Optimal execution
    recommended_stakes: Dict[str, Dict[str, Decimal]] = Field(
        ..., description="Recommended stakes per opportunity"
    )
    ...
    # Timing analysis
    optimal_execution_order: List[str] = Field(
        ..., description="Optimal execution order"
    )
    ...
    # Sportsbook analysis
    sportsbook_exposure: Dict[str, Decimal] = Field(
        ..., description="Exposure per sportsbook"
    )
    ...
    # Preserve Decimal -> float serialization for arbitrage numeric fields
    @field_serializer(
        "recommended_stakes",
        when_used="json",
    )
    def _ser_recommended_stakes(self, v, _info):
        # recommended_stakes is a nested mapping; convert Decimal values to float
        def convert(obj):
    ...
    @field_serializer("total_stake_required", "guaranteed_profit", when_used="json")
    def _ser_decimal_simple(self, v: Decimal, _info):
        return float(v)

    @field_serializer("sportsbook_exposure", when_used="json")
    def _ser_sportsbook_exposure(self, v: Dict[str, Decimal], _info):
...
class RiskAssessment(BaseModel):
    ...
    overall_risk_score: float = Field(
        ..., ge=0, le=1, description="Overall risk score (0-1)"
    )
...
class OptimizationConfig(BaseModel):
    ...
    # Kelly settings
    default_fractional_kelly: float = Field(
        0.25, description="Default fractional Kelly"
    )
    ...
    default_max_allocation: float = Field(
        0.25, description="Default max allocation per bet"
    )

    # Risk settings
    default_var_confidence: float = Field(
        0.95, description="Default VaR confidence level"
    )
    stress_test_scenarios: int = Field(
        1000, description="Number of stress test scenarios"
    )
