...
class SportsbookOdds(BaseModel):
    ...
    # Preserve Decimal -> float serialization for min/max bet values
    model_config = ConfigDict()

    @field_serializer("min_bet", "max_bet", when_used="json")
    def _ser_min_max_bet(self, v: Decimal, _info):
...
class WebhookEvent(BaseModel):
    ...
    # For WebhookEvent timestamps normal Pydantic serialization suffices
    model_config = ConfigDict()
...
class ArbitrageOpportunity(BaseModel):
    ...
    confidence_score: float = Field(
        ..., ge=0, le=1, description="Opportunity confidence"
    )
    time_to_expire: Optional[int] = Field(
        None, description="Estimated expiration in seconds"
    )
    ...
    # Ensure Decimal fields serialize to float (preserve previous behavior)
    @field_serializer(
        "stake_a", "stake_b", "total_stake", "guaranteed_profit", when_used="json"
    )
    def _ser_decimal_fields(self, v, _info):
        return float(v) if v is not None else None
...
class HealthResponse(BaseModel):
    ...
    # Pydantic v2 configuration (default datetime serialization)
    model_config = ConfigDict()
