...
def require_api_key(api_key: str = Security(api_key_header)):
    """Dependency to require a valid API key via X-API-Key header."""
...
class SecurityManager:
    ...
    def validate_api_key(self, api_key: str) -> bool:
        ...
        try:
            user = (
                session.query(User)
                .filter_by(api_key_encrypted=api_key, is_active=True)
                .first()
            )
            return user is not None
        except Exception as e:
            logger.error(f"API key validation DB error: {e}")
            return False
        finally:
            session.close()
