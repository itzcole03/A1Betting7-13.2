...
try:
    ...
    logger.warning("aioredis not available on sys.path, attempting to load local shim")
    # Attempt to load a local aioredis shim from the repository root (aioredis.py)
    try:
        import importlib.util
        from pathlib import Path

        # cache_service.py path: backend/domains/database/cache_service.py
        # repo root is four levels up from this file
        repo_root = Path(__file__).resolve().parents[4]
        shim_path = repo_root / "aioredis.py"
        if shim_path.exists():
            spec = importlib.util.spec_from_file_location(
                "aioredis_shim", str(shim_path)
            )
            shim = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(shim)  # type: ignore
            aioredis = shim
            logger.info(f"Loaded local aioredis shim from {shim_path}")
        else:
            logger.warning("No local aioredis shim found at %s", shim_path)
    except Exception as _e:
        logger.exception("Failed loading local aioredis shim: %s", _e)
...
class UnifiedCacheService:
    ...
    def __init__(self, config: CacheConfig = None):
        ...
        self.cache_stats = {"hits": 0, "misses": 0, "errors": 0, "invalidations": 0}
    ...
    async def get_redis(self):
        ...
        if not self.redis_pool or aioredis is None:
            yield None
            return
    ...
    async def cache_match(
        self, match_id: int, match_data: Dict, ttl: int = None
    ) -> bool:
    ...
    async def cache_user_data(
        self, user_id: str, user_data: Dict, ttl: int = None
    ) -> bool:
    ...
    async def cache_prediction(
        self, match_id: int, prediction_data: Dict, ttl: int = None
    ) -> bool:
    ...
    async def cache_odds(
        self, match_id: int, sportsbook: str, odds_data: Dict, ttl: int = None
    ) -> bool:
    ...
    async def cache_search_results(
        self, query_hash: str, results: List[Dict], ttl: int = None
    ) -> bool:
