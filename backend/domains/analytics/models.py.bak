...
class AnalyticsRequest(BaseModel):
    ...
    time_frame: TimeFrame = Field(
        TimeFrame.LAST_24_HOURS, description="Time frame for analytics"
    )
...
class MetricPoint(BaseModel):
    ...
    # Remove deprecated json_encoders; rely on Pydantic v2 default serialization
...
class BusinessIntelligenceReport(BaseModel):
    ...
    # Convert Decimal fields and dict-of-Decimal values to float when
    # serializing to JSON to preserve previous behavior previously provided
    # by `json_encoders`.
    @field_serializer(
        "total_revenue",
    ...
    def _ser_decimal_fields(self, v: Decimal, _info):
        return float(v)

    @field_serializer("revenue_by_sport", when_used="json")
    def _ser_revenue_by_sport(self, v: Dict[str, Decimal], _info):
