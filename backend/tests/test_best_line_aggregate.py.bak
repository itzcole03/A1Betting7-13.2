...
from backend.services.odds_store import BestLineResult, odds_store_service
...
def test_update_best_line_aggregate_upsert(monkeypatch):
    ...
    asyncio.get_event_loop().run_until_complete(
        odds_store_service._update_best_line_aggregate("test-prop-123", "NBA")
    )
    ...
    with Session(sync_engine) as s:
        res = s.exec(
            select(BestLineAggregate).where(
                BestLineAggregate.prop_id == "test-prop-123"
            )
        )
