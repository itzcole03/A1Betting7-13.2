...
class RawExternalPropDTO(BaseModel):
    ...
    team_code: Optional[str] = Field(
        ...
        min_length=0,
        max_length=50,
        description="Team abbreviation or code or full name",
    )
    ...
    payout_type: PayoutType = Field(
        default=PayoutType.STANDARD, description="Type of payout structure"
    )
    ...
    provider_name: str = Field(
        default="stub_provider", description="Name of the data provider"
    )
...
class PayoutSchema(BaseModel):
    ...
    variant_code: Optional[PayoutVariant] = Field(
        default=PayoutVariant.STANDARD_ODDS, description="Payout structure variant"
    )

    # Canonical multiplier representation (normalized from provider formats)
    over_multiplier: Optional[float] = Field(
        None, ge=0, description="Over bet payout multiplier"
    )
    under_multiplier: Optional[float] = Field(
        None, ge=0, description="Under bet payout multiplier"
    )

    # Legacy fields for backward compatibility
    over: Optional[float] = Field(None, description="Legacy over bet odds (deprecated)")
    under: Optional[float] = Field(
        None, description="Legacy under bet odds (deprecated)"
    )

    # Additional payout modifiers
    boost_multiplier: Optional[float] = Field(
        None, ge=1, description="Promotional boost multiplier"
    )

    # Provider-specific metadata (for debugging/traceability)
    provider_format: Optional[Dict[str, Any]] = Field(
...
class NormalizedPropDTO(BaseModel):
    ...
    player_id: Optional[int] = Field(
        None, description="Internal player ID (set during persistence)"
    )
    ...
    payout_schema: PayoutSchema = Field(
        ..., description="Standardized payout information"
    )
    external_ids: Dict[str, str] = Field(
        ..., description="External identifier mappings"
    )
    timestamp: datetime = Field(..., description="Processing timestamp")
    line_hash: Optional[str] = Field(
        None, description="Computed line hash for change detection"
    )
...
class IngestResult(BaseModel):
    ...
    # Status
    status: str = Field(
        ..., description="Overall ingestion status: success, partial, failed"
    )
    ...
    total_new_quotes: int = Field(
        default=0, ge=0, description="New market quote records created"
    )
    total_line_changes: int = Field(
        default=0, ge=0, description="Line changes detected"
    )
    ...
    props_processed: int = Field(
        default=0, ge=0, description="Number of props processed by the pipeline"
    )
    quotes_created: int = Field(
        default=0, ge=0, description="Number of market quotes created during the run"
    )

    # Details
    changed_quote_ids: List[int] = Field(
        default_factory=list, description="IDs of quotes that changed"
    )
    new_prop_ids: List[int] = Field(
        default_factory=list, description="IDs of newly created props"
    )
    new_player_ids: List[int] = Field(
        default_factory=list, description="IDs of newly created players"
    )
    ...
    # Run metadata
    ingest_run_id: Optional[int] = Field(
        None, description="Database ID of the ingest run record"
    )
    ...
    def add_error(
        ...
        external_prop_id: Optional[str] = None,
    ):
