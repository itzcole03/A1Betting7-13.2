...
class Prop(Base):
    ...
    market_quotes = relationship(
        "MarketQuote", back_populates="prop", cascade="all, delete-orphan"
    )
...
class MarketQuote(Base):
    ...
    __table_args__ = (
        Index(
            "ix_market_quotes_prop_source_last_seen",
            "prop_id",
            "source",
            "last_seen_at",
        ),
...
class IngestRun(Base):
    ...
    total_new_quotes = Column(
        Integer, nullable=False, default=0
    )  # New quote records created
    total_line_changes = Column(
        Integer, nullable=False, default=0
    )  # Line changes detected
    total_new_players = Column(
        Integer, nullable=False, default=0
    )  # New players created
    total_new_props = Column(Integer, nullable=False, default=0)  # New props created
    # Counters used by tests and reporting
    quotes_created = Column(Integer, nullable=False, default=0)
    props_processed = Column(Integer, nullable=False, default=0)
