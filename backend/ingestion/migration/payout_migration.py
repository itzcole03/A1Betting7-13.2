...
from ..models.database_models import IngestRun, MarketQuote
from ..models.dto import PayoutSchema, PayoutType, PayoutVariant
from ..normalization.payout_normalizer import PayoutNormalizationError, normalize_payout
...
def migrate_payout_schemas():
    ...
    with SessionLocal() as session:
        try:
            ...
            logger.info(
                f"Found {migration_stats['total_quotes']} market quotes to potentially migrate"
            )
            ...
            while processed < migration_stats["total_quotes"]:
                ...
                for quote in batch:
                    try:
                        ...
                        canonical_schema = _convert_to_canonical(
                            current_schema, quote.source
                        )
                        ...
                        if old_hash != new_hash:
                            ...
                            logger.debug(
                                f"Hash changed for quote {quote.id}: {old_hash[:8]} -> {new_hash[:8]}"
                            )
                ...
                logger.info(
                    f"Processed {processed}/{migration_stats['total_quotes']} quotes "
            ...
            migration_run.duration_ms = int(
                (
                    migration_stats["finished_at"] - migration_stats["started_at"]
                ).total_seconds()
                * 1000
            )
            ...
            logger.info(
                f"Canonical payout migration completed: "
...
def _is_canonical_format(payout_schema: Dict[str, Any]) -> bool:
    ...
    has_multipliers = (
        "over_multiplier" in payout_schema or "under_multiplier" in payout_schema
    )
...
def _convert_to_canonical(legacy_schema: Dict[str, Any], source: str) -> PayoutSchema:
    ...
    from ..models.dto import PayoutType, RawExternalPropDTO
...
def _manual_conversion_fallback(
    legacy_schema: Dict[str, Any], source: str
) -> PayoutSchema:
    """
...
def document_expected_edge_churn(migration_stats: Dict[str, Any]) -> str:
    ...
    """
    churn_rate = (
        migration_stats["hash_changes"] / max(migration_stats["total_quotes"], 1)
    ) * 100
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "summary":
        ...
        try:
            ...
            print(
                f"Migrated: {stats['migrated_quotes']}/{stats['total_quotes']} quotes"
            )
            ...
            if stats["hash_changes"] > 0:
                ...
                with open("payout_migration_churn_report.md", "w") as f:
                    f.write(churn_doc)
                print(
                    "Edge churn documentation saved to: payout_migration_churn_report.md"
                )
