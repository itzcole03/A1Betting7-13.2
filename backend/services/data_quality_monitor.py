...
_DQM_DEBUG = os.environ.get("DATA_QUALITY_MONITOR_DEBUG", "").lower() in (
    "1",
    "true",
    "yes",
)
...
def _dqm_debug(msg: str, *args, **kwargs):
    if _DQM_DEBUG:
...
class DataQualityMonitor:
    ...
    async def _register_default_rules(self):
        ...
        self.register_rule(
            QualityRule(
                ...
                data_source=DataSourceType.ODDS_API,
                field_path="$.bookmakers[*].markets[*].outcomes[*].price",
                rule_function=lambda x: isinstance(x, (int, float))
                and 1.01 <= x <= 1000,
                threshold=0.99,
                severity=AlertSeverity.ERROR,
        ...
        self.register_rule(
            QualityRule(
                ...
                rule_function=lambda x: x is None
                or (isinstance(x, int) and 0 <= x <= 50),
                threshold=0.99,
                severity=AlertSeverity.ERROR,
    ...
    async def validate_data(
        ...
        for rule in applicable_rules:
            try:
                ...
                valid_values = sum(
                    1 for value in field_values if rule.rule_function(value)
                )
                ...
                if total_values > 0:
                    ...
                    if validity_ratio < rule.threshold:
                        violation = QualityViolation(
                            ...
                            data_sample=self._sample_invalid_data(
                                data, field_values, rule
                            ),
        ...
        await self._update_quality_metrics(
            data_source, violations, len(applicable_rules)
        )
    ...
    def _sample_invalid_data(
        self, data: Dict[str, Any], field_values: List[Any], rule: QualityRule
    ) -> Dict[str, Any]:
    ...
    async def _update_quality_metrics(
        ...
        if metric_scores[QualityMetricType.COMPLETENESS]:
            metrics.completeness_score = statistics.mean(
                metric_scores[QualityMetricType.COMPLETENESS]
            )

        if metric_scores[QualityMetricType.ACCURACY]:
            metrics.accuracy_score = statistics.mean(
                metric_scores[QualityMetricType.ACCURACY]
            )

        if metric_scores[QualityMetricType.CONSISTENCY]:
            metrics.consistency_score = statistics.mean(
                metric_scores[QualityMetricType.CONSISTENCY]
            )

        if metric_scores[QualityMetricType.TIMELINESS]:
            metrics.timeliness_score = statistics.mean(
                metric_scores[QualityMetricType.TIMELINESS]
            )

        if metric_scores[QualityMetricType.VALIDITY]:
            metrics.validity_score = statistics.mean(
                metric_scores[QualityMetricType.VALIDITY]
            )
    ...
    async def detect_anomalies(
        ...
        try:
            ...
            extracted_metrics = self._extract_metrics_for_anomaly_detection(
                data_source, data
            )
    ...
    def _extract_metrics_for_anomaly_detection(
        self, data_source: DataSourceType, data: Dict[str, Any]
    ) -> Dict[str, float]:
        """Extract key metrics for anomaly detection"""
        metrics = {}
        ...
        try:
            if data_source == DataSourceType.ODDS_API:
                ...
                odds_values = self._extract_field_values(
                ...
                if home_scores:
                    metrics["avg_home_score"] = np.mean(
                        [s for s in home_scores if s is not None]
                    )
                if away_scores:
                    metrics["avg_away_score"] = np.mean(
                        [s for s in away_scores if s is not None]
                    )
    ...
    async def reconcile_cross_source_data(
        self,
        primary_source: DataSourceType,
        secondary_source: DataSourceType,
    ...
    async def _default_cross_source_reconciliation(
        self,
        primary_source: DataSourceType,
        secondary_source: DataSourceType,
        primary_data: Dict[str, Any],
        ...
        if (
            ...
            for primary_game in primary_games:
                matching_event = self._find_matching_event(
                    primary_game, secondary_events
                )
                if matching_event:
                    game_discrepancies = self._compare_game_data(
                        primary_game, matching_event
                    )
    ...
    def _find_matching_event(
        self, game: Dict[str, Any], events: List[Dict[str, Any]]
    ) -> Optional[Dict[str, Any]]:
    ...
    def _compare_game_data(
        self, primary_game: Dict[str, Any], secondary_event: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        ...
        if (
            primary_home_score is not None
            and secondary_home_score is not None
            and abs(primary_home_score - secondary_home_score) > 0
        ):
    ...
    async def update_timeliness_metric(self, data_source: DataSourceType):
        ...
        expected_next_update = metric.last_update + timedelta(
            seconds=metric.expected_interval
        )

        if now > expected_next_update:
            delay_seconds = (now - expected_next_update).total_seconds()
            metric.avg_delay = (
                metric.avg_delay * metric.update_count + delay_seconds
            ...
            if delay_seconds > metric.expected_interval * 2:
                ...
                violation = QualityViolation(
                    ...
                    severity=(
                        ...
                        if delay_seconds < metric.expected_interval * 3
            ...
            try:
                ...
                for source_type, timeliness in self.timeliness_metrics.items():
                    ...
                    if time_since_update > timeliness.expected_interval * 3:
                        logger.warning(
                            f"⚠️ Data source {source_type.value} is stale ({time_since_update:.1f}s)"
                        )
        ...
        while True:
            try:
                ...
                for source_type, metric in self.timeliness_metrics.items():
                    expected_next = metric.last_update + timedelta(
                        seconds=metric.expected_interval
                    )

                    if now > expected_next + timedelta(
                        seconds=metric.expected_interval
                    ):
                        # Data is overdue
    ...
    async def _reconciliation_monitor(self):
        ...
        while True:
            try:
                ...
                _dqm_debug("🔄 Running cross-source reconciliation check")
    ...
    async def _cleanup_old_data(self):
        ...
        while True:
            try:
                ...
                _dqm_debug("🧹 Cleaned up old violation data")
