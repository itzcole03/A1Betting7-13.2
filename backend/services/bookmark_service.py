...
class BookmarkService:
    ...
    async def bookmark_prop(
        ...
        if session is None:
            ...
            async with AsyncSession(async_engine) as session:
                return await self.bookmark_prop(
                    user_id, prop_id, sport, player, market, team, session
                )

        try:
            # Check if user exists
            user_result = await unified_session_execute(
                session, select(UserORM).where(UserORM.id == user_id)
            )
            ...
            existing_result = await unified_session_execute(
                session,
                select(BookmarkORM).where(
                    BookmarkORM.user_id == user_id, BookmarkORM.prop_id == prop_id
                ),
            )
            ...
            if existing_bookmark:
                self.logger.info(
                    f"Bookmark already exists for user {user_id}, prop {prop_id}"
                )
                return False
            ...
            self.logger.warning(
                f"Integrity error bookmarking prop {prop_id} for user {user_id}: {e}"
            )
            return False
        except Exception as e:
            await session.rollback()
            self.logger.error(
                f"Error bookmarking prop {prop_id} for user {user_id}: {e}"
            )
            raise

    async def unbookmark_prop(
        self, user_id: str, prop_id: str, session: Optional[AsyncSession] = None
    ) -> bool:
        """
        ...
        try:
            ...
            if deleted_count > 0:
                ...
                self.logger.info(
                    f"No bookmark found for user {user_id}, prop {prop_id}"
                )
                return False
            ...
            await session.rollback()
            self.logger.error(
                f"Error unbookmarking prop {prop_id} for user {user_id}: {e}"
            )
            raise
    ...
    async def get_user_bookmarks(
        ...
        try:
            ...
            self.logger.debug(
                f"Retrieved {len(bookmarks)} bookmarks for user {user_id}"
            )
    ...
    async def get_user_bookmarked_prop_ids(
        ...
        try:
            ...
            self.logger.error(
                f"Error retrieving bookmarked prop IDs for user {user_id}: {e}"
            )
            return set()
    ...
    async def is_prop_bookmarked(
        ...
        try:
            ...
            self.logger.error(
                f"Error checking if prop {prop_id} is bookmarked for user {user_id}: {e}"
            )
            return False
