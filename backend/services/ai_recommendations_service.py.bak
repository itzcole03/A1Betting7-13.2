...
class AIRecommendationsService:
    ...
    async def generate_recommendations(
        ...
        try:
            ...
            logger.info(
                f"Generating AI recommendations for user {user_profile.user_id}, sport: {sport}"
            )

            # Get current opportunities from existing services
            best_lines = await self.odds_service.find_best_lines(sport)
            arbitrage_opportunities = (
                await self.odds_service.find_arbitrage_opportunities(sport)
            )
            ...
            async with self._cache_lock:
                if self._cache is None:
                    try:
                        ...
                        logger.debug("AI recommendations cache unavailable: %s", exc)
                        return None
        return self._cache
    ...
    async def _analyze_prop_opportunity(
        ...
        try:
            ...
            if over_analysis and under_analysis:
                return (
                    over_analysis
                    if over_analysis.ai_score > under_analysis.ai_score
                    else under_analysis
                )
            elif over_analysis:
    ...
    async def _analyze_prop_side(
        ...
        try:
            ...
            market_odds = (
                line.best_over_price if side == "over" else line.best_under_price
            )
            ...
            # Calculate edge
            edge_percentage = (
                (fair_probability - implied_probability) / implied_probability
            ) * 100
            ...
            recommendation = SmartRecommendation(
                ...
                recommendation_type=self._classify_recommendation_type(
                    edge_percentage, situational_factors
                ),
    ...
    def _determine_risk_level(
        self, edge_percentage: float, confidence: float
    ) -> RiskLevel:
    ...
    def _classify_recommendation_type(
        self, edge_percentage: float, situational_factors: Dict[str, Any]
    ) -> RecommendationType:
    ...
    def _calculate_market_efficiency(self, line: Any) -> float:
        ...
        try:
            ...
            under_odds = [
                book.under_price for book in line.books if book.under_price != 0
            ]
    ...
    async def _create_arbitrage_recommendation(
        self, arbitrage_opportunity: Any, user_profile: UserProfile
    ) -> Optional[SmartRecommendation]:
        ...
        try:
            ...
            recommendation = SmartRecommendation(
                ...
                + timedelta(minutes=15),  # Arbitrage expires quickly
    ...
    async def _get_cached_recommendations(
        self, cache_key: str
    ) -> Optional[List[SmartRecommendation]]:
        """Get cached recommendations"""
        try:
            cache = await self._get_cache()
            if cache is None:
                logger.debug("No cache backend available for recommendation lookup")
                return None
    ...
    async def _cache_recommendations(
        self, cache_key: str, recommendations: List[SmartRecommendation]
    ):
        ...
        try:
            ...
            if cache is None:
                logger.debug(
                    "No cache backend available; skipping recommendation cache"
                )
                return
            await cache.cache_data(cache_key, cache_data, ttl=self.cache_ttl)
        except Exception as e:
    ...
    async def send_recommendation_alert(
        self, user_id: str, recommendation: SmartRecommendation
    ) -> bool:
        """Send alert for high-value recommendation"""
        try:
            # This would integrate with notification system
            logger.info(
                f"Alert: High-value recommendation for user {user_id}: {recommendation.player_name} {recommendation.stat_type}"
            )
            return True
        except Exception as e:
