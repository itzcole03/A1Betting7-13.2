...
class ModelRegistryService:
            ...
            for key in model_keys:
                ...
                if model_data:
                    ...
                    model_metadata = ModelMetadata(
                        ...
                        model_type=ModelType(
                            model_data.get("model_type", "transformer")
                        ),
                        ...
                        deployment_target=model_data.get(
                            "deployment_target", "production"
                        ),
                        feature_flag_id=model_data.get("feature_flag_id"),
                        rollout_percentage=float(
                            model_data.get("rollout_percentage", 0.0)
                        ),
                    ...
                    if model_data.get("resource_requirements"):
                        model_metadata.resource_requirements = json.loads(
                            model_data["resource_requirements"]
                        )
                    if model_data.get("dependencies"):
                        model_metadata.dependencies = json.loads(
                ...
                if metrics_data:
                    ...
                    metrics = ModelPerformanceMetrics(
                        ...
                        total_inference_time_ms=float(
                            metrics_data.get("total_inference_time_ms", 0.0)
                        ),
                        successful_inferences=int(
                            metrics_data.get("successful_inferences", 0)
                        ),
        ...
        async with self._lock:
            try:
                ...
                if not metadata.model_id:
                    metadata.model_id = (
                        f"{metadata.sport.lower()}_model_{uuid4().hex[:8]}"
                    )
                ...
                logger.info(
                    f"SUCCESS: Registered model {metadata.model_id} ({metadata.name} v{metadata.version})"
                )
    ...
    async def update_model_status(self, model_id: str, status: ModelStatus) -> bool:
        ...
        async with self._lock:
            ...
            logger.info(
                f"SUCCESS: Updated model {model_id} status: {old_status.value} → {status.value}"
            )
            return True
    ...
    async def record_inference_timing(
        self,
        model_id: str,
        timing_ms: float,
        success: bool = True,
        ...
        async with self._lock:
            ...
            if success:
                ...
                metrics.min_inference_time_ms = min(
                    metrics.min_inference_time_ms, timing_ms
                )
                metrics.max_inference_time_ms = max(
                    metrics.max_inference_time_ms, timing_ms
                )
                ...
                if len(metrics.recent_timings) > metrics.max_timing_samples:
                    metrics.recent_timings = metrics.recent_timings[
                        -metrics.max_timing_samples :
                    ]
