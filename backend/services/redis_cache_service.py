...
class RedisCacheService:
    ...
    def __init__(self):
        try:
            ...
            Exception
        ):  # pragma: no cover - use shim when aioredis not installed in test env
            aioredis = None
            from backend.services.shims.redis_shim import RedisClientShim
        self.settings = get_settings()
        # Ensure attribute exists even if connect() hasn't been called yet.
        # Tests and code may check redis_cache.redis_client before connect()-ing.
    ...
    async def cache_batch_predictions(
        ...
        cache_data = {
            ...
            "batch_hash": hashlib.md5(
                json.dumps(batch_input, sort_keys=True).encode()
            ).hexdigest(),
            "cached_at": datetime.now(timezone.utc).isoformat().replace("+00:00", "Z"),
...
def redis_cached(cache_type: str, ttl: Optional[int] = None):
    ...
    def decorator(func):
        async def wrapper(*args, **kwargs):
            ...
            cache_key = cache_service._generate_cache_key(
                f"endpoint_{func.__name__}", cache_key_data
            )
    ...
    async def add_line_snapshot(
        self, sport: str, player: str, market: str, book: str, line: float, odds: int
    ):
    ...
    async def get_line_snapshots(
        self, sport: str, player: str, market: str, book: str
    ) -> List[Dict]:
