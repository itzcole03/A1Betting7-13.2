...
class SmartSignalsService:
    ...
    async def generate_smart_signals(self, request: SmartSignalRequest) -> SmartSignalResponse:
        ...
        try:
            ...
            qualified_signals = [s for s in signals if s["overall_score"] >= request.min_score]
            ...
            avg_score = sum(s["overall_score"] for s in signals) / len(signals) if signals else 0
            ...
            return SmartSignalResponse(
                ...
                metadata={
                    ...
                    "score_breakdown": {
                        "very_strong": len([s for s in signals if s["overall_score"] >= 85]),
                        "strong": len([s for s in signals if 75 <= s["overall_score"] < 85]),
                        "moderate": len([s for s in signals if 60 <= s["overall_score"] < 75]),
        ...
        for player in mock_players[:8]:  # Limit to 8 players for prototype
            for market in mock_markets[:3]:  # Limit to 3 markets per player
                for line in market["lines"][:2]:  # Limit to 2 lines per market
                    signal = await self._create_mock_signal(player, market, line, request.sport)
                    if signal["overall_score"] >= 40:  # Only include reasonable signals
                        signals.append(signal)
        
        return signals

    async def _create_mock_signal(self, player: dict, market: dict, line: float, sport: str) -> dict:
        ...
        return {
            ...
            "component_breakdown": {
                "ev_component": {
                    ...
                    "contribution": round(ev_score * self.component_weights["ev_score"], 1)
                },
                "trend_component": {
                    "score": round(trend_score, 1),
                    "weight": self.component_weights["trend_score"],
                    "contribution": round(trend_score * self.component_weights["trend_score"], 1)
                },
                "juice_component": {
                    "score": round(juice_score, 1),
                    "weight": self.component_weights["juice_score"],
                    "contribution": round(juice_score * self.component_weights["juice_score"], 1)
                },
                "line_movement_component": {
                    "score": round(line_movement_score, 1),
                    "weight": self.component_weights["line_movement_score"],
                    "contribution": round(line_movement_score * self.component_weights["line_movement_score"], 1)
                }
            },
            "is_active": True,
    ...
    async def get_active_signals(self, sport: Optional[str] = None, min_score: float = 70.0) -> List[SmartSignal]:
        ...
        try:
            query = self.db_session.query(SmartSignal).filter(
                SmartSignal.is_active == True,
                SmartSignal.overall_score >= min_score
            )
