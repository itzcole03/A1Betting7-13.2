...
class HealthCollector:
        ...
        try:
            ...
            try:
                ...
                await asyncio.wait_for(
                    db_service.execute_query("SELECT 1"), timeout=5.0
                )
                ...
                return ServiceStatus(
                    ...
                    details={"query": "SELECT 1", "connection_pool": "active"},
                )
    ...
    async def check_redis_health(self) -> ServiceStatus:
        ...
        try:
            probe_key = f"health:redis:{int(start_time * 1000)}"
            payload = {"ping": datetime.now(timezone.utc).isoformat()}

            cache_write = await asyncio.wait_for(
                redis_cache.set(probe_key, payload, ttl=10), timeout=3.0
            )
            ...
            if not cache_write:
                return ServiceStatus(
                    ...
                    status="degraded",
                    latency_ms=None,
                    details={"error": "cache_write_failed"},
                )

            cached_payload = await asyncio.wait_for(
                redis_cache.get(probe_key), timeout=3.0
            )
            await redis_cache.delete(probe_key)

            latency_ms = (time.time() - start_time) * 1000
            backend_name = await redis_cache.backend_name()
            metrics = await redis_cache.get_metrics()

            details: Dict[str, Any] = {
                "backend": backend_name,
                "cache_roundtrip": cached_payload is not None,
            }
            if metrics:
                summary = {
                    key: metrics[key]
                    for key in ("hit_rate", "total_keys", "connected_clients")
                    if key in metrics
                }
                if summary:
    ...
    async def check_model_registry_health(self) -> ServiceStatus:
        ...
        try:
            ...
            try:
                ...
                model_count = await asyncio.wait_for(
                    registry_service.count_models(), timeout=3.0
                )
    ...
    async def check_websocket_manager_health(self) -> ServiceStatus:
        ...
        try:
            ...
            try:
                from ..realtime_integration_service import (
                ...
                if integration_service.websocket_manager:
                    ...
                    ws_status = await asyncio.wait_for(
                        integration_service.websocket_manager.get_status(), timeout=2.0
                    )
    ...
    def get_performance_stats(self) -> PerformanceStats:
        ...
        return PerformanceStats(
            ...
            event_loop_lag_ms=metrics_snapshot.get("event_loop", {}).get(
                "avg_lag_ms", 0.0
            ),
    ...
    def get_cache_stats(self) -> CacheStats:
        ...
        try:
            ...
            if cache_metrics and (
                cache_metrics.get("hits", 0) > 0 or cache_metrics.get("misses", 0) > 0
            ):
            ...
            return CacheStats(hit_rate=0.0, hits=0, misses=0, evictions=0)
    ...
    def get_infrastructure_stats(self) -> InfrastructureStats:
        ...
        if (
            self._cached_infrastructure_stats
            and self._last_health_check > 0
    ...
    async def collect_health(self) -> HealthResponse:
        ...
        try:
            ...
            services = await asyncio.wait_for(
                asyncio.gather(*service_tasks, return_exceptions=True), timeout=10.0
            )
            ...
            valid_services = [
                ServiceStatus(
                    ...
                    status="degraded",
                    latency_ms=None,
                    details={"error": "health_check_timeout"},
                ),
                ServiceStatus(
                    name="redis",
                    status="degraded",
