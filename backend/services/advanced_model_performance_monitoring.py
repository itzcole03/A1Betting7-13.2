...
# Module-level env gate: make debug no-op unless explicitly enabled to avoid expensive formatting
_AMPM_DEBUG = os.getenv("ADVANCED_MODEL_PERFORMANCE_MONITORING_DEBUG", "").lower() in (
    "1",
    "true",
    "yes",
)
...
class AdvancedModelPerformanceMonitoring:
    ...
    async def register_model(
        self, model_id: str, baseline_data: Dict[str, Any]
    ) -> bool:
        """
    ...
    async def record_prediction_batch(
        ...
        try:
            ...
            metrics = await self._calculate_batch_metrics(
    ...
    async def _calculate_batch_metrics(
        ...
        error_rate = sum(1 for p in predictions if p.get("error", False)) / max(
            predictions_count, 1
        )
    ...
    async def _check_real_time_drift(
        self, model_id: str, predictions: List[Dict[str, Any]]
    ) -> None:
    ...
    async def _detect_prediction_drift(
        ...
        try:
            ...
            if len(recent_metrics) >= 2:
                ...
                historical_avg_conf = np.mean(
                    [m.avg_confidence for m in recent_metrics[-5:]]
                )
                current_avg_conf = np.mean(current_confidences)

                confidence_drift = abs(current_avg_conf - historical_avg_conf) / max(
                    historical_avg_conf, 0.001
                )
    ...
    async def _detect_performance_drift(
        self, model_id: str, recent_metrics: List[PerformanceMetrics]
    ) -> None:
        ...
        try:
            ...
            accuracy_drift = (baseline_accuracy - recent_accuracy) / max(
                baseline_accuracy, 0.001
            )
    ...
    async def get_model_health_status(
        ...
        try:
            ...
            if recent_metrics:
                uptime_hours = (
                    datetime.now() - recent_metrics[0].timestamp
                ).total_seconds() / 3600
            else:
                uptime_hours = 0
            ...
            metrics_summary = {
                ...
                "avg_throughput": np.mean(
                    [m.throughput_per_second for m in recent_metrics]
                ),
    ...
    async def _calculate_health_score(
        ...
        try:
            ...
            baseline_latency = baseline.get("latency_ms", 50)
            latency_score = max(
                0, 1 - (avg_latency - baseline_latency) / baseline_latency
            )
    ...
    async def get_comparative_analysis(self, model_ids: List[str]) -> Dict[str, Any]:
        ...
        try:
            comparison = {"models": {}, "rankings": {}, "summary": {}}
            ...
            comparison["rankings"] = {
                ...
                "latency": sorted(
                    model_stats.items(), key=lambda x: x[1]["avg_latency_ms"]
                ),
                "throughput": sorted(
                    model_stats.items(),
                ...
                "stability": sorted(
                    model_stats.items(), key=lambda x: x[1]["stability"], reverse=True
                ),
            }
            ...
            comparison["summary"] = {
                ...
                "best_accuracy": max(
                    model_stats.values(), key=lambda x: x["avg_accuracy"]
                ),
                "fastest_model": min(
                    model_stats.values(), key=lambda x: x["avg_latency_ms"]
                ),
    ...
    async def _comprehensive_drift_analysis(self, model_id: str) -> None:
        ...
        try:
            ...
            logger.error(
                f"Failed comprehensive drift analysis for {model_id}: {str(e)}"
            )

    async def _statistical_drift_test(
        self, model_id: str, metric_name: str, values: List[float]
    ) -> None:
        """Run statistical tests for drift detection"""
        try:
            if len(values) < 20:
            ...
            try:
                ...
                for model_id in self.performance_history.keys():
                    ...
                    if health_status and health_status.status in [
                        ...
                        logger.warning(
                            f"Model {model_id} health status: {health_status.status.value} "
        ...
        try:
            ...
            report = {
                ...
                "performance_summary": {
                    ...
                    "avg_throughput": np.mean(
                        [m.throughput_per_second for m in metrics]
                    ),
                ...
                "health_status": await self.get_model_health_status(model_id),
                "detailed_metrics": [
                    m.to_dict() for m in metrics[-100:]
                ],  # Last 100 records
            }
            ...
            logger.error(
                f"Failed to export performance report for {model_id}: {str(e)}"
            )
            return {}
