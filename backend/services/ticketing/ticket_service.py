...
class TicketService:
    ...
    def create_draft_ticket(
        self, user_id: Optional[int], stake: float, edge_ids: List[int]
    ) -> TicketDTO:
        """
        ...
        try:
            ...
            for edge in edges:
                ...
                valuation = (
                    ...
                    .filter(Valuation.id == edge.valuation_id)
                    .first()
                )

                if not valuation:
                    raise TicketValidationError(
                        f"Valuation not found for edge {edge.id}", "VALUATION_NOT_FOUND"
                    )
            ...
            raise TicketValidationError(
                f"Failed to create ticket: {str(e)}", "CREATION_FAILED"
            )
        finally:
            session.close()
    ...
    def submit_ticket(self, ticket_id: int) -> TicketDTO:
        ...
        if not ticket:
            raise TicketValidationError(
                f"Ticket {ticket_id} not found", "TICKET_NOT_FOUND"
            )

        if ticket.status != TicketStatus.DRAFT:
            raise TicketValidationError(
                f"Ticket {ticket_id} is not in DRAFT status", "INVALID_STATUS"
            )

        # Load ticket legs
        legs = session.query(TicketLeg).filter(TicketLeg.ticket_id == ticket_id).all()
        ...
        for leg in legs:
            ...
            if not edge:
                raise TicketValidationError(
                    f"Edge {leg.edge_id} not found", "EDGE_NOT_FOUND"
                )

            if edge.status != EdgeStatus.ACTIVE:
                raise TicketValidationError(
                    f"Edge {leg.edge_id} is no longer active", "EDGE_STATE_CHANGED"
                )
            ...
            if (
                not current_valuation
                or current_valuation.valuation_hash != leg.valuation_hash_snapshot
            ):
                raise TicketValidationError(
                    f"Valuation changed for edge {leg.edge_id}", "EDGE_STATE_CHANGED"
                )
        ...
        try:
                ...
                "legs": [
                    {
                        ...
                        "correlation_cluster_id": getattr(
                            leg, "correlation_cluster_id", None
                        ),
            ...
            critical_findings = [
                ...
                if getattr(f.level, "value", None) == "CRITICAL"
            ]
            ...
            try:
                ...
                violations = [
                    d for d in exposure_decisions if not getattr(d, "allowed", True)
                ]
                if violations:
                    violation_messages = [
                        getattr(d, "reason", None) or "Exposure limit exceeded"
                        for d in violations
                    ]
                    raise TicketValidationError(
        ...
        try:
            ...
            raise TicketValidationError(
                f"Failed to submit ticket: {str(e)}", "SUBMISSION_FAILED"
            )
        finally:
            session.close()
    ...
    def get_ticket(self, ticket_id: int) -> TicketDTO:
        ...
        try:
            ...
            legs = (
                session.query(TicketLeg).filter(TicketLeg.ticket_id == ticket_id).all()
            )
    ...
    def recalc_ticket(self, ticket_id: int) -> TicketDTO:
        ...
        try:
            ...
            raise TicketValidationError(
                f"Failed to recalculate ticket: {str(e)}", "RECALC_FAILED"
            )
        finally:
            session.close()
    ...
    def _load_and_validate_edges(
        self, session: Session, edge_ids: List[int]
    ) -> List[Edge]:
        """Load and validate edges"""
        ...
        if len(edges) != len(edge_ids):
            ...
            raise TicketValidationError(
                f"Edges not found: {missing_ids}", "EDGES_NOT_FOUND"
            )
        ...
        if inactive_edges:
            raise TicketValidationError(
                f"Inactive edges: {inactive_edges}", "INACTIVE_EDGES"
            )
    ...
    def _enforce_correlation_constraints(
        self, prop_ids: List[int], correlation_matrix: Dict[int, Dict[int, float]]
    ):
        """Enforce correlation constraints"""
        if len(prop_ids) <= 1:
            return
    ...
    def _build_ticket_dto(
        ...
        return TicketDTO(
            ...
            submitted_at=(
                ticket.submitted_at.isoformat() if ticket.submitted_at else None
            ),
        )
    ...
    def _schedule_llm_prefetch(self, edge_ids: List[int]):
        ...
        logger.debug(
            ...
            extra={"edge_ids": edge_ids, "action": "schedule_llm_prefetch"},
        )
...
def create_draft_ticket(
    user_id: Optional[int], stake: float, edge_ids: List[int]
) -> TicketDTO:
