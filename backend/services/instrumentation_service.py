...
class InstrumentationService:
    ...
    async def trace_operation(
        ...
        if not self.active_flags.get("tracing_enabled", True):
            ...
            dummy_span = TraceSpan(
                span_id="disabled", operation=operation, start_time=time.time()
            )
            yield dummy_span
            return
        ...
        if np.random.random() > self.active_flags.get("span_sampling_rate", 1.0):
            dummy_span = TraceSpan(
                span_id="sampled_out", operation=operation, start_time=time.time()
            )
            yield dummy_span
            return
        ...
        try:
            ...
            if duration_ms > warn_threshold:
                logger.warning(
                    f"Long-running operation: {operation} took {duration_ms:.2f}ms"
                )

            logger.debug(
                f"Completed span: {operation} [{span_id}] in {duration_ms:.2f}ms"
            )

    async def trace_ev_enrichment(
        self, player_id: str, market_type: str, enrichment_func, *args, **kwargs
    ) -> Any:
        """
        ...
        metadata = {
            "function_name": (
                enrichment_func.__name__
                if hasattr(enrichment_func, "__name__")
                else str(enrichment_func)
            ),
            "args_count": len(args),
        ...
        async with self.trace_operation(
            "ev_enrichment", tags=tags, metadata=metadata
        ) as span:
            ...
            # When deciding whether to convert a single kwarg into a positional
            # argument for legacy callers, ignore internal kwargs that start
            # with an underscore (these are used for control flags). This makes
            # the behavior robust if helpers or middleware inject control keys.
            user_kw_keys = [k for k in call_kwargs.keys() if not k.startswith("_")]

            if not preserve_kwargs and not call_args and len(user_kw_keys) == 1:
                # Convert the single user-provided kw value to a positional arg
                single_key = user_kw_keys[0]
                single_value = call_kwargs.pop(single_key)
                # Remove any leftover internal keys but leave them in call_kwargs
                # in case the enrichment function expects them; however for the
                # legacy single-arg path we pass only the single positional value.
            ...
            # Add result metadata
            span.metadata.update(
                {"result_type": type(result).__name__, "has_result": result is not None}
            )
    ...
    async def trace_arbitrage_detection(
        self, odds_data: List[Dict[str, Any]], detection_func, *args, **kwargs
    ) -> Any:
        """
        ...
        tags = {
            ...
            "books_count": str(
                len(set(item.get("sportsbook", "") for item in odds_data))
            ),
        }

        metadata = {
            "function_name": (
                detection_func.__name__
                if hasattr(detection_func, "__name__")
                else str(detection_func)
            ),
        ...
        async with self.trace_operation(
            "arbitrage_detection", tags=tags, metadata=metadata
        ) as span:
            ...
            span.metadata.update(
                {
                    ...
                    "market_types": list(
                        set(item.get("market_type", "") for item in odds_data)
                    ),
                }
            )
    ...
    async def trace_odds_normalization(
        self, raw_odds: List[Dict[str, Any]], normalization_func, *args, **kwargs
    ) -> Any:
        """
        ...
        metadata = {
            "function_name": (
                normalization_func.__name__
                if hasattr(normalization_func, "__name__")
                else str(normalization_func)
            ),
        ...
        async with self.trace_operation(
            "odds_normalization", tags=tags, metadata=metadata
        ) as span:
            ...
            span.metadata.update(
                {
                    ...
                    "odds_formats": list(
                        set(item.get("format", "decimal") for item in raw_odds)
                    ),
                }
            )
            ...
            span.metadata.update(
                {
                    ...
                    "normalization_ratio": (
                        normalized_count / len(raw_odds) if raw_odds else 0
                    ),
                    "result_type": type(result).__name__,
                }
            )
    ...
    async def trace_line_movement_snapshot(
        self, sport: str, player: str, market: str, snapshot_func, *args, **kwargs
    ) -> Any:
        """
        ...
        metadata = {
            "function_name": (
                snapshot_func.__name__
                if hasattr(snapshot_func, "__name__")
                else str(snapshot_func)
            ),
        ...
        async with self.trace_operation(
            "line_movement_snapshot", tags=tags, metadata=metadata
        ) as span:
            ...
            if isinstance(result, dict):
                movement_detected = (
                    result.get("movement_detected", False)
                    or result.get("magnitude", 0) > 0
                )
    ...
    async def _hash_and_track_error(
        self, error: Exception, operation: str, span_id: str
    ) -> str:
        """
    ...
    def _normalize_stack_trace(self, stack_trace: str) -> str:
        ...
        normalized = re.sub(
            r"\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}", "YYYY-MM-DD HH:MM:SS", normalized
        )
    ...
    async def get_observability_snapshot(self) -> Dict[str, Any]:
        ...
        return {
            "timings": {
                ...
                "arbitrage_ms_avg": round(
                    self.timing_aggregates["arbitrage_ms_avg"], 2
                ),
                "odds_norm_ms_avg": round(
                    self.timing_aggregates["odds_norm_ms_avg"], 2
                ),
                "line_movement_ms_avg": round(
                    self.timing_aggregates["line_movement_ms_avg"], 2
                ),
            },
            ...
            "instrumentationHealth": {
                ...
                "errorHashingEnabled": self.active_flags.get(
                    "error_hashing_enabled", True
                ),
                "metricsCollectionEnabled": self.active_flags.get(
                    "metrics_collection_enabled", True
                ),
...
async def trace_ev_enrichment(player_id: str, market_type: str, func, *args, **kwargs):
    ...
    # Convenience wrapper intentionally does not force preservation of kwargs
    # so that legacy callers which pass a single kwarg can continue to be
    # invoked positionally by the underlying service implementation.
    # Tests and many call sites expect kwargs to be preserved when using
    # this convenience wrapper. Ensure we default to preserving kwargs
    # unless the caller explicitly overrides the control flag.
    if "_preserve_call_kwargs" not in kwargs:
...
async def trace_arbitrage_detection(
    odds_data: List[Dict[str, Any]], func, *args, **kwargs
):
...
async def trace_odds_normalization(
    raw_odds: List[Dict[str, Any]], func, *args, **kwargs
):
...
async def trace_line_movement_snapshot(
    sport: str, player: str, market: str, func, *args, **kwargs
):
...
def instrument_operation(operation_name: str, tags: Optional[Dict[str, str]] = None):
    ...
    def decorator(func):
        async def async_wrapper(*args, **kwargs):
            async with instrumentation_service.trace_operation(
                operation_name, tags=tags
            ):
