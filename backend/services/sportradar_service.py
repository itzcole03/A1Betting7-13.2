...
class SportradarService:
    ...
    def __init__(self):
        from backend.services.unified_config import UnifiedConfigManager

        self.config = UnifiedConfigManager().get_config()
        self.api_key = getattr(self.config.external_apis, "sportradar_api_key", None)
    ...
    def _initialize_endpoints(
        self,
    ) -> Dict[Tuple[SportType, DataType], SportradarEndpoint]:
    ...
    async def initialize(self):
        ...
        enhanced_data_pipeline.register_data_source(
            "sportradar", failure_threshold=3, recovery_timeout=120, success_threshold=2
        )
    ...
    async def _test_connection(self):
        ...
        async def test_fetch():
            async with self.session.get(
                f"{test_url}?api_key={self.api_key}"
            ) as response:
                if response.status == 200:
                    ...
                    raise Exception(
                        "Sportradar API access forbidden - check subscription"
                    )
                else:
                    raise Exception(f"Sportradar API test failed: {response.status}")

        await enhanced_data_pipeline.fetch_data_with_resilience(
            "sportradar_test", test_fetch, use_cache=False
        )

    async def get_schedules(
        self, sport: SportType, year: int = None, season: str = "REG"
    ...
    async def get_player_stats(
        self, sport: SportType, player_id: str
    ) -> Optional[Dict[str, Any]]:
    ...
    async def get_team_stats(
        self, sport: SportType, team_id: str
    ) -> Optional[Dict[str, Any]]:
        """Get team statistics"""
    ...
    async def get_all_sports_data(
        self, sports: List[SportType] = None
        ...
        for sport in sports:
            ...
            requests.append(
                (f"live_scores_{sport.value}", self.get_live_scores, (sport,), {})
            )

            # Add schedules request
            requests.append(
                (f"schedules_{sport.value}", self.get_schedules, (sport,), {})
            )

            # Add injury reports request
            requests.append(
                (f"injuries_{sport.value}", self.get_injury_reports, (sport,), {})
            )

        # Execute parallel requests
        results = await enhanced_data_pipeline.fetch_parallel_data(
            requests, max_failures=len(requests) // 2  # Allow 50% failures
        )
    ...
    async def _make_api_request(
        self, url: str, endpoint: SportradarEndpoint
    ) -> Dict[str, Any]:
        ...
        async with self.session.get(full_url) as response:
            ...
            if response.status == 429:
                ...
                logger.warning(
                    f"⚠️ Sportradar rate limit hit, retry after {retry_after}s"
                )
                raise Exception(
    ...
    async def start_push_feeds(self, sports: List[SportType] = None):
        ...
        try:
            ...
            self._push_feed_task = asyncio.create_task(self._manage_push_feeds(sports))
        ...
        while True:
            try:
                ...
                if self.active_subscriptions:
                    logger.debug(
                        f"💓 Push feed heartbeat - {len(self.active_subscriptions)} active subscriptions"
                    )
    ...
    async def get_health_status(self) -> Dict[str, Any]:
        ...
        health = {
            ...
            "push_feeds": {
                "active": bool(
                    self._push_feed_task and not self._push_feed_task.done()
                ),
                "subscriptions": len(self.active_subscriptions),
                "subscription_types": list(self.active_subscriptions),
            },
            "endpoints": {"total": len(self.endpoints), "by_sport": {}},
        }
