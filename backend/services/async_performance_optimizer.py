...
class AsyncPerformanceOptimizer:
    ...
    def setup_logging(self) -> logging.Logger:
        ...
        # Module-level env-gate: avoid expensive debug formatting on hot paths unless explicitly enabled
        _APO_DEBUG = os.getenv("ASYNC_PERFORMANCE_OPTIMIZER_DEBUG", "").lower() in (
            "1",
            "true",
            "yes",
        )
        if not _APO_DEBUG:
            # make debug a cheap no-op when disabled
    ...
    async def initialize_async_resources(self):
        ...
        try:
            ...
            if HAS_ASYNCPG:
                ...
                self.logger.warning(
                    "asyncpg not installed - database features disabled"
                )
    ...
    def sync_to_async(
        ...
        async def async_wrapper(*args: Any, **kwargs: Any) -> T:
            ...
            return await loop.run_in_executor(
                self.thread_pool, functools.partial(sync_func, *args, **kwargs)
            )
    ...
    async def parallel_requests(
        ...
        self.logger.info(
            f"Parallel requests completed: {len(requests)} requests in {total_time:.3f}s"
        )
    ...
    async def async_prediction_endpoint(self, prediction_data: Dict) -> Dict:
        ...
        try:
            ...
            asyncio.create_task(
                self.async_store_prediction(prediction_data, prediction)
            )
    ...
    def generate_async_recommendations(self) -> List[str]:
        ...
        avg_improvement = sum(m.improvement_percent for m in self.metrics) / len(
            self.metrics
        )

        if avg_improvement > 50:
            recommendations.append(
                "Excellent async performance! Consider applying to more endpoints"
            )
        elif avg_improvement > 20:
            recommendations.append(
                "Good async improvements. Focus on I/O-heavy operations"
            )
        else:
            recommendations.append(
                "Limited async benefits. Review implementation for optimization opportunities"
            )

        # Check for specific patterns
        io_heavy_endpoints = [
            m for m in self.metrics if "prediction" in m.endpoint.lower()
        ]
        if io_heavy_endpoints:
            avg_io_improvement = sum(
                m.improvement_percent for m in io_heavy_endpoints
            ) / len(io_heavy_endpoints)
            if avg_io_improvement > 30:
                recommendations.append(
                    "I/O-heavy endpoints show good async benefits - prioritize these"
                )
...
async def main():
    ...
    try:
        ...
        print(
            f"Prediction result: {result['prediction']['outcome']} "
