...
from backend.models.analytics import ArbitrageHistory, EVOpportunityHistory
...
def _truncate(value: Optional[float], decimals: int = 2) -> float:
    ...
    decimal_value = Decimal(str(value)).quantize(
        Decimal(quantize_pattern), rounding=ROUND_DOWN
    )
...
class AnalyticsPersistenceService:
    ...
    async def persist_arbitrage_opportunity(
        self, data: ArbitrageOpportunityData
    ) -> bool:
        """
        ...
        try:
            ...
            async with self._get_lock(opp_hash):
                async with self._session_scope() as session:
                    ...
                    existing = await unified_session_execute(
                        session,
    ...
    async def _persist_arbitrage_opportunity_async(
        self, data: ArbitrageOpportunityData
    ) -> None:
        ...
        try:
            ...
            async with self._get_lock(arb_hash):
                async with self._session_scope() as session:
                    ...
                    if existing.scalar_one_or_none():
                        logger.debug(
                            f"Arbitrage opportunity already persisted: {arb_hash}"
                        )
                        return
        ...
        try:
            async with self._session_scope() as session:
                ...
                for i in range(days):
                    ...
                    # Get day's data
                    day_data = await unified_session_execute(
                        session,
                        select(EVOpportunityHistory).where(
                            and_(
        ...
        try:
            async with self._session_scope() as session:
                ...
                for i in range(days):
                    ...
                    total_opps = len(opportunities)
                    avg_profit = (
                        sum(opp.profit_pct for opp in opportunities) / total_opps
                    )
    ...
    async def prune_old_records(self) -> Dict[str, int]:
        ...
        try:
            async with self._session_scope() as session:
                ...
                ev_cutoff = datetime.now(timezone.utc) - timedelta(
                    days=EV_HISTORY_RETENTION_DAYS
                )
                arb_cutoff = datetime.now(timezone.utc) - timedelta(
                    days=ARB_HISTORY_RETENTION_DAYS
                )

                # Delete old EV records
                ev_result = await unified_session_execute(
                    session,
                ...
                arb_result = await unified_session_execute(
                    session,
                ...
                logger.info(
                    f"Pruned {ev_deleted} EV records, {arb_deleted} arbitrage records"
                )
    ...
    async def get_summary_stats(self) -> Dict[str, Any]:
        ...
        try:
            async with self._session_scope() as session:
                ...
                ev_tier_result = await unified_session_execute(
                    session,
                    select(
                ...
                high_ev_result = await unified_session_execute(
                    session,
                    select(func.count(EVOpportunityHistory.id)).where(
                        and_(
