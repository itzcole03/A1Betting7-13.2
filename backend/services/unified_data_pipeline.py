...
class UnifiedDataPipeline:
            ...
            async with session.get(
                ...
                if response.status == 200:
                    ...
                    if request.validate_response:
                        quality = self.quality_validator.validate_data(data, request.source)
                    
                    # Check if quality meets threshold
                    if quality.value in ['poor', 'invalid'] and request.quality_threshold != DataQuality.POOR:
                        self.metrics['quality_degradations'] += 1
                        logger.warning(f"Quality degradation for {request.source}: {quality}")
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        return {
            **self.metrics,
            'cache_hit_rate': self.metrics['cache_hits'] / max(1, self.metrics['requests_processed']),
            'quality_degradation_rate': self.metrics['quality_degradations'] / max(1, self.metrics['requests_processed']),
