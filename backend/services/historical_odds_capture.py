            ...
            if _cache_instance is None:
                try:
                    ...
                    logging.getLogger(__name__).debug(
                        "Historical odds cache unavailable: %s", exc
                    )
                    return None
...
class OddsSnapshot:
    ...
    def _calculate_implied_probability(
        self, american_odds: Optional[int]
    ) -> Optional[float]:
...
class HistoricalOddsCapture:
    ...
    def __init__(self):
        ...
        # Configuration
        self.capture_interval = self.config.get(
            "odds_capture_interval_seconds", 300
        )  # 5 minutes
        self.supported_sports = self.config.get(
    ...
    async def start_capture(self):
        ...
        self.logger.info(
            f"Started historical odds capture (interval: {self.capture_interval}s)"
        )
    ...
    async def _capture_loop(self):
        ...
        while self.is_running:
            try:
                ...
                capture_duration = (
                    datetime.now(timezone.utc) - capture_start
                ).total_seconds()
    ...
    async def _cache_snapshot(self, key: str, data: Dict[str, Any]):
        ...
        try:
            ...
            self.logger.warning("Failed to cache snapshot %s: %s", key, exc)
    ...
    async def _cleanup_old_snapshots(self):
        ...
        self.snapshots_storage = [
            s for s in self.snapshots_storage if s.captured_at > cutoff_time
        ]
    ...
    async def get_historical_odds(
        self, prop_id: str, hours_back: int = 24, sportsbook: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        ...
        filtered_snapshots = [
            s
            for s in self.snapshots_storage
            if (
                s.prop_id == prop_id
                and s.captured_at > cutoff_time
                and (sportsbook is None or s.sportsbook == sportsbook)
            )
        ]
    ...
    async def get_line_movement_summary(
        self, prop_id: str, sportsbook: str
    ) -> Dict[str, Any]:
        """Calculate line movement summary for a prop"""
        snapshots = await self.get_historical_odds(
            prop_id, hours_back=24, sportsbook=sportsbook
        )
        ...
        if first_snapshot["over_odds"] and latest_snapshot["over_odds"]:
            odds_movement_over = (
                latest_snapshot["over_odds"] - first_snapshot["over_odds"]
            )
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_capture": max(
                (s.captured_at for s in self.snapshots_storage), default=None
            ),
        }
