...
from ..services.unified_error_handler import ErrorContext, unified_error_handler
...
async def _get_cache() -> Optional[Any]:
    ...
    if not _CACHE_AVAILABLE or get_cache is None:
        logger.debug("Unified cache backend unavailable for JWT service")
        return None
        ...
        async with _cache_lock:
            if _cache_instance is None:
                try:
                    ...
                    logger.debug("JWT cache initialization failed: %s", exc)
                    return None
...
class TokenBlacklist:
    ...
    async def blacklist_token(self, token_jti: str, expires_at: datetime):
        ...
        if ttl_seconds > 0:
            ...
            try:
                ...
                logger.debug(
                    "Failed to cache blacklist entry for %s: %s", token_jti, exc
                )
    ...
    async def is_blacklisted(self, token_jti: str) -> bool:
        ...
        if cache is None:
            return False
        try:
            result = await cache.get(f"blacklist:{token_jti}")
        except Exception as exc:
            logger.debug("Failed to read blacklist entry for %s: %s", token_jti, exc)
            return False
    ...
    async def blacklist_user_tokens(self, user_id: str):
        ...
        if cache is None:
            return
        try:
            active_tokens = await cache.get(f"user_tokens:{user_id}")
        except Exception as exc:
            logger.debug("Failed to load active tokens for %s: %s", user_id, exc)
            active_tokens = []
        ...
        for token_info in active_tokens:
            ...
            if not token_jti or not isinstance(expires_at, str):
                continue
            try:
                expire_dt = datetime.fromisoformat(expires_at)
            except Exception as exc:
                logger.debug("Invalid expires_at for %s: %s", token_jti, exc)
                continue
            await self.blacklist_token(token_jti, expire_dt)

        # Clear user's token list
        try:
            await cache.delete(f"user_tokens:{user_id}")
        except Exception as exc:
            logger.debug("Failed to clear active tokens for %s: %s", user_id, exc)
...
class JWTManager:
    ...
    async def create_token_pair(self, user_claims: UserClaims) -> TokenPair:
        ...
        await self._store_token_metadata(
            user_claims.user_id, refresh_jti, refresh_payload["exp"]
        )
    ...
    async def _store_token_metadata(
        self, user_id: str, token_jti: str, expires_at: datetime
    ):
        ...
        user_tokens = await cache.get(f"user_tokens:{user_id}")
        ...
        if len(user_tokens) > self.config.max_refresh_tokens_per_user:
            ...
            await self.blacklist.blacklist_token(
                oldest_token["jti"], datetime.fromisoformat(oldest_token["expires_at"])
            )
...
async def refresh_tokens(request: Request, response: Response):
    ...
    try:
        ...
        logger.info(
            ...
            extra={
                "user_id": new_token_pair.access_token
                and jwt.decode(
                    new_token_pair.access_token,
        ...
        return {
            ...
            "refresh_expires_in": jwt_manager.config.refresh_token_expire_days
            * 24
            * 3600,
        }
        ...
        unified_error_handler.handle_error(
            e, ErrorContext(endpoint="/api/auth/refresh", method="POST")
        )
        raise HTTPException(
...
async def revoke_token(
    ...
    try:
        ...
        logger.info("Token revoked", extra={"user_id": current_user.get("sub")})
        ...
        unified_error_handler.handle_error(
            e, ErrorContext(endpoint="/api/auth/revoke", method="POST")
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def revoke_all_tokens(current_user: Dict[str, Any] = Depends(get_current_user)):
    ...
    try:
        ...
        logger.info("All tokens revoked", extra={"user_id": current_user.get("sub")})
        ...
        unified_error_handler.handle_error(
            e, ErrorContext(endpoint="/api/auth/revoke-all", method="POST")
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
