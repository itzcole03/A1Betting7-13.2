...
from backend.services.health.health_collector import (
    get_health_collector,
    map_statuses_to_overall,
)
...
from .metrics_window_aggregator import (
    add_reliability_metrics,
    metrics_window_aggregator,
)
...
class ReliabilityOrchestrator:
    ...
    async def generate_report(self, include_traces: bool = False) -> Dict[str, Any]:
        ...
        if (
            self._report_cache
            and current_time - self._last_report_time < self._cache_ttl
            and self._report_cache.get("include_traces") == include_traces
        ):
        ...
        try:
            ...
            add_reliability_metrics(
                {
                    ...
                    "partial_refresh_count": optimization_stats.get(
                        "partial_refresh_count", 0
                    ),
                    "avg_refresh_latency_ms": optimization_stats.get(
                        "avg_refresh_latency_ms", 0
                    ),
                    "rationale_requests": rationale_stats.get("requests", 0),
                    "rationale_cache_hit_rate": rationale_stats.get(
                        "cache_hit_rate", 0.0
                    ),
                }
            )
            ...
            # Ensure 'traces' key always present for clients/tests that expect it
            # If traces weren't requested, present an empty list.
            ...
            logger.info(
                f"Reliability report generated in {report['generation_time_ms']}ms"
            )
    ...
    async def _collect_model_registry_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                total_models = (
                    await registry_service.count_models()
                    if hasattr(registry_service, "count_models")
                    else 0
                )
                ...
                if hasattr(registry_service, "get_default_model"):
                    try:
                        ...
                        if default_model_info:
                            default_model = {
                                "id": getattr(
                                    default_model_info, "model_id", "unknown"
                                ),
                                "version": getattr(
                                    default_model_info, "version", "unknown"
                                ),
                            }
                    except Exception:
                        default_model = None
            ...
            "services": [
                {
                    ...
                    "status": "degraded",
                    "latency_ms": None,
                    "details": {"error": "health_collector_unavailable"},
                }
            ],
            "performance": {
                "cpu_percent": 0.0,
    ...
    def _determine_overall_status(
        self, health_data: Dict[str, Any], anomalies: List[Dict[str, Any]]
    ) -> str:
        """
        ...
        try:
            ...
            if anomalies:
                ...
                has_warning = any(
                    anomaly.get("severity") == "warning" for anomaly in anomalies
                )
    ...
    def _generate_report_notes(
        self, snapshot: Dict[str, Any], anomalies: List[Dict[str, Any]]
    ) -> List[str]:
        """
        ...
        try:
            ...
            if performance.get("cpu_percent", 0) > 70:
                notes.append(
                    f"High CPU usage: {performance.get('cpu_percent', 0):.1f}%"
                )
            ...
            if (
                cache.get("hit_rate", 1.0) < 0.5
                and (cache.get("hits", 0) + cache.get("misses", 0)) > 50
            ):
    ...
    def _log_reliability_event(self, report: Dict[str, Any]) -> None:
        ...
        try:
            ...
            event_data = {
                ...
                "p95_request_latency_ms": report.get("performance", {}).get(
                    "p95_request_latency_ms", 0
                ),
                "cache_hit_rate": report.get("cache", {}).get("hit_rate", 0),
                "active_connections": report.get("websocket", {}).get(
                    "active_connections", 0
                ),
            ...
            logger.info(
                "Reliability report generated", extra={"reliability_report": event_data}
            )
