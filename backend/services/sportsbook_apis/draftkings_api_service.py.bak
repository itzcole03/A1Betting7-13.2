...
class DraftKingsAPIService:
            ...
            async with self._cache_lock:
                if self._cache is None:
                    try:
                        ...
                        logger.debug("DraftKings cache unavailable: %s", exc)
                        return None
        return self._cache
        ...
        try:
            ...
            async with session.get(url, params=params) as response:
                if response.status == 200:
                    ...
                    logger.error(
                        f"DraftKings API error: {response.status} - {await response.text()}"
                    )
                    return None
        ...
        try:
            ...
            if cache is not None:
                try:
                    ...
                    logger.debug(
                        "DraftKings events cache fetch failed for %s: %s",
                        cache_key,
                        exc,
                    )
            else:
                logger.debug("DraftKings cache backend not configured for events")
            if cached_data:
            ...
            for group in event_groups:
                for category in group.get("offerCategories", []):
                    for subcategory in category.get("offerSubcategoryDescriptors", []):
                        for offer_group in subcategory.get("offerSubcategory", {}).get(
                            "offers", []
                        ):
            ...
            if cache is not None:
                try:
                    ...
                    logger.debug(
                        "DraftKings events cache store failed for %s: %s",
                        cache_key,
                        exc,
                    )
        ...
        try:
            ...
            if cache is not None:
                try:
                    ...
                    logger.debug(
                        "DraftKings odds cache fetch failed for %s: %s",
                        cache_key,
                        exc,
                    )
            else:
                logger.debug("DraftKings cache backend not configured for odds")
            if cached_data:
            ...
            if cache is not None:
                try:
                    ...
                    logger.debug(
                        "DraftKings odds cache store failed for %s: %s",
                        cache_key,
                        exc,
                    )
    ...
    async def _get_event_odds(
        self, event_id: str, markets: Optional[List[DraftKingsMarket]] = None
    ) -> List[DraftKingsOdds]:
        ...
        try:
            ...
            if cache is not None:
                try:
                    ...
                    logger.debug(
                        "DraftKings props cache fetch failed for %s: %s",
                        cache_key,
                        exc,
                    )
            else:
                logger.debug("DraftKings cache backend not configured for props")
            if cached_data:
                return [DraftKingsOdds(**odds) for odds in cached_data]
            ...
            if cache is not None:
                try:
                    ...
                    logger.debug(
                        "DraftKings props cache store failed for %s: %s",
                        cache_key,
                        exc,
                    )
                ...
                for subcategory in category.get("subcategories", []):
                    for market in subcategory.get("markets", []):
                        ...
                        if prop_types and not any(
                            prop_type.lower() in market_name for prop_type in prop_types
                        ):
                            continue
    ...
    def _parse_player_prop(
        self, outcome_data: Dict[str, Any], market_data: Dict[str, Any], event_id: str
    ) -> Optional[DraftKingsOdds]:
    ...
    def _extract_player_name(
        self, market_name: str, outcome_label: str
    ) -> Optional[str]:
