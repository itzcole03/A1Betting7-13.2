...
class BetMGMAPIService:
    ...
    async def _get_cache(self) -> Optional[Any]:
        """Return the shared cache backend when available."""
            ...
            async with self._cache_lock:
                if self._cache is None:
                    try:
                        ...
                        logger.debug("BetMGM cache unavailable: %s", exc)
                        return None
        return self._cache
    ...
    async def _check_rate_limit(self) -> bool:
        ...
        # Remove old requests
        self.last_requests = [
            req_time for req_time in self.last_requests if req_time > cutoff
        ]
        ...
        try:
            ...
            async with session.get(url, params=params) as response:
                if response.status == 200:
                    ...
                    logger.error(
                        f"BetMGM API error: {response.status} - {await response.text()}"
                    )
                    return None
        ...
        try:
            ...
            if cache is not None:
                try:
                    ...
                    logger.debug(
                        "BetMGM events cache fetch failed for %s: %s", cache_key, exc
                    )
            else:
                logger.debug("BetMGM cache backend not configured for events")
            if cached_data:
            ...
            params = {"lang": "en", "country": "US", "timezone": "America/New_York"}
            ...
            if cache is not None:
                try:
                    ...
                    logger.debug(
                        "BetMGM events cache store failed for %s: %s", cache_key, exc
                    )
        ...
        try:
            ...
            if cache is not None:
                try:
                    ...
                    logger.debug(
                        "BetMGM odds cache fetch failed for %s: %s", cache_key, exc
                    )
            else:
                logger.debug("BetMGM cache backend not configured for odds")
            if cached_data:
            ...
            if cache is not None:
                try:
                    ...
                    logger.debug(
                        "BetMGM odds cache store failed for %s: %s", cache_key, exc
                    )
    ...
    async def _get_event_odds(
        self, event_id: str, markets: Optional[List[BetMGMMarket]] = None
    ) -> List[BetMGMOdds]:
        """Get odds for a specific event"""

        try:
            # BetMGM uses fixture IDs
            params = {"fixtureIds": event_id, "lang": "en", "country": "US"}
                    ...
                    for outcome in game.get("outcomes", []):
                        ...
                        if odds:
                            ...
                            if markets and not any(
                                market.value in odds.market_type for market in markets
                            ):
                                continue
        ...
        try:
            ...
            if cache is not None:
                try:
                    ...
                    logger.debug(
                        "BetMGM props cache fetch failed for %s: %s", cache_key, exc
                    )
            else:
                logger.debug("BetMGM cache backend not configured for props")
            if cached_data:
                return [BetMGMOdds(**odds) for odds in cached_data]
            ...
            for event in events[:6]:  # Limit to avoid rate limits
                event_props = await self._get_event_player_props(
                    event.id, player_name, prop_types
                )
            ...
            if cache is not None:
                try:
                    ...
                    logger.debug(
                        "BetMGM props cache store failed for %s: %s", cache_key, exc
                    )
        ...
        try:
            ...
            for fixture in fixtures:
                ...
                for game in games:
                    ...
                    if prop_types and not any(
                        prop_type.lower() in game_name for prop_type in prop_types
                    ):
                        continue
                    ...
                    for outcome in game.get("outcomes", []):
                        ...
                        if prop:
                            ...
                            if (
                                ...
                                not in (prop.player_name or "").lower()
                            ):
                                continue
    ...
    def _parse_event_data(
        self, event_data: Dict[str, Any], is_live: bool
    ) -> Optional[BetMGMEvent]:
        ...
        try:
            ...
            league = (
                competition.get("name", {}).get("value", "")
    ...
    def _parse_odds(
        ...
        try:
            # Extract basic info
            selection = (
                outcome_data.get("name", {}).get("value", "")
                if isinstance(outcome_data.get("name"), dict)
                else str(outcome_data.get("name", ""))
            )
            ...
            # Extract event info
            fixture_name = (
                fixture_data.get("name", {}).get("value", "")
                if isinstance(fixture_data.get("name"), dict)
                else str(fixture_data.get("name", ""))
            )
            ...
            game_name = (
                game_data.get("name", {}).get("value", "")
                if isinstance(game_data.get("name"), dict)
                else str(game_data.get("name", ""))
            )
            ...
            return BetMGMOdds(
                ...
                sport=self._normalize_sport_name(
                    fixture_data.get("sport", {}).get("name", {}).get("value", "")
                ),
                league=fixture_data.get("competition", {})
                .get("name", {})
                .get("value", ""),
                home_team=home_team,
                away_team=away_team,
