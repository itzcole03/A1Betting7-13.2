...
# Env-gated debug suppression for batch prediction optimizer. When the
# DEBUG env var is unset, raise the logger level to INFO to avoid debug
# formatting overhead on tight prediction loops.
_BATCH_PREDICTION_OPTIMIZER_DEBUG = bool(os.getenv("BATCH_PREDICTION_OPTIMIZER_DEBUG"))
if not _BATCH_PREDICTION_OPTIMIZER_DEBUG:
    try:
...
class BatchPredictionOptimizer:
    ...
    def __init__(
        ...
        logger.info(
            f"Batch prediction optimizer initialized: batch_size={max_batch_size}, timeout={batch_timeout}s"
        )
    ...
    def register_model(
        self, model_name: str, model: Any, batch_predict_fn: Optional[callable] = None
    ):
    ...
    async def _process_batch(self, batch: List[BatchPredictionRequest]):
        ...
        try:
            async with self.processing_lock:
                ...
                logger.debug(
                    f"Processed batch of {len(batch)} requests in {batch_time:.3f}s"
                )
    ...
    def _group_by_models(
        self, batch: List[BatchPredictionRequest]
    ) -> Dict[Tuple[str, ...], List[BatchPredictionRequest]]:
    ...
    async def _process_model_group(
        self, models: Tuple[str, ...], requests: List[BatchPredictionRequest]
    ) -> Dict[str, BatchPredictionResponse]:
            ...
            if model_name in self.registered_models:
                ...
                try:
                    ...
                    self._update_model_performance(
                        model_name, len(uncached_requests), process_time
                    )
        ...
        for i, request in enumerate(uncached_requests):
            try:
                ...
                for model_name in models:
                    if model_name in model_results and i < len(
                ...
                shap_values = {
                    name: np.random.uniform(-0.05, 0.05)
                    for name in request.features.keys()
                }
                ...
                response = BatchPredictionResponse(
                    ...
                    processing_time=(
                        time.time() - batch_features[i]
                        if hasattr(batch_features[i], "__time__")
                        else 0.0
                    ),
                )
                ...
                logger.error(
                    f"Error creating response for request {request.request_id}: {e}"
                )
                responses[request.request_id] = BatchPredictionResponse(
                    request_id=request.request_id,
                    prediction=0.0,
    ...
    def _check_cache(
        self, requests: List[BatchPredictionRequest]
    ) -> Tuple[Dict[str, BatchPredictionResponse], List[BatchPredictionRequest]]:
    ...
    def _update_batch_stats(self, batch_size: int, processing_time: float):
        ...
        self.stats.avg_processing_time = (
            self.stats.avg_processing_time * (total_batches - 1) + processing_time
        ) / total_batches
        ...
        if time_since_reset > 0:
            self.stats.throughput_per_second = (
                self.stats.total_requests / time_since_reset
            )

        # Update cache hit rate
        cache_info = getattr(
            self.prediction_cache, "info", lambda: {"hits": 0, "misses": 0}
        )()
    ...
    def _update_model_performance(
        self, model_name: str, request_count: int, processing_time: float
    ):
        ...
        if model_name in self.registered_models:
            ...
            self.model_performance[model_name]["total_predictions"] = model_info[
                "prediction_count"
            ]
            self.model_performance[model_name]["throughput"] = (
                request_count / processing_time if processing_time > 0 else 0
            )
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "detailed_stats": {
                ...
                "recent_processing_times": self.detailed_stats["processing_times"][
                    -10:
                ],
                "avg_batch_size_last_100": (
                    np.mean(self.detailed_stats["batch_sizes"][-100:])
                    if self.detailed_stats["batch_sizes"]
                    else 0
                ),
                "avg_processing_time_last_100": (
                    np.mean(self.detailed_stats["processing_times"][-100:])
                    if self.detailed_stats["processing_times"]
                    else 0
                ),
            },
        }
