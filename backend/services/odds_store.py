...
class OddsStoreService:
    ...
    async def initialize_bookmakers(self, session: AsyncSession) -> List[Bookmaker]:
        ...
        try:
            ...
            result = await session.execute(
                select(Bookmaker).where(Bookmaker.name.in_(names))
            )
            ...
            if bookmakers_to_add:
                ...
                try:
                    ...
                    self.logger.info(
                        f"Initialized {len(bookmakers_to_add)} new bookmakers"
                    )
        ...
        try:
            for odds_data in bookmaker_odds:
                ...
                if (
                    ...
                    and odds_data.over_odds
                    and odds_data.under_odds
                ):
                    try:
                        # Convert to decimal odds
                        over_decimal = self.odds_normalizer.american_to_decimal(
                            odds_data.over_odds
                        )
                        under_decimal = self.odds_normalizer.american_to_decimal(
                        ...
                        self.logger.warning(
                            f"Error normalizing odds for {prop_id}: {e}"
                        )
            ...
            await session.commit()
            self.logger.info(
                f"Stored {len(snapshots)} odds snapshots for prop {prop_id}"
            )
    ...
    async def get_best_line(
        self, session: AsyncSession, prop_id: str, max_age_minutes: int = 30
        ...
        try:
            ...
            query = (
                select(OddsSnapshot)
                .options(selectinload(OddsSnapshot.bookmaker))
                .where(
                    and_(
            ...
            for snapshot in snapshots:
                ...
                if (
                    book_id not in latest_by_bookmaker
                    or snapshot.captured_at > latest_by_bookmaker[book_id].captured_at
                ):
            ...
            arbitrage_opportunity, arbitrage_profit = (
                BestLineCalculator.detect_arbitrage(
                    best_data.get("best_over_odds"), best_data.get("best_under_odds")
                )
            )
            ...
            result = BestLineResult(
                ...
                best_over_bookmaker=(
                    best_data.get("best_over_bookmaker").short_name
                    if best_data.get("best_over_bookmaker")
                    else None
                ),
                best_under_odds=best_data.get("best_under_odds"),
                best_under_bookmaker=(
                    best_data.get("best_under_bookmaker").short_name
                    if best_data.get("best_under_bookmaker")
                    else None
                ),
            ...
            for snapshot in snapshots:
                ...
                if book_name in prev_by_bookmaker:
                    ...
                    if snapshot.over_odds is not None and prev.over_odds is not None:
                        movement_data["over_odds_movement"] = (
                            snapshot.over_odds - prev.over_odds
                        )
                    if snapshot.under_odds is not None and prev.under_odds is not None:
                        movement_data["under_odds_movement"] = (
                            snapshot.under_odds - prev.under_odds
                        )
    ...
    async def detect_steam_moves(
        self, session: AsyncSession, prop_id: str, lookback_minutes: int = 30
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            for i in range(len(snapshots) - 2):
                window_snapshots = [
                    s
                    for s in snapshots
                    if abs((s.captured_at - snapshots[i].captured_at).total_seconds())
                    <= window_size * 60
                ]
                        ...
                        steam_moves.append(
                            {
                                ...
                                "confidence": min(
                                    1.0, len(line_movements) / 5.0
                                ),  # Confidence based on book count
                                "max_movement": max(
                                    m["movement"] for m in line_movements
                                ),
                            }
                        )
    ...
    async def _update_best_line_aggregate(self, prop_id: str, sport: str) -> None:
        ...
        try:
            ...
            if not SQLALCHEMY_AVAILABLE:
                self.logger.warning(
                    "SQLAlchemy not available - skipping best line aggregate update"
                )
                return
            ...
            async with AsyncSession(async_engine) as session:
                try:
                    ...
                    if not best:
                        self.logger.info(
                            f"No best line data for {prop_id}, skipping aggregate upsert"
                        )
                        return
                    ...
                    query = select(BestLineAggregate).where(
                    ...
                    async def _resolve_bookmaker_id(name_or_short):
                        try:
                            ...
                            q = select(Bookmaker).where(
                                (Bookmaker.short_name.ilike(name_or_short))
                                | (Bookmaker.name.ilike(name_or_short))
                                | (Bookmaker.display_name.ilike(name_or_short))
                            )
                    ...
                    if existing:
                        ...
                        existing.best_over_bookmaker_id = await _resolve_bookmaker_id(
                            best.best_over_bookmaker
                        )
                        existing.best_under_odds = best.best_under_odds
                        existing.best_under_bookmaker_id = await _resolve_bookmaker_id(
                            best.best_under_bookmaker
                        )
                        ...
                        agg = BestLineAggregate(
                            ...
                            best_over_bookmaker_id=await _resolve_bookmaker_id(
                                best.best_over_bookmaker
                            ),
                            best_under_odds=best.best_under_odds,
                            best_under_bookmaker_id=await _resolve_bookmaker_id(
                                best.best_under_bookmaker
                            ),
                    ...
                    self.logger.error(
                        f"Error updating aggregate inside session for {prop_id}: {inner_e}"
                    )
        except Exception as e:
    ...
    def get_bookmaker_by_name(
        ...
        for bookmaker in bookmakers:
            if (
                bookmaker.name.lower() == name.lower()
                or bookmaker.short_name.lower() == name.lower()
            ):
                return bookmaker
        return None
...
async def store_prop_odds(
    prop_id: str,
    sport: str,
    market_type: str,
    bookmaker_odds_dict: Dict[str, Dict[str, int]],
) -> List[Dict[str, Any]]:
    """
...
def create_enhanced_bookmaker_response(
    bookmaker_odds_dict: Dict[str, Dict[str, int]],
    ...
    try:
        ...
        for book_name, odds_data in bookmaker_odds_dict.items():
            ...
            if odds is not None:
                ...
                if best_odds is None or BestLineCalculator._is_better_odds(
                    odds, best_odds
                ):
                    best_odds = odds
