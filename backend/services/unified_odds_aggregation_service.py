...
from .sportsbook_apis.betmgm_api_service import BetMGMOdds, get_betmgm_service

# Sportsbook services
from .sportsbook_apis.draftkings_api_service import (
    DraftKingsOdds,
...
if TYPE_CHECKING:  # pragma: no cover - hints only
    from .core.unified_cache_service import UnifiedCacheService
...
class UnifiedOddsAggregationService:
            ...
            async with self._cache_lock:
                if self._cache is None:
                    try:
                        ...
                        logger.debug("Unified odds cache unavailable: %s", exc)
                        return None
        return self._cache
    ...
    async def get_aggregated_odds(
        ...
        try:
            ...
            # Check cache first
            cache_ttl = (
                self.cache_ttl["live_odds"]
                if live_only
                else self.cache_ttl["pre_game_odds"]
            )
            cached_data = None
            if cache is not None:
                try:
                    ...
                    logger.debug(
                        "Unified odds cache fetch failed for %s: %s", cache_key, exc
                    )
            else:
                logger.debug("Unified odds cache backend not configured")
            if cached_data:
            ...
            if include_arbitrage:
                for odds in aggregated_odds:
                    odds.arbitrage_opportunity = (
                        await self._detect_arbitrage_opportunity(odds)
                    )
            ...
            if cache is not None:
                try:
                    ...
                    logger.debug(
                        "Unified odds cache store failed for %s: %s", cache_key, exc
                    )
    ...
    async def _collect_odds_from_all_sportsbooks(
        ...
        try:
            ...
            dk_odds = await dk_service.get_odds(
                event_id=event_id, sport=sport, live_only=live_only
            )
    ...
    def _aggregate_odds_by_market(
        self, all_odds: Dict[str, List[Any]]
    ) -> List[UnifiedOdds]:
    ...
    def _create_unified_odds(self, market_data: Dict[str, Any]) -> UnifiedOdds:
        ...
        best_sportsbook = next(
            odds["sportsbook"]
            for odds in market_data["odds_data"]
        ...
        market_efficiency = (
            1.0 / (1.0 + statistics.variance(odds_values))
    ...
    async def get_arbitrage_opportunities(
        ...
        try:
            ...
            aggregated_odds = await self.get_aggregated_odds(
                sport=sport, include_arbitrage=True
            )
            ...
            for odds in aggregated_odds:
                if odds.arbitrage_opportunity:
                    ...
                    if arb_data["profit_margin"] >= min_profit_margin:
                        ...
                        stake_distribution = self._calculate_optimal_stakes(
                            arb_data["selections"]
                        )
                        ...
                        opportunity = ArbitrageOpportunity(
                            ...
                            sportsbooks_involved=[
                                s["sportsbook"] for s in arb_data["selections"]
                            ],
            ...
            if cache is not None:
                try:
                    await cache.set(
                        cache_key, opp_data, ttl=self.cache_ttl["arbitrage"]
                    )
                except Exception as exc:  # pragma: no cover - defensive guard
                    logger.debug(
                        "Unified odds cache store failed for %s: %s", cache_key, exc
    ...
    async def get_best_lines(
        ...
        try:
            # Get aggregated odds
            aggregated_odds = await self.get_aggregated_odds(
                sport=sport, market_types=market_types
            )
    ...
    async def track_line_movement(
        self, event_id: str, market_type: str, lookback_hours: int = 24
    ) -> List[LineMovement]:
        ...
        try:
            ...
            if cache is not None:
                try:
                    await cache.set(
                        cache_key, movement_data, ttl=self.cache_ttl["line_movement"]
                    )
                except Exception as exc:  # pragma: no cover - defensive guard
                    logger.debug(
                        "Unified odds cache store failed for %s: %s", cache_key, exc
    ...
    async def _generate_mock_line_movement(
            ...
            return {
                ...
                "current_stats": {
                    ...
                    "avg_sportsbooks_per_market": round(
                        sum(len(odds.odds_data) for odds in current_odds)
                        / max(len(current_odds), 1),
                        1,
                    ),
    ...
    async def capture_odds_snapshot(
        ...
        try:
            ...
            self.historical_odds[prop_id][sportsbook] = [
                s
                for s in self.historical_odds[prop_id][sportsbook]
    ...
    async def get_prop_history(
        ...
        try:
            ...
            snapshots = sorted(snapshots, key=lambda x: x.captured_at, reverse=True)[
                :limit
            ]
    ...
    async def analyze_line_movement(
        self, prop_id: str, sportsbook: str, hours_back: int = 24
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            analysis = await analytics_service.analyze_prop_movement(
                prop_id=prop_id, sportsbook=sportsbook, historical_data=historical_data
            )
    ...
    async def detect_steam_movement(self, prop_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            steam_alert = await analytics_service.detect_steam_across_books(
                prop_id=prop_id, all_books_data=all_books_data
            )
