...
from sqlmodel import select, func, SQLModel, delete, desc
...
class CLVPersistenceService:
    ...
    def __init__(self):
        ...
        if not self.enabled:
            ...
            asyncio.create_task(self._ensure_clv_table())
            ...
            async with db_manager.get_session() as session:
                try:
                    ...
                    if "UNIQUE constraint failed" in str(e) or "duplicate" in str(e).lower():
                        logger.debug(f"Some CLV records already exist - persisting unique ones only")
                        ...
                        for record in records:
                            try:
                                ...
                                if "UNIQUE constraint failed" not in str(individual_error):
                                    logger.debug(f"Individual record persistence error: {individual_error}")
                        logger.debug(f"Persisted {success_count}/{len(records)} CLV records (duplicates skipped)")
    ...
    async def get_recent(
    ...
    ) -> List[Any]:  # Changed from List[CLVHistory] to avoid type issues when CLVHistory is None
        ...
        try:
            async with db_manager.get_session() as session:
                stmt = select(CLVHistory).order_by(desc(CLVHistory.computed_at)).limit(limit)
    ...
    async def get_summary(
        ...
        try:
            async with db_manager.get_session() as session:
                ...
                ).select_from(CLVHistory).where(CLVHistory.computed_at >= cutoff)
    ...
    async def prune_old_records(self, days: int = 7) -> int:
        ...
        try:
            async with db_manager.get_session() as session:
                ...
                logger.debug(f"Pruned {deleted_count} CLV records older than {days} days")
