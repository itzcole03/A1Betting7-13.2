...
# Module-level env-gate to reduce hot-path debug formatting cost
_ANALYTICS_SCHEDULER_DEBUG = os.getenv("ANALYTICS_SCHEDULER_DEBUG", "").lower() in (
    "1",
    "true",
    "yes",
)
if not _ANALYTICS_SCHEDULER_DEBUG:
...
class AnalyticsScheduler:
    ...
    async def _scheduler_loop(self) -> None:
        ...
        while self.running:
            try:
                ...
                logger.info(
                    f"Next analytics prune scheduled for {next_prune} (in {sleep_seconds:.0f}s)"
                )
    ...
    def _get_next_prune_time(self, current_time: datetime) -> datetime:
        ...
        target_time = current_time.replace(
            hour=self.prune_hour, minute=self.prune_minute, second=0, microsecond=0
        )
    ...
    async def _pre_calculate_daily_stats(self) -> None:
        ...
        try:
            ...
            logger.debug(
                f"Pre-calculated arbitrage stats for 7 days: {len(arb_stats)} records"
            )

            # Test summary stats
            summary = await self.analytics_service.get_summary_stats()
            logger.debug(
                f"Pre-calculated summary stats: EV avg={summary['ev']['avg']:.2f}%"
            )
