...
try:
    ...
    UnifiedCacheService = Any  # type: ignore[assignment]
...
async def _get_cache() -> Optional[Any]:
    ...
    if not _CACHE_AVAILABLE or get_cache is None:
        logger.debug(
            "Unified cache backend not available; skipping model selection cache"
        )
        return None
        ...
        async with _cache_lock:
            if _cache_instance is None:
                try:
                    ...
                    logger.debug("Model selection cache unavailable: %s", exc)
                    return None
...
class ModelSelectionService:
    ...
    def __init__(self):
        ...
        self._cache_ttl_seconds = int(self._cache_ttl.total_seconds())
    ...
    def _initialize_default_features(self):
        ...
        try:
            ...
            logger.warning("Could not initialize default features: %s", e)
    ...
    async def select_best_model(
        ...
        if cached_result:
            logger.info("Using cached model selection for %s", criteria.sport)
        ...
        try:
            ...
            filtered_models = await self._apply_feature_flag_filtering(
                candidate_models, criteria
            )

            # Apply performance filtering
            performance_models = await self._apply_performance_filtering(
                filtered_models, criteria
            )

            # Select best model using scoring algorithm
            selected_model, selection_reason, confidence = (
                await self._score_and_select_model(performance_models, criteria)
            )
            ...
            result = ModelSelectionResult(
                ...
                alternative_models=[
                    m.model_id
                    for m in performance_models
                    if m.model_id != selected_model.model_id
                ],
            )
            ...
            logger.info(
                "Selected model %s for %s (reason: %s)",
    ...
    async def _apply_feature_flag_filtering(
        self, models: List[Any], criteria: ModelSelectionCriteria
    ) -> List[Any]:
        ...
        canary_enabled = self.feature_flags.is_feature_enabled(
            "model_canary_testing", criteria.user_context
        )
        ...
        if canary_enabled:
            ...
            return [
                m
                for m in models
                if m.status in [ModelStatus.STABLE, ModelStatus.CANARY]
            ]
        else:
            # User only gets stable models
            return [m for m in models if m.status == ModelStatus.STABLE]

    async def _apply_performance_filtering(
        self, models: List[Any], criteria: ModelSelectionCriteria
    ) -> List[Any]:
        """Filter models based on performance criteria"""
        performance_based = self.feature_flags.is_feature_enabled(
            "performance_based_selection", criteria.user_context
        )
        ...
        for model in models:
            ...
            avg_latency = (
                ...
                if metrics.successful_inferences > 0
                else float("inf")
            )

            # Apply performance thresholds
            if (
                success_rate >= criteria.performance_threshold
                and avg_latency <= criteria.max_latency_ms
            ):
                filtered_models.append(model)
    ...
    async def _fallback_selection(
        self, criteria: ModelSelectionCriteria, error: str
    ) -> ModelSelectionResult:
        """Fallback model selection when primary selection fails"""
        logger.warning("Using fallback model selection due to error: %s", error)

        # Get any stable model for the sport
        stable_models = self.registry.list_models(
            sport=criteria.sport, status=ModelStatus.STABLE
        )
        ...
        if stable_models:
            ...
            return ModelSelectionResult(
                ...
                selection_reason="fallback_selection_due_to_error",
            ...
            if active_models:
                ...
                return ModelSelectionResult(
                    ...
                    model_version=fallback_model.version,
                    model_status=fallback_model.status.value,
                    selection_reason="emergency_fallback_selection",
                    confidence_score=0.3,  # Very low confidence for emergency fallback
                    fallback_used=True,
                )
            else:
                raise ValueError(
                    f"No models available for fallback selection in sport {criteria.sport}"
                )
    ...
    async def _get_cached_selection(
        self, cache_key: str
        ...
        if cache is not None:
            try:
                ...
                logger.debug("Unable to read model selection cache: %s", exc)
            else:
                if isinstance(cached_payload, dict):
                    try:
                        return ModelSelectionResult(**cached_payload)
                    except TypeError as exc:  # pragma: no cover - defensive guard
                        logger.debug(
                            "Discarding invalid cached model selection: %s", exc
                        )
    ...
    async def _cache_selection(self, cache_key: str, result: ModelSelectionResult):
        ...
        if cache is not None:
            try:
                await cache.set(cache_key, asdict(result), ttl=self._cache_ttl_seconds)
            except Exception as exc:  # pragma: no cover - cache backend failure
                logger.debug("Unable to persist model selection cache entry: %s", exc)
    ...
    async def _cleanup_cache(self):
        ...
        expired_keys = [
            key
            for key, (_, timestamp) in self._selection_cache.items()
    ...
    async def get_model_selection_stats(
        self, sport: Optional[str] = None, days: int = 7
    ) -> Dict[str, Any]:
        """Get model selection statistics for analysis"""
        try:
            models = (
                self.registry.list_models(sport=sport)
                if sport
            ...
            overall_success_rate = (
                (total_success / total_inferences * 100) if total_inferences > 0 else 0
            )
            ...
            logger.error("Failed to get model selection stats: %s", e)
            return {"error": str(e)}
    ...
    def create_user_context(
        ...
        return UserContext(
            user_id=user_id, user_groups=user_groups or [], attributes=attributes or {}
        )
