...
class LegacyRegistry:
    ...
    def __init__(self):
        ...
        logger.info(
            f"Legacy registry initialized: enabled={self._enabled}, sunset_date={self._sunset_date}"
        )
    ...
    def is_enabled(self) -> bool:
        ...
        # Re-evaluate environment flag at call time to respect test-time
        # environment patching and avoid import-order sensitivity.
        try:
            return self._get_legacy_enabled()
        except Exception:
            return self._enabled

    def get_sunset_date(self) -> Optional[str]:
        """Get configured sunset date for legacy endpoints"""
        # Read the environment variable at call time so tests or runtime
        # patches to A1_LEGACY_SUNSET are respected without needing to
        # recreate the registry instance.
        try:
            env_val = os.getenv("A1_LEGACY_SUNSET")
            return env_val if env_val is not None else self._sunset_date
        except Exception:
    ...
    def increment_usage(self, path: str) -> bool:
        ...
        if path not in self._data:
            ...
            for registered_path, data in self._data.items():
                ...
                if path.startswith(registered_path):
                    if matching_prefix is None or len(registered_path) > len(
                        matching_prefix
                    ):
        ...
        if self._prometheus_client:
            try:
                ...
                logger.debug(
                    f"Would record Prometheus metric for legacy endpoint: {path}"
                )
            except Exception as e:
                logger.debug(f"Failed to update Prometheus legacy metrics: {e}")

        logger.debug(
            f"Legacy endpoint usage incremented: {path} (count: {self._data[path].count})"
        )
        return True
    ...
    def get_usage_data(self) -> Dict[str, Any]:
            ...
            endpoints.append(
                {
                    ...
                    "last_access_iso": (
                        datetime.fromtimestamp(
                            endpoint_data.last_access_ts, tz=timezone.utc
                        ).isoformat()
                        if endpoint_data.last_access_ts
                        else None
                    ),
                }
            )
        ...
        return {
            # Use dynamic getters so runtime env patches are respected in tests
            "enabled": self.is_enabled(),
            "endpoints": endpoints,
            "total": total_calls,
            "first_recorded_ts": self._start_time,
            "first_recorded_iso": datetime.fromtimestamp(
                self._start_time, tz=timezone.utc
            ).isoformat(),
            "since_seconds": int(now - self._start_time),
            "sunset_date": self.get_sunset_date(),
        }
    ...
    def get_migration_readiness(self, threshold: int = 50) -> Dict[str, Any]:
        ...
        total_calls_last_24h = sum(
            ...
            for endpoint_data in self._data.values()
            if endpoint_data.last_access_ts
            and (now - endpoint_data.last_access_ts) <= 86400
        )
        ...
        return {
            ...
            "analysis": {
                ...
                "recommendations": self._get_migration_recommendations(
                    score, usage_rate_per_hour, threshold
                ),
            },
        }

    def _get_migration_recommendations(
        self, score: float, usage_rate: float, threshold: int
    ) -> List[str]:
        ...
        if score >= 0.8:
            recommendations.append(
                "✅ Safe to proceed with legacy endpoint deprecation"
            )
            recommendations.append(
                "Monitor usage for 1-2 weeks after deprecation notices"
            )
        elif score >= 0.5:
            ...
            recommendations.append(
                "Focus on client migration before deprecating endpoints"
            )
            recommendations.append(
                f"Current usage ({usage_rate:.1f}/hr) exceeds threshold ({threshold}/hr)"
            )
...
# Global registry instance (lazy-initialized to respect test-time env patches)
legacy_registry: Optional[LegacyRegistry] = None


def get_legacy_registry() -> LegacyRegistry:
    """Get (and lazily create) the global legacy registry instance.

    This avoids import-time side-effects so tests can patch environment
    variables (like A1_LEGACY_ENABLED) before the registry is created.
    """
    global legacy_registry
