...
def _edc_debug(*args, **kwargs):
    """Emit debug logs only when EVENT_DRIVEN_CACHE_DEBUG is truthy."""
    if _EDC_DEBUG:
...
class EventDrivenCacheManager:
    ...
    async def register_cache_dependency(
        self, primary_key: str, dependent_keys: List[str]
    ):
        ...
        _edc_debug(
            f"🔗 Registered dependencies for {primary_key}: {len(dependent_keys)} keys"
        )

    async def invalidate_cache_immediately(
        self, patterns: List[str], event: CacheEvent = None
    ) -> int:
        ...
        try:
            ...
            for pattern in patterns:
                ...
                _edc_debug(
                    f"🗑️ Invalidated {invalidated_count} keys for pattern: {pattern}"
                )
            ...
            logger.info(
                f"✅ Invalidated {total_invalidated} cache keys in {processing_time:.3f}s"
            )
    ...
    async def _handle_cascade_invalidation(self, primary_pattern: str) -> int:
        ...
        try:
            ...
            _edc_debug(f"🌊 Handling cascade invalidation for: {primary_pattern}")
    ...
    async def _process_invalidation_rule(
        self, event: CacheEvent, rule: InvalidationRule
    ):
    ...
    async def _add_to_batch(self, event: CacheEvent, rule: InvalidationRule):
        ...
        if batch_key not in self.pending_invalidations:
            ...
            asyncio.create_task(
                self._schedule_batch_processing(batch_key, rule.delay_seconds)
            )

            _edc_debug(f"📦 Added event to batch: {batch_key}")
        ...
        while True:
            try:
                ...
                if self.pending_invalidations:
                    _edc_debug(
                        f"📦 Processing {len(self.pending_invalidations)} pending batches"
                    )
    ...
    async def _cleanup_old_events(self):
        ...
        while True:
            try:
                ...
                old_batches = [
                    batch_key
                    for batch_key, event in self.pending_invalidations.items()
    ...
    async def get_invalidation_stats(self) -> Dict[str, Any]:
        ...
        return {
            "stats": {
                ...
                "last_invalidation": (
                    self.stats.last_invalidation.isoformat()
                    if self.stats.last_invalidation
                    else None
                ),
            },
            "rules": {
                ...
                "enabled_rules": len(
                    [r for r in self.invalidation_rules.values() if r.enabled]
                ),
                "rules_by_event_type": {},
            },
            "current_state": {
