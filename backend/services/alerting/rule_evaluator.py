...
class AlertRuleEvaluator:
    ...
    def __init__(self):
        ...
        # Risk configuration is stored under the "risk" section of unified config
    ...
    async def evaluate_all_rules(self) -> List[AlertEvent]:
        ...
        try:
            ...
            logger.debug(
                f"Evaluated {len(active_rules)} rules, triggered {len(events)} events"
            )
            return events
    ...
    async def _evaluate_line_movement(self, rule: Dict[str, Any]) -> List[AlertEvent]:
        ...
        raw_threshold = rule["parameters"].get("movement_threshold", 0.5)
        try:
            movement_threshold = float(raw_threshold)
        except (TypeError, ValueError):
            movement_threshold = 0.5

        raw_window = rule["parameters"].get("time_window_hours", 4)
        try:
            time_window_hours = int(raw_window)
        except (TypeError, ValueError):
            time_window_hours = 4

        movements = await self._get_line_movements(
            rule,
        ...
        for movement in movements:
            ...
            new_line = movement.get("new_line")
            ...
            event = AlertEvent(
                ...
                title=f"Line {direction_word}: {line_change:+.1f}",
                message=(
                    f"{movement.get('player', 'Unknown')} {movement.get('market', '')} "
                    f"line {direction_word} from {old_line_str} to {new_line_str} "
                    f"over the last {time_window_hours}h"
                ).strip(),
                data={
                    "prop_id": movement.get("prop_id"),
                    ...
                    "movement_time": triggered_at.isoformat(),
                    "time_window_hours": time_window_hours,
                    "movement_threshold": movement_threshold,
                    "sportsbook": movement.get("sportsbook"),
                },
                triggered_at=triggered_at,
                expires_at=triggered_at + timedelta(hours=1),
            )
            events.append(event)

        return events

    async def _evaluate_valuation_change(
        self, rule: Dict[str, Any]
    ) -> List[AlertEvent]:
        """Evaluate valuation change alert rule"""
        change_threshold = rule["parameters"].get(
        ...
        for change in changes:
            if abs(change["valuation_change"]) >= change_threshold:
                ...
                event = AlertEvent(
                    ...
                    severity=self._calculate_valuation_severity(
                        abs(change["valuation_change"])
                    ),
    ...
    async def _evaluate_watchlist_update(
        self, rule: Dict[str, Any]
    ) -> List[AlertEvent]:
    ...
    async def _get_line_movements(
        ...
        *,
        ...
        """Fetch detailed movement candidates from the line movement service."""
        ...
        min_volatility = parameters.get("min_volatility")

        return await service.get_movement_alert_candidates(
            hours_back=hours,
            movement_threshold=movement_threshold,
        ...
        return [
            {
                ...
                "details": {"prop_type": "points", "line": 25.5, "odds": -110},
            }
        ]
