...
class OptimizedRealTimeDataService:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.priority_queues: Dict[int, asyncio.Queue] = {
            i: asyncio.Queue() for i in range(1, 6)
        }
        ...
        logger.info(
            f"OptimizedRealTimeDataService initialized with {len(self.api_endpoints)} API endpoints"
        )
    ...
    async def _initialize_redis(self) -> None:
        ...
        try:
            ...
            logger.warning(
                f"Redis initialization failed: {e}. Falling back to in-memory cache."
            )
    ...
    async def _start_stream_processor(self) -> None:
        """Launch the stream processor task if it is not already running."""
    ...
    async def get_player_data(
        self, player_id: str, sport: str = "MLB", force_refresh: bool = False
        ...
        if not force_refresh:
            ...
            if cached_data:
                ...
                if (
                    self._validate_player_data_quality(cached_data)
                    >= DataQuality.MEDIUM
                ):
                    return cached_data
                else:
        ...
        future = asyncio.create_task(
            self._fetch_player_data_multi_source(player_id, sport)
        )
        ...
        try:
            ...
            if player_data:
                ...
                await self._cache_data(
                    cache_key, player_data, ttl=self.config.get("player_data_ttl", 120)
                )
    ...
    async def _fetch_player_data_multi_source(
        ...
        player_endpoints = [
            ep for ep in self.api_endpoints if "player" in ep.name.lower()
        ]
        ...
        for endpoint in player_endpoints:
            try:
                ...
                # Fetch data
                data = await self._fetch_from_endpoint(
                    endpoint, f"/players/{player_id}", {"sport": sport}
                )
                ...
                if data:
                    ...
                    aggregated_data = self._merge_player_data(
    ...
    async def search_players(
        ...
        search_endpoints = [
            ...
            if "search" in ep.name.lower() or "player" in ep.name.lower()
        ]
        ...
        for endpoint in sorted(search_endpoints, key=lambda x: x.priority):
            try:
                ...
                if results:
                    normalized_results = [
                        self._normalize_player_data(r, endpoint.name) for r in results
                    ]
        ...
        # Cache results
        await self._cache_data(
            cache_key, ranked_results, ttl=self.config.get("search_results_ttl", 600)
        )
    ...
    async def stream_live_data(self, data_types: List[str], callback: Callable) -> str:
        ...
        subscription_id = hashlib.md5(
            f"{data_types}_{time.time()}".encode()
        ).hexdigest()
        ...
        logger.info(
            f"Created subscription {subscription_id} for data types: {data_types}"
        )
    ...
    async def _rate_limit_allow(self, endpoint_name: str) -> bool:
        ...
        endpoint_config = next(
            (ep for ep in self.api_endpoints if ep.name == endpoint_name), None
        )
        ...
        limiter["minute_requests"] = [
            t for t in limiter["minute_requests"] if t > minute_cutoff
        ]
        limiter["hour_requests"] = [
            t for t in limiter["hour_requests"] if t > hour_cutoff
        ]
    ...
    def _validate_player_data_quality(self, data: Dict[str, Any]) -> DataQuality:
        ...
        if fetched_at:
            try:
                ...
                age = (
                    datetime.now(timezone.utc).replace(tzinfo=fetch_time.tzinfo)
                    - fetch_time
                )
    ...
    def _normalize_player_data(
        ...
        normalized = {
            ...
            "_source": source,
            "_normalized_at": datetime.now(timezone.utc)
            .isoformat()
            .replace("+00:00", "Z"),
        }
    ...
    def _normalize_stats(self, stats: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for source_key, value in stats.items():
            if source_key in stat_mappings:
                ...
                try:
                    normalized_stats[target_key] = (
                        target_type(value) if value is not None else None
                    )
    ...
    async def _fetch_from_endpoint(
        self, endpoint: APIEndpoint, path: str, params: Dict[str, Any]
    ) -> Optional[Dict[str, Any]]:
                ...
                async with aiohttp.ClientSession(
                    ...
                    async with session.get(
                        ...
                        if response.status == 200:
                            ...
                            logger.warning(
                                f"API {endpoint.name} returned status {response.status}"
                            )
                            break
            ...
            try:
                ...
                for data_type, qualities in self.data_quality_metrics.items():
                    if qualities:
                        ...
                        logger.info(
                            f"Average data quality for {data_type}: {avg_quality:.2f}"
                        )
                        qualities.clear()
    ...
    def _merge_player_data(
        ...
        for key in ["name", "team", "position", "sport"]:
            if (
                key in new_data
                and new_data[key]
                and (key not in merged or not merged[key])
            ):
...
def get_optimized_real_time_service(
    config: Dict[str, Any],
) -> OptimizedRealTimeDataService:
