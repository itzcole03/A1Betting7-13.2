...
class DataIngestionLog(Base):
    ...
    # 'metadata' is a reserved attribute name on Declarative Base; map the DB column
    # named 'metadata' to a different Python attribute to avoid import-time collisions.
...
class EnterpriseDataPipeline:
    ...
    def __init__(
        ...
        database_url: str = "sqlite:///data_pipeline.db",
    ):
        ...
        # Metrics
        self.pipeline_metrics: Dict[str, DataPipelineMetrics] = defaultdict(
            DataPipelineMetrics
        )
    ...
    def setup_validation_rules(self):
        ...
        self.validation_rules[DataSourceType.PRIZEPICKS] = [
            ...
            DataValidationRule(
                "line_score", "type", {"type": "float"}, "Line score must be numeric"
            ),
            DataValidationRule(
                "line_score",
                "range",
                {"min": 0, "max": 1000},
                "Line score out of valid range",
            ),
        ...
        self.validation_rules[DataSourceType.SPORTSBOOK] = [
            DataValidationRule(
                "odds", "type", {"type": "float"}, "Odds must be numeric"
            ),
            DataValidationRule(
                "odds", "range", {"min": -1000, "max": 1000}, "Odds out of valid range"
            ),
            DataValidationRule(
                "line", "type", {"type": "float"}, "Line must be numeric"
            ),
        ...
        self.validation_rules[DataSourceType.PLAYER_STATS] = [
            ...
            DataValidationRule(
                "stats", "type", {"type": "dict"}, "Stats must be a dictionary"
            ),
        ...
        self.validation_rules[DataSourceType.INJURY_REPORTS] = [
            ...
            DataValidationRule(
                "status",
                "pattern",
                {"pattern": "^(out|questionable|probable|healthy)$"},
    ...
    def setup_data_processors(self):
        ...
        self.data_processors[DataSourceType.PLAYER_STATS] = (
            self.process_player_stats_data
        )
    ...
    async def ingest_data(
        self, source: str, source_type: DataSourceType, data: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        ...
        for item in data:
            try:
                ...
                if data_point.quality != DataQuality.INVALID:
                    ...
                    ingestion_metrics["validation_errors"].extend(
                        data_point.validation_errors
                    )
        ...
        logger.info(
            f"📊 Ingested {ingestion_metrics['valid_records']}/{ingestion_metrics['total_records']} "
            f"valid records from {source} in {processing_time:.2f}s"
        )

        return {"processed_data": processed_data, "metrics": ingestion_metrics}
            ...
            try:
                ...
                if error:
                    ...
                    if rule.severity == "error":
                        ...
                        rule.severity == "warning" and quality == DataQuality.EXCELLENT
                    ):
    ...
    def apply_validation_rule(
        self, data: Dict[str, Any], rule: DataValidationRule
    ) -> Optional[str]:
    ...
    async def process_prizepicks_data(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        processed["market_key"] = (
            f"{processed.get('player_name', '')}_{processed.get('stat_type', '')}"
        )
    ...
    async def process_sportsbook_data(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "over_odds" in processed:
            processed["over_odds_decimal"] = self.american_to_decimal(
                processed["over_odds"]
            )
        if "under_odds" in processed:
            processed["under_odds_decimal"] = self.american_to_decimal(
                processed["under_odds"]
            )
    ...
    async def process_injury_data(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "status" in processed:
            processed["status"] = status_mapping.get(
                processed["status"].lower(), processed["status"]
            )
    ...
    def generate_cache_key(self, data_point: DataPoint) -> str:
        ...
        base_key = (
            f"{data_point.source}:{data_point.source_type.value}:{data_point.data_id}"
        )
        return base_key
        ...
        try:
            log_entry = DataIngestionLog(
                ...
                data_hash=hashlib.md5(
                    json.dumps(data_point.data, sort_keys=True).encode()
                ).hexdigest(),
                quality=data_point.quality.value,
                confidence=data_point.confidence,
            ...
            try:
                ...
                for source, metrics in self.pipeline_metrics.items():
                    if metrics.last_update:
                        ...
                        age = (
                            datetime.now(timezone.utc) - metrics.last_update
                        ).total_seconds()
                        metrics.freshness_score = max(
                            0, 1 - (age / 3600)
                        )  # Decreases over 1 hour
        ...
        while True:
            try:
                if self.redis_client:
                    ...
                    logger.info(
                        f"📊 Cache memory usage: {used_memory / 1024 / 1024:.1f} MB"
                    )
        ...
        while True:
            try:
                ...
                current_metrics = {
                    ...
                    "total_records": sum(
                        m.total_records for m in self.pipeline_metrics.values()
                    ),
                    "valid_records": sum(
                        m.valid_records for m in self.pipeline_metrics.values()
                    ),
                    "error_rate": sum(
                        m.error_rate for m in self.pipeline_metrics.values()
                    )
                    / max(len(self.pipeline_metrics), 1),
                    "avg_throughput": sum(
                        m.throughput for m in self.pipeline_metrics.values()
                    )
                    / max(len(self.pipeline_metrics), 1),
                ...
                # Log summary
                logger.info(
                    f"📊 Pipeline metrics: {current_metrics['valid_records']} valid records, "
            ...
            try:
                ...
                if len(self.performance_history) >= 10:
                    ...
                    if current_throughput < avg_throughput * 0.5:  # 50% drop
                        logger.warning(
                            f"⚠️ Throughput anomaly detected: {current_throughput:.1f} vs avg {avg_throughput:.1f}"
                        )
    ...
    async def get_stream_data(
        self, source: str, source_type: DataSourceType, limit: int = 100
    ) -> List[DataPoint]:
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        health = {
            ...
            "error_rate": sum(m.error_rate for m in self.pipeline_metrics.values())
            / max(len(self.pipeline_metrics), 1),
            "issues": [],
        }
