...
class AnomalyType(str, Enum):
    ...
    SUSPICIOUS_PROFIT_MARGIN = (
        "suspicious_profit_margin"  # Profit too high to be realistic
    )
    ODDS_OUTLIER = "odds_outlier"  # Odds significantly different from market consensus
    STALE_ODDS_DETECTED = (
        "stale_odds_detected"  # One book's odds haven't updated recently
    )
...
class HardenedArbitrageValidator:
    ...
    async def validate_arbitrage_opportunity(
        self, odds_snapshots: List[OddsSnapshot], profit_pct: float
    ) -> ValidationResult:
        """
        ...
        validation_result = ValidationResult(
            is_valid=True, confidence_score=1.0, anomaly_flags=[], validation_notes=[]
        )

        try:
            # 1. Implied Probability Validation
            await self._validate_implied_probabilities(
                odds_snapshots, validation_result
            )

            # 2. Triangle Consistency Check (if enabled and sufficient books)
            if (
                self.config.enable_triangle_validation
                and len(odds_snapshots) >= self.config.min_books_for_validation
            ):
                await self._validate_triangle_consistency(
                    odds_snapshots, validation_result
                )
            ...
            if self.config.enable_anomaly_detection:
                await self._detect_anomalies(
                    odds_snapshots, profit_pct, validation_result
                )
    ...
    async def _validate_implied_probabilities(
        self, odds_snapshots: List[OddsSnapshot], result: ValidationResult
    ):
        """
    ...
    async def _validate_triangle_consistency(
        ...
        try:
            ...
            if len(book_ids) < 3:
                result.validation_notes.append(
                    "Insufficient books for triangle validation"
                )
                return
    ...
    async def _detect_stale_odds(
        ...
        try:
            ...
            stale_threshold = timedelta(
                seconds=self.config.stale_odds_threshold_seconds
            )
                ...
                if len(snapshots) >= 2:
                    ...
                    for i in range(1, len(snapshots)):
                        ...
                        if time_diff > 0 and time_diff < 60:  # Within 1 minute
                            odds_change_pct = (
                                abs(
                                    (curr_snapshot.odds - prev_snapshot.odds)
                                    / prev_snapshot.odds
                                )
                                * 100
                            )
                            ...
                            if odds_change_pct > 10:  # > 10% change in < 1 minute
                                result.anomaly_flags.append(
                                    AnomalyType.RAPID_ODDS_MOVEMENT
                                )
                                result.validation_notes.append(
...
class HardenedArbitrageService:
    ...
    async def detect_arbitrage_opportunities(
        ...
        try:
            ...
            logger.info(
                f"Detected {len(filtered_opportunities)} arbitrage opportunities"
            )
        ...
        for odds_entry in odds_data:
            try:
                snapshot = OddsSnapshot(
                    book_id=odds_entry.get(
                        "book_id", odds_entry.get("sportsbook", "unknown")
                    ),
                    event_id=odds_entry.get("event_id", "unknown"),
                    market_type=odds_entry.get(
                        "market_type", odds_entry.get("market", "unknown")
                    ),
                    outcome=odds_entry.get(
                        "outcome", odds_entry.get("side", "unknown")
                    ),
    ...
    async def _detect_market_arbitrage(
        ...
        try:
            ...
            total_implied_prob = sum(
                1 / snapshot.odds for snapshot in best_odds_by_outcome.values()
            )
            ...
            if total_implied_prob < 1.0:  # Arbitrage exists
                ...
                if profit_pct >= self._runtime_config.min_profit_pct:
                    ...
                    validation_result = (
                        await self.validator.validate_arbitrage_opportunity(
                    ...
                    if (
                        ...
                        or len(validation_result.anomaly_flags) == 0
                    ):
    ...
    async def _create_arbitrage_opportunity(
        ...
        detection_reason = self._determine_detection_reason(
            odds_snapshots, validation_result
        )
        ...
        opportunity = HardenedArbitrageOpportunity(
            ...
            implied_probabilities={
                snapshot.outcome: 1 / snapshot.odds for snapshot in odds_snapshots
            },
    ...
    def _generate_odds_hash(self, odds_snapshots: List[OddsSnapshot]) -> str:
        ...
        sorted_snapshots = sorted(
            odds_snapshots, key=lambda s: (s.book_id, s.outcome, s.odds)
        )
        ...
        for snapshot in sorted_snapshots:
            hash_data.append(
                f"{snapshot.book_id}:{snapshot.outcome}:{snapshot.odds:.4f}"
            )
    ...
    def _determine_detection_reason(
        self, odds_snapshots: List[OddsSnapshot], validation_result: ValidationResult
    ) -> DetectionReason:
        ...
        if len(outcomes) == 2:
            ...
            validation_result.triangle_consistency_score
            and validation_result.triangle_consistency_score < 0.7
        ):
    ...
    def _calculate_execution_risk(self, odds_snapshots: List[OddsSnapshot]) -> float:
        ...
        # Source quality risk
        if odds_snapshots:
            quality_risk = 1.0 - float(
                np.mean([s.source_quality for s in odds_snapshots])
            )
    ...
    async def _check_alerting_thresholds(
        self, opportunities: List[HardenedArbitrageOpportunity]
    ):
        ...
        window_start = current_time - timedelta(
            minutes=self._runtime_config.alert_time_window_minutes
        )
        recent_opportunities = [
            alert for alert in self.alert_window if alert["timestamp"] >= window_start
        ]

        if len(recent_opportunities) > self._runtime_config.alert_volume_threshold:
            # Emit alert
            await self._emit_volume_alert(
                len(recent_opportunities), recent_opportunities
            )

    async def _emit_volume_alert(
        self, opportunity_count: int, recent_opportunities: List[Dict[str, Any]]
    ):
        ...
        alert_data = {
            ...
            "avg_profit_pct": (
                float(np.mean([opp["profit_pct"] for opp in recent_opportunities]))
                if recent_opportunities
                else 0.0
            ),
            "unique_books": len(
                set().union(*[opp["books"] for opp in recent_opportunities])
            ),
            "timestamp": datetime.now(timezone.utc).isoformat(),
        }
    ...
    async def update_arbitrage_config(
        ...
        try:
            ...
            valid_fields = {
                "min_profit_pct",
                "max_profit_pct",
                "max_stake_per_opportunity",
                "alert_volume_threshold",
                "alert_time_window_minutes",
                "enable_anomaly_detection",
                "enable_triangle_validation",
                "enable_cross_market_validation",
                "stale_odds_threshold_seconds",
                "min_books_for_validation",
                "suspicious_profit_threshold",
                "odds_outlier_z_score_threshold",
