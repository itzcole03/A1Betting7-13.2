...
from backend.services.optimized_redis_service import (
    CacheNamespace,
    OptimizedRedisService,
)
...
# Env-gated short-circuit for noisy orchestrator debug logs.
_DATA_SOURCE_ORCHESTRATOR_DEBUG = bool(os.getenv("DATA_SOURCE_ORCHESTRATOR_DEBUG"))
if not _DATA_SOURCE_ORCHESTRATOR_DEBUG:
    try:
...
class DataSourceOrchestrator:
    ...
    async def start_orchestrator(self, num_workers: int = 10):
            ...
            for i in range(worker_count):
                task = asyncio.create_task(
                    self._priority_worker(
                        priority, f"worker-{priority.name.lower()}-{i}"
                    )
                )
        ...
        logger.info(
            f"Data source orchestrator started with {len(self.worker_tasks)} workers"
        )
    ...
    def _calculate_priority_score(self, task: ScheduledTask) -> float:
        ...
        time_factor = max(
            0, (datetime.now() - task.scheduled_time).total_seconds() / 60
        )
    ...
    async def _update_source_metrics(
        self, source: DataSourceType, response: DataResponse, start_time: float
    ):
    ...
    async def _update_source_reliability(self):
        ...
        for source, metrics in self.source_metrics.items():
            ...
            if old_reliability != metrics.reliability:
                await self._handle_reliability_change(
                    source, old_reliability, metrics.reliability
                )
    ...
    async def _handle_reliability_change(
        ...
        logger.info(
            f"Source {source} reliability changed: {old_reliability} → {new_reliability}"
        )
    ...
    async def _handle_quality_degradation(
        self, task: ScheduledTask, response: DataResponse
    ):
        """Handle quality degradation by triggering failover"""
        logger.warning(
            f"Quality degradation detected for {task.request.source}: {response.quality}"
        )
            ...
            if fallback_source in self.active_sources:
                ...
                try:
                    ...
                    if response.status == DataStatus.SUCCESS:
                        ...
                        self.orchestrator_metrics["tasks_completed"] += 1
                        logger.info(
                            f"Successful failover: {task.request.source} → {fallback_source}"
                        )
                        return

                except Exception as e:
                    logger.warning(
                        f"Failover attempt failed for {fallback_source}: {e}"
                    )
                    continue
    ...
    async def _handle_task_failure(self, task: ScheduledTask, error: str):
        ...
        if task.can_retry and not task.is_overdue:
            ...
            lower_priority = min(
                DataPriority.LOW, DataPriority(task.priority.value + 1)
            )
            priority_score = self._calculate_priority_score(task)
            ...
            logger.debug(
                f"Rescheduled failed task with {delay}s delay (attempt {task.retry_count})"
            )
        else:
...
async def get_data_source_orchestrator(
    pipeline: UnifiedDataPipeline, redis_service: OptimizedRedisService
