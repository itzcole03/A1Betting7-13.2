...
from typing import Any, Awaitable, Callable, DefaultDict, Dict, List, Optional, Set

logger = logging.getLogger(__name__)

# Env-gated module-level short-circuit for noisy cache instrumentation debug logs.
# When CACHE_INSTRUMENTATION_DEBUG is not set, make logger.debug a cheap no-op.
_CACHE_INSTRUMENTATION_DEBUG = bool(os.getenv("CACHE_INSTRUMENTATION_DEBUG"))
if not _CACHE_INSTRUMENTATION_DEBUG:
    # When detailed cache instrumentation debug is disabled, raise the logger
    # level to INFO to avoid debug-level formatting and handler work. This is a
    # safe, low-risk alternative to reassigning logger.debug directly.
    try:
        logger.setLevel(max(getattr(logger, "level", logging.INFO), logging.INFO))
    except Exception:
        # If level manipulation fails for any logger implementation, ignore
        # and fall back to doing nothing (debug will still run but this is
        # unlikely in normal environments).
        pass
...
class CacheInstrumentation:
    ...
    def __init__(self, cache_version: str = "v1"):
        ...
        # Namespace tracking
        self._namespace_stats: DefaultDict[str, NamespaceStats] = defaultdict(
            NamespaceStats
        )

        # Tier tracking
        self._tier_stats: DefaultDict[str, DefaultDict[str, int]] = defaultdict(
    ...
    async def _periodic_cleanup(self):
        ...
        while True:
            try:
                ...
                if locks_to_remove:
                    logger.debug(
                        f"🧹 Cleaned up {len(locks_to_remove)} unused stampede locks"
                    )
    ...
    def record_delete(
        self, key: str, namespace: str = "default", tier: str = "default"
    ):
    ...
    def _update_latency(self, latency_ms: float):
        ...
        if self._ewma_latency_ms == 0.0:
            ...
            alpha = 0.1
            self._ewma_latency_ms = (alpha * latency_ms) + (
                (1 - alpha) * self._ewma_latency_ms
            )

    def _update_namespace_stats(
        self,
        namespace: str,
        hits: int = 0,
        misses: int = 0,
        ...
        if exc_type is None:
            ...
            if self.operation == "get_hit":
                if not self._should_record:
                    return False
                self.instrumentation.record_hit(
                    self.key, self.latency_ms, self.namespace, self.tier
                )
            elif self.operation == "get_miss":
                if not self._should_record:
                    return False
                self.instrumentation.record_miss(
                    self.key, self.latency_ms, self.namespace, self.tier
                )
            elif self.operation == "set":
                if not self._should_record:
                    return False
                self.instrumentation.record_set(
                    self.key, self.latency_ms, self.namespace, self.tier
                )
