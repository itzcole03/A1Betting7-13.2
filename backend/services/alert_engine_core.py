...
from backend.services.alerting.rule_evaluator import (
    AlertEvent,
    AlertEventType,
    AlertRuleEvaluator,
)
...
class AlertEngineCore:
    ...
    def __init__(self):
        ...
        self.line_movement_service = None
    ...
    async def initialize(self):
        ...
        try:
            ...
            try:
                self.line_movement_service = await get_line_movement_service()
            except Exception as exc:
                logger.warning("Line movement service unavailable during init: %s", exc)
                self.line_movement_service = None
    ...
    async def start(self):
        ...
        try:
            ...
            await asyncio.gather(
                evaluation_task, rule_sync_task, cleanup_task, return_exceptions=True
            )
        ...
        try:
            ...
            for rule in self.active_rules.values():
                if self._should_evaluate_rule(rule):
                    task = asyncio.create_task(self._evaluate_rule(rule, prop_data))
    ...
    async def _evaluate_rule(
        self, rule: AlertRule, prop_data: List[PropData]
    ) -> List[AlertTrigger]:
        ...
        try:
            if rule.rule_type == AlertRuleType.EV_THRESHOLD:
                ...
                triggers.extend(
                    await self._evaluate_arbitrage_opportunity(rule, prop_data)
                )
    ...
    async def _evaluate_ev_threshold(
        self, rule: AlertRule, prop_data: List[PropData]
    ) -> List[AlertTrigger]:
        ...
        for prop in prop_data:
            if (
                prop.ev_value
                and prop.ev_value >= threshold
                and prop.confidence_score
                and prop.confidence_score >= min_confidence
            ):

                trigger = AlertTrigger(
                    trigger_id=self._generate_trigger_id(
                        rule.rule_id, prop.prop_id, "ev_threshold"
                    ),
    ...
    async def _evaluate_line_movement(
        self, rule: AlertRule, prop_data: List[PropData]
    ) -> List[AlertTrigger]:
        """Evaluate line movement alert rule"""
        triggers = []

        if self.line_movement_service is None:
            try:
                self.line_movement_service = await get_line_movement_service()
            except Exception as exc:
                logger.warning("Line movement service unavailable: %s", exc)
                return triggers

        raw_threshold = rule.conditions.get("movement_threshold", 1.0)
        try:
            movement_threshold = float(raw_threshold)
        except (TypeError, ValueError):
            movement_threshold = 1.0

        raw_window = rule.conditions.get("time_window_hours", 4)
        try:
            time_window_hours = int(raw_window)
        except (TypeError, ValueError):
            time_window_hours = 4

        direction_filter = rule.conditions.get("direction")
        min_volatility = rule.conditions.get("min_volatility")

        candidates = await self.line_movement_service.get_movement_alert_candidates(
            hours_back=time_window_hours,
            movement_threshold=movement_threshold,
            sport=rule.conditions.get("sport"),
            player=rule.conditions.get("player"),
            market=rule.conditions.get("market"),
            direction=direction_filter,
            min_volatility=min_volatility,
        )

        for movement in candidates:
            line_change = movement.get("line_change") or 0.0
            abs_change = abs(line_change)
            ...
            direction_word = (
                "increased"
                if line_change > 0
                else "decreased" if line_change < 0 else "held steady"
            )

            old_line = movement.get("old_line")
            new_line = movement.get("new_line") or old_line
            old_line_str = f"{old_line:.2f}" if old_line is not None else "N/A"
            new_line_str = f"{new_line:.2f}" if new_line is not None else "N/A"

            movement_time = movement.get("movement_time")
            if isinstance(movement_time, datetime):
                movement_time_aware = (
                    movement_time.replace(tzinfo=timezone.utc)
                    if movement_time.tzinfo is None
                    else movement_time.astimezone(timezone.utc)
                )
            else:
                movement_time_aware = datetime.now(timezone.utc)

            prop_line = new_line if new_line is not None else old_line

            prop_stub = PropData(
                prop_id=movement.get("prop_id", f"movement:{movement.get('player')}")
                or "movement",
                player_name=movement.get("player", "Unknown"),
                team="Unknown",
                opponent="Unknown",
                sport=movement.get("sport", "UNKNOWN"),
                ...
                last_updated=movement_time_aware,
            )

            trigger = AlertTrigger(
                trigger_id=self._generate_trigger_id(
                    rule.rule_id, prop_stub.prop_id, "line_movement"
                ),
                rule_id=rule.rule_id,
                user_id=rule.user_id,
                prop_data=prop_stub,
                trigger_type=AlertRuleType.LINE_MOVEMENT,
                severity=self._calculate_movement_severity(abs_change),
                message=(
                    f"Line {direction_word}: {line_change:+.1f} on {prop_stub.player_name}"
                    f" {prop_stub.market} (from {old_line_str} to {new_line_str})"
                ),
                data={
                    "movement_amount": line_change,
                    ...
                    "volatility": movement.get("volatility"),
                    "snapshot_count": movement.get("snapshot_count"),
                    "movement_time": movement_time_aware.isoformat(),
                    "time_window_hours": time_window_hours,
                },
                triggered_at=movement_time_aware,
                expires_at=movement_time_aware + timedelta(hours=1),
            )
    ...
    async def _evaluate_edge_emergence(
        self, rule: AlertRule, prop_data: List[PropData]
    ) -> List[AlertTrigger]:
        ...
        for prop in prop_data:
            if (
                prop.edge_percentage
                and prop.edge_percentage >= min_edge
                and prop.confidence_score
                and prop.confidence_score >= min_confidence
            ):
                ...
                if not await self._is_recently_detected(cache_key):
                    trigger = AlertTrigger(
                        trigger_id=self._generate_trigger_id(
                            rule.rule_id, prop.prop_id, "edge_emergence"
                        ),
                        rule_id=rule.rule_id,
                        user_id=rule.user_id,
                        prop_data=prop,
    ...
    async def _evaluate_steam_detection(
        self, rule: AlertRule, prop_data: List[PropData]
    ) -> List[AlertTrigger]:
            ...
            if len(props) >= 2:  # Need multiple books for steam detection
                try:
                    if self.odds_service:
                        steam_data = await self.odds_service.detect_steam_movement(
                            prop_id
                        )
                    else:
                        steam_data = None
                    ...
                    if steam_data and steam_data.get("steam_detected"):
                        ...
                        trigger = AlertTrigger(
                            trigger_id=self._generate_trigger_id(
                                rule.rule_id, prop_id, "steam_detection"
                            ),
                            rule_id=rule.rule_id,
                            user_id=rule.user_id,
                            ...
                            data={
                                ...
                                "confidence_score": steam_data.get(
                                    "confidence_score", 0
                                ),
                                "synchronized_window": steam_data.get(
                                    "synchronized_window_minutes", 0
                                ),
            ...
            try:
                ...
                if success:
                    ...
                    if trigger.rule_id in self.active_rules:
                        self.active_rules[trigger.rule_id].last_triggered = (
        ...
        try:
            ...
            mock_rules = [
                AlertRule(
                    ...
                    is_active=True,
                    conditions={"min_ev_percentage": 8.0, "min_confidence": 85.0},
                    cooldown_minutes=30,
                    priority="high",
                ...
                AlertRule(
                    ...
                    is_active=True,
                    conditions={"movement_threshold": 1.5, "time_window_hours": 2},
                    cooldown_minutes=15,
                    priority="medium",
                    created_at=datetime.now(timezone.utc),
                ),
                AlertRule(
                    ...
                    is_active=True,
                    conditions={"min_edge_percentage": 12.0, "min_confidence": 80.0},
                    cooldown_minutes=60,
                    priority="high",
                    created_at=datetime.now(timezone.utc),
                ),
    ...
    def _generate_trigger_id(
        self, rule_id: str, prop_id: str, trigger_type: str
    ) -> str:
    ...
    def _trigger_to_alert_event(self, trigger: AlertTrigger) -> AlertEvent:
        ...
        return AlertEvent(
            alert_rule_id=(
                int(trigger.rule_id.split("_")[-1])
                if trigger.rule_id.split("_")[-1].isdigit()
                else 1
            ),
        ...
        try:
            if self.cache_service:
                await self.cache_service.set(
                    cache_key, datetime.now(timezone.utc).isoformat(), ttl=hours * 3600
                )
        except Exception as e:
    ...
    async def _cleanup_expired_data(self):
        ...
        expired_triggers = [
            trigger_id
            for trigger_id, trigger in self.triggered_alerts.items()
        ...
        expired_dedup = [
            key
            for key, timestamp in self.deduplication_cache.items()
        ...
        if expired_triggers or expired_dedup:
            logger.debug(
                f"Cleaned up {len(expired_triggers)} triggers and {len(expired_dedup)} dedup entries"
            )
