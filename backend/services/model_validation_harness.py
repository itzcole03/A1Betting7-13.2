...
except ImportError:  # pragma: no cover - optional cache dependency
        ...
        async with _cache_lock:
            if _cache_instance is None:
                try:
                    ...
                    logger.debug("Validation cache unavailable: %s", exc)
                    return None
...
class ModelValidationHarness:
    ...
    async def run_validation(
        self, model_id: str, test_case_ids: Optional[List[str]] = None
    ) -> ValidationRun:
        ...
        logger.info(
            f"🧪 Starting validation run {validation_run.run_id} for model {model_id} ({len(applicable_cases)} tests)"
        )
        ...
        for test_case in applicable_cases:
            try:
                ...
                validation_run.max_execution_time_ms = max(
                    validation_run.max_execution_time_ms, result.execution_time_ms
                )
        ...
        validation_run.avg_execution_time_ms /= (
            len(applicable_cases) if applicable_cases else 1
        )
        ...
        logger.info(
            f"✅ Completed validation run {validation_run.run_id}: {validation_run.status.value}"
        )
        logger.info(
            f"   Results: {validation_run.passed_tests} passed, {validation_run.failed_tests} failed, {validation_run.error_tests} errors"
        )
    ...
    async def _run_single_test(self, model, test_case: TestCase) -> ValidationResult:
        ...
        try:
            ...
            runner_func = self._validation_runners.get(
                model.model_type.value, self._default_validation_runner
            )

            # Execute the test
            prediction_output, confidence_score = await runner_func(
                model, test_case.input_data
            )
            ...
            if baseline_key in self.baseline_results:
                ...
                result.regression_check = (
                    result.regression_delta <= test_case.regression_tolerance
                )
            else:
            ...
            result.accuracy_check = self._validate_prediction_accuracy(
                test_case, prediction_output, confidence_score
            )
            ...
            logger.error(
                f"Failed validation test {test_case.test_id} for model {model.model_id}: {e}"
            )
            raise

    async def _default_validation_runner(
    ...
    def _calculate_regression_delta(
        self, current_output: Dict[str, Any], baseline_output: Dict[str, Any]
    ) -> float:
    ...
    def _validate_prediction_accuracy(
        self, test_case: TestCase, prediction: Dict[str, Any], confidence: float
    ) -> bool:
    ...
    async def _analyze_regression(self, validation_run: ValidationRun):
        ...
        regression_count = sum(
            1 for result in validation_run.test_results if not result.regression_check
        )
        total_with_baselines = sum(
            1
        ...
        try:
            ...
            if cache is not None:
                ...
                try:
                    ...
                    logger.debug("Validation cache set failed: %s", exc)
            ...
            logger.info(
                f"✅ Persisted validation results for run {validation_run.run_id}"
            )
    ...
    async def schedule_nightly_validation(self):
        ...
        models_to_validate = self.model_registry.list_models(
        ...
        for model in models_to_validate:
            try:
                ...
                logger.error(
                    f"Failed nightly validation for model {model.model_id}: {e}"
                )

        logger.info(
            f"✅ Completed nightly validation: {len(validation_results)} models processed"
        )
    ...
    async def _handle_validation_failure(self, model, validation_result: ValidationRun):
        ...
        if validation_result.regression_severity == "high":
            logger.warning(
                f"🚨 High-severity regression detected for model {model.model_id}"
            )
            ...
            if model.status == ModelStatus.STABLE:
                await self.model_registry.update_model_status(
                    model.model_id, ModelStatus.CANARY
                )
                logger.info(
                    f"⬇️ Downgraded model {model.model_id} from STABLE to CANARY"
                )
            elif model.status == ModelStatus.CANARY:
                await self.model_registry.update_model_status(
                    model.model_id, ModelStatus.DEVELOPMENT
                )
                logger.info(
                    f"⬇️ Downgraded model {model.model_id} from CANARY to DEVELOPMENT"
                )
    ...
    def get_validation_history(
        ...
        history = [
            run for run in self.validation_history if run.started_at >= cutoff_date
        ]
    ...
    def get_regression_report(self, model_id: str, days: int = 7) -> Dict[str, Any]:
        ...
        recent_regression_rate = sum(
            1 for run in recent_runs if run.regression_detected
        ) / len(recent_runs)
        ...
        return {
            ...
            "regression_rate": (
                runs_with_regression / total_runs if total_runs > 0 else 0
            ),
            "recent_regression_rate": recent_regression_rate,
            "trend": (
                "improving"
                if recent_regression_rate < 0.2
                else "concerning" if recent_regression_rate > 0.5 else "stable"
            ),
            ...
            "severity_breakdown": {
                ...
                "medium": sum(
                    1 for run in history if run.regression_severity == "medium"
                ),
