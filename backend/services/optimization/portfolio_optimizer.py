...
class PortfolioOptimizer:
    ...
    async def optimize_portfolio(
        ...
        if run_async:
            ...
            asyncio.create_task(
                self._execute_optimization_async(
                    run_id, objective, edge_ids, constraints
                )
            )
        else:
            # Execute synchronously
            await self._execute_optimization_async(
                run_id, objective, edge_ids, constraints
            )
    ...
    async def _execute_optimization_async(
        ...
        try:
            ...
            if len(candidates) < constraints.min_legs:
                raise ValueError(
                    f"Insufficient edge candidates: {len(candidates)} < {constraints.min_legs}"
                )

            # Filter candidates by minimum EV
            filtered_candidates = [
                c for c in candidates if c.ev >= constraints.min_ev_per_leg
            ]

            if len(filtered_candidates) < constraints.min_legs:
                raise ValueError(
                    f"Insufficient candidates after EV filter: {len(filtered_candidates)}"
                )
            ...
            correlation_matrix, diagnostics = (
                await self._compute_candidate_correlations(filtered_candidates)
            )
    ...
    async def _compute_candidate_correlations(
        self, candidates: List[EdgeCandidate]
        ...
        try:
            ...
            correlation_matrix, diagnostics = (
                self.correlation_engine.compute_pairwise_matrix(
                    prop_ids=prop_ids,
                    method="pearson",
                    shrinkage=True,
                    shrinkage_alpha=0.1,
    ...
    async def _optimize_target_prob(
        ...
        async def score_function_async(edge_set: Set[int]) -> float:
            ...
            sub_matrix = [
                [
                    correlation_matrix[indices[i]][indices[j]]
                    for j in range(len(indices))
                ]
                for i in range(len(indices))
            ]
        ...
        if len(all_indices) > 1:
            ...
            for i in range(len(all_indices)):
                for j in range(i + 1, len(all_indices)):
                    total_corr += abs(
                        correlation_matrix[all_indices[i]][all_indices[j]]
                    )
                    count += 1
            avg_corr = total_corr / count if count > 0 else 0.0
    ...
    def _store_artifact(
        self, run_id: str, artifact_type: ArtifactType, content: Dict[str, Any]
    ):
