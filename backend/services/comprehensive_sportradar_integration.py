...
class ComprehensiveSportRadarService:
    ...
    def __init__(self):
        ...
        self.api_key = self.config.sportradar_api_key or "SFvaMFGToGtrHBBEhROlT2keKedDtDVijvaDbiNz"
        ...
        self.rate_limiters: Dict[APIType, Dict[str, Any]] = {}
        self.quota_usage: Dict[APIType, int] = {}
    ...
    def _initialize_all_apis(self) -> Dict[APIType, SportRadarAPI]:
        ...
        apis[APIType.GETTY_IMAGES] = SportRadarAPI(
            ...
            trial_end="09/10/2025",
            packages=[
                "Getty Action Shots Argentina Superliga", "Getty Action Shots Badminton",
                "Getty Action Shots Beach Volleyball", "Getty Action Shots Belgian First Division",
                "Getty Action Shots Brasileiro Series A", "Getty Action Shots Cricket",
                "Getty Action Shots English Premier League", "Getty Action Shots Euro League Basketball",
                "Getty Action Shots Formula 1", "Getty Action Shots French Ligue 1",
                "Getty Action Shots German Bundesliga", "Getty Action Shots Handball",
                "Getty Premium Headshots NBA", "Getty Premium Headshots NFL", "Getty Premium Headshots NHL"
            ],
        ...
        apis[APIType.MLB] = SportRadarAPI(
            ...
            trial_end="09/10/2025",
            packages=["MLB Base"],
            data_categories=[DataCategory.SPORTS_DATA, DataCategory.LIVE_SCORES, DataCategory.SCHEDULES],
            cache_ttl=300
        )
        ...
        for api_type in priority_apis:
            try:
                if api_type == APIType.MLB:
                    result = await self.get_mlb_data("league_hierarchy")
                elif api_type == APIType.NFL:
                    result = await self.get_nfl_data("league_hierarchy")
                elif api_type == APIType.NBA:
                    result = await self.get_nba_data("league_hierarchy")
                elif api_type == APIType.NHL:
                    result = await self.get_nhl_data("league_hierarchy")
    ...
    async def _make_api_request(self, api_type: APIType, endpoint: str, **kwargs) -> Dict[str, Any]:
        ...
        if not endpoint_template:
            raise Exception(f"Endpoint '{endpoint}' not found for {api_type.value}")
        
        # Format endpoint with parameters
        try:
            formatted_endpoint = endpoint_template.format(**kwargs)
        except KeyError as e:
            raise Exception(f"Missing parameter {e} for endpoint {endpoint}")
        ...
        async with self.session.get(full_url) as response:
            ...
            if response.status == 429:
                ...
                logger.warning(f"⚠️ {api_type.value} rate limit hit, retry after {retry_after}s")
                raise Exception(f"Rate limit exceeded for {api_type.value}")
            
            # Handle authentication errors
            elif response.status == 401:
                logger.error(f"❌ {api_type.value} authentication failed")
                raise Exception(f"Invalid API key for {api_type.value}")
            ...
            # Handle successful response
                ...
                if quota_remaining:
                    logger.debug(f"📊 {api_type.value} quota remaining: {quota_remaining}")
            ...
            # Handle other errors
                ...
                logger.error(f"❌ {api_type.value} API error {response.status}: {error_text}")
                raise Exception(f"{api_type.value} API error: {response.status}")
    ...
    async def _check_rate_limit(self, api_type: APIType):
        ...
        if rate_limiter["requests"] >= rate_limiter["limit"]:
            ...
            if sleep_time > 0:
                logger.warning(f"⚠️ Rate limit reached for {api_type.value}, sleeping {sleep_time:.1f}s")
    ...
    async def get_soccer_data(self, endpoint: str, **kwargs) -> Optional[Dict[str, Any]]:
        """Get Soccer data"""
    ...
    async def get_player_props_odds(self, sport: str, competition: str, **kwargs) -> Optional[Dict[str, Any]]:
    ...
    async def get_prematch_odds(self, sport: str, competition: str, **kwargs) -> Optional[Dict[str, Any]]:
        """Get prematch odds"""
    ...
    async def get_futures_odds(self, sport: str, competition: str, **kwargs) -> Optional[Dict[str, Any]]:
        """Get futures odds"""
    ...
    # Image API Methods
    async def get_getty_images(self, sport: str, competition: str, image_type: str = "action_shots", **kwargs) -> Optional[Dict[str, Any]]:
        """Get Getty Images"""
    ...
    async def get_sportradar_images(self, image_type: str = "country_flags", **kwargs) -> Optional[Dict[str, Any]]:
        """Get SportRadar images"""
        async def fetch_sr_images():
            return await self._make_api_request(APIType.SPORTRADAR_IMAGES, image_type, **kwargs)
    ...
    async def get_comprehensive_sports_data(self, sports: List[str] = None) -> Dict[str, Any]:
        ...
        for sport in sports:
            ...
            if sport == "mlb":
                requests.extend([
                    ...
                    (f"mlb_hierarchy", self.get_mlb_data, ("league_hierarchy",), {})
                ])
            elif sport == "nfl":
                requests.extend([
                    (f"nfl_live", self.get_nfl_data, ("live_scores",), {}),
                    (f"nfl_hierarchy", self.get_nfl_data, ("league_hierarchy",), {})
                ])
            elif sport == "nba":
                requests.extend([
                    (f"nba_live", self.get_nba_data, ("live_scores",), {}),
                    (f"nba_hierarchy", self.get_nba_data, ("league_hierarchy",), {})
                ])
            elif sport == "nhl":
                requests.extend([
                    (f"nhl_live", self.get_nhl_data, ("live_scores",), {}),
                    (f"nhl_hierarchy", self.get_nhl_data, ("league_hierarchy",), {})
                ])
            elif sport == "soccer":
                requests.extend([
                    (f"soccer_live", self.get_soccer_data, ("live_scores",), {}),
                    (f"soccer_tournaments", self.get_soccer_data, ("tournaments",), {})
                ])
            
            # Odds data requests
            competition = "premier_league" if sport == "soccer" else "regular_season"
            requests.extend([
                (f"props_{sport}", self.get_player_props_odds, (sport, competition), {}),
                (f"prematch_{sport}", self.get_prematch_odds, (sport, competition), {}),
                (f"futures_{sport}", self.get_futures_odds, (sport, competition), {})
            ])
        
        # Execute parallel requests
        results = await enhanced_data_pipeline.fetch_parallel_data(
            requests,
            max_failures=len(requests) // 3  # Allow up to 1/3 failures
        )
        ...
        comprehensive_data["metadata"]["apis_used"] = list(set([
            api_type.value for api_type in self.apis.keys() 
    ...
    def _get_trial_status(self) -> Dict[str, Any]:
        ...
        for api_type, api_config in self.apis.items():
            trial_status[api_type.value] = {
                ...
                "quota_remaining": api_config.quota_limit - self.quota_usage.get(api_type, 0),
    ...
    async def _monitor_usage(self):
        ...
        while True:
            try:
                ...
                logger.info(f"📊 SportRadar Usage Summary: {total_requests} total requests")
                ...
                for api_type, usage in self.quota_usage.items():
                    if usage > 0:
                        ...
                        logger.info(f"  {api_type.value}: {usage}/{api_config.quota_limit} ({percentage:.1f}%)")
    ...
    async def get_health_status(self) -> Dict[str, Any]:
        ...
        exhausted_apis = [
            api_type.value for api_type, usage in self.quota_usage.items()
