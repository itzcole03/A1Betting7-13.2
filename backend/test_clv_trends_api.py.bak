...
# Mock the FastAPI dependencies only if FastAPI is unavailable. This prevents
# shadowing the real package during other pytest modules.
if "fastapi" not in sys.modules:
    sys.modules["fastapi"] = type(
        "MockModule",
        (),
        {
            "APIRouter": lambda **kwargs: type(
                "MockRouter", (), {"get": lambda *args, **kwargs: lambda f: f}
            )(),
            "FastAPI": type(
                "MockFastAPI",
                (),
                {
                    ...
                    "add_exception_handler": lambda self, *args, **kwargs: None,
                    "add_event_handler": lambda self, *args, **kwargs: None,
                    "include_router": lambda self, *args, **kwargs: None,
                },
            ),
            ...
            "WebSocketDisconnect": type("MockWebSocketDisconnect", (), {}),
            "Depends": lambda f: f,
        },
    )()
...
if "pydantic" not in sys.modules:
    sys.modules["pydantic"] = type(
        "MockModule", (), {"BaseModel": object, "Field": lambda **kwargs: None}
    )()
...
# Mock the response builder only when the canonical module has not been loaded.
if "backend.core.response_models" not in sys.modules:
    sys.modules["backend.core.response_models"] = type(
        "MockModule", (), {"StandardAPIResponse": MockStandardAPIResponse}
    )()

if "backend.core.exceptions" not in sys.modules:
    sys.modules["backend.core.exceptions"] = type(
        "MockModule",
        (),
        {
...
def test_distribution_endpoint():
    ...
    try:
        ...
        print(
            f"   ✅ Positive CLV opportunities: {data['opportunities_with_positive_clv']}"
        )
...
def test_alerts_endpoint():
    ...
    try:
        ...
        for alert in alerts:
            print(
                f"   ✅ Alert: {alert['alert_type']} - {alert['player']} {alert['market']}"
            )
            print(
                f"      CLV change: {alert['clv_change']}% (severity: {alert['severity']})"
            )
...
def test_closing_snapshots_endpoint():
    ...
    try:
        ...
        for snapshot in snapshots:
            ...
            print(
                f"      Opening line: {snapshot['opening_line']} → Closing line: {snapshot['closing_line']}"
            )
...
def test_response_format_consistency():
    ...
    expected_format = {"success": bool, "data": "Any", "error": "Optional"}
