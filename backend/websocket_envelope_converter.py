...
class WebSocketEnvelopeConverter:
    ...
    def __init__(self):
        # update envelope timestamp to timezone-aware
        # envelope template (timestamp updated to timezone-aware)
...
'''
        ...
        def replace_send_text(match):
            ...
            if type_match:
                ...
                data_match = re.search(
                    r'"(?!type"|status"|error"|timestamp")([^"]+)":\s*([^,}]+)',
                    dict_content,
                )
                ...
                if data_match:
                    ...
                    remaining_content = re.sub(
                        r'"type":\s*"[^"]*",?\s*', "", remaining_content
                    )
                    remaining_content = re.sub(
                        r'"status":\s*[^,}]+,?\s*', "", remaining_content
                    )
                    remaining_content = re.sub(
                        r'"error":\s*[^,}]+,?\s*', "", remaining_content
                    )
                    remaining_content = re.sub(
                        r'"timestamp":\s*[^,}]+,?\s*', "", remaining_content
                    )
    ...
    def _ensure_proper_cleanup(self, content: str) -> str:
        ...
        websocket_functions = re.findall(
            r"async def (websocket_[^(]*)\([^)]*\):", content
        )
...
def main():
    ...
    print("\n" + "=" * 60)
    print(
        f"Conversion complete! {converted_count}/{len(files_to_convert)} files converted."
    )
