...
def fail(error_code="ERROR", message="An error occurred", data=None):
    ...
    warnings.warn(
        "fail() is deprecated, use ApiError with error taxonomy instead",
        DeprecationWarning,
    )
    return {
        "success": False,
...
async def general_exception_handler(request: Request, exc: Exception) -> JSONResponse:
    ...
    show_details = getattr(settings, "DEBUG", False) or getattr(
        settings, "SHOW_ERROR_DETAILS", False
    )
...
async def http_exception_handler(
    ...
    # Build the canonical error envelope using the taxonomy helper
    error_response = build_error(
        code=error_code,
        message=(
            str(detail)
            if not isinstance(detail, (list, dict))
            else "Validation or HTTP error"
        ),
    ...
    # Preserve original detail when it's already a list/dict (tests expect this shape).
    response_body = dict(error_response)
    try:
        if isinstance(detail, (list, dict)):
            response_body["detail"] = detail
        else:
            # For backwards compatibility, include a stringified detail as well
            response_body.setdefault("detail", str(detail))
    except Exception:
        # Ensure we always return a serializable body; fall back to original envelope
...
def register_exception_handlers(app) -> None:
    ...
    try:
        from backend.middleware.payload_guard import (
            ...
            payload_rejection_error_handler,
        )

        app.add_exception_handler(
            PayloadRejectionError, payload_rejection_error_handler
        )
        app_logger.info("✅ PayloadRejectionError handler registered")
    except ImportError:
        app_logger.warning(
            "⚠️ PayloadRejectionError handler not available - payload guard not installed"
        )
    ...
    app_logger.info(
        "✅ All exception handlers registered with structured error taxonomy"
    )
