                ...
                "widgets": [
                    {
                        ...
                        "config": {"data_source": "performance_history", "timeframe": "30d"},
                        "visible": True,
                    ...
                    },
                    {
                        ...
                        "config": {"data_source": "sport_performance", "group_by": "sport"},
                        "visible": True,
                        "refresh_interval": 300
                    }
                ],
                ...
                "widgets": [
                    {
                        ...
                        "config": {"sports": ["NBA", "NFL"], "markets": ["moneyline", "spread"]},
                        "visible": True,
                        "refresh_interval": 5
                    },
                    {
...
async def save_dashboard_layout(layout: DashboardLayout) -> Dict[str, str]:
    ...
    try:
        ...
        await cache.set(f"dashboard_layout:{layout.id}", layout.dict(), ttl=86400 * 30)  # 30 days
        ...
        return {"layout_id": layout.id, "message": "Dashboard layout saved successfully"}
...
async def get_widget_data(request: WidgetDataRequest) -> Dict[str, Any]:
    ...
    try:
        ...
        widget_data = await generate_widget_data(request.widget_type, request.config, request.time_range)
...
async def generate_widget_data(widget_type: str, config: Dict[str, Any], time_range: str) -> Dict[str, Any]:
    ...
    if widget_type == "stats_card":
        ...
        props = ["Points", "Assists", "Rebounds", "Strikeouts", "Passing Yards", "Goals"]
        ...
        return {
            ...
            "summary": {
                ...
                "pending_count": len([b for b in mock_bets if b.get("status") == "pending"])
            }
        }
        ...
        opportunities = []
        players = ["LeBron James", "Stephen Curry", "Aaron Judge", "Josh Allen", "Connor McDavid"]
        props = ["Points Over", "Assists Over", "Rebounds Over", "Strikeouts Over", "Passing Yards Over"]
        ...
        data = {
            ...
            "change_percentage": ((current_balance - starting_balance) / starting_balance) * 100,
            "history": history
        }
...
async def get_dashboard_preferences(user_id: str = Query(...)) -> DashboardPreferences:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to get dashboard preferences")

@router.post("/preferences")
async def save_dashboard_preferences(preferences: DashboardPreferences) -> Dict[str, str]:
    """Save user dashboard preferences"""
    try:
        cache = await get_cache()
        
        # Save to cache (in production, save to database)
        await cache.set(f"dashboard_prefs:{preferences.user_id}", preferences.dict(), ttl=86400 * 30)
        
        return {"message": "Dashboard preferences saved successfully"}
        
    except Exception as e:
        logger.error(f"Error saving dashboard preferences: {e}")
        raise HTTPException(status_code=500, detail="Failed to save dashboard preferences")
