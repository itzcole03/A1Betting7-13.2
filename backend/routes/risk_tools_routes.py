...
class KellyInputsModel(BaseModel):
    ...
    win_probability: float = Field(..., ge=0.01, le=0.99, description="Win probability (0.01 to 0.99)")
    american_odds: int = Field(..., description="American odds (e.g., -110, +150)")
    bankroll: float = Field(..., gt=0, description="Total bankroll amount")
    kelly_fraction: float = Field(0.25, ge=0.01, le=1.0, description="Fraction of Kelly to use")
    max_bet_percentage: float = Field(5.0, ge=0.1, le=50.0, description="Maximum bet as % of bankroll")
...
class SimulationRequest(BaseModel):
    ...
    num_simulations: int = Field(1000, ge=100, le=10000, description="Number of simulations")
    num_bets: int = Field(100, ge=10, le=1000, description="Number of bets per simulation")

class SessionRequest(BaseModel):
    """Request to save a betting session"""
    kelly_inputs: KellyInputsModel
    actual_bet_size: Optional[float] = None
    outcome: Optional[str] = Field(None, regex="^(win|loss|push)$")
...
class OptimalFractionRequest(BaseModel):
    ...
    american_odds: int
    target_max_drawdown: float = Field(0.2, ge=0.05, le=0.5, description="Target maximum drawdown")
...
async def calculate_fractional_kelly(
    ...
    try:
        ...
        results = risk_service.calculate_fractional_kelly(base_inputs, request.fractions)
        ...
        raise HTTPException(status_code=500, detail="Failed to calculate fractional Kelly")
...
async def calculate_optimal_fraction(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to calculate optimal fraction")
...
async def convert_odds(
    ...
    fractional: Optional[str] = Query(None, description="Fractional odds (e.g., '3/2')")
):
    """
    ...
    try:
        ...
        if american is not None:
            ...
            decimal_odds = (american / 100 + 1) if american > 0 else (100 / abs(american) + 1)
            ...
            american_odds = int((decimal - 1) * 100) if decimal >= 2 else int(-100 / (decimal - 1))
            ...
            try:
                ...
                raise ValueError("Invalid fractional odds format. Use 'numerator/denominator'")
