...
class LegacyEndpointInfo(BaseModel):
    ...
    last_access_ts: Optional[float] = Field(
        None, description="Last access timestamp (Unix)"
    )
    last_access_iso: Optional[str] = Field(
        None, description="Last access in ISO format"
    )
...
class LegacyUsageResponse(BaseModel):
    ...
    endpoints: list[LegacyEndpointInfo] = Field(
...
class MigrationAnalysis(BaseModel):
    ...
    high_usage_endpoints: list[Dict[str, Any]] = Field(
        ..., description="Endpoints with high usage"
    )
...
@router.get(
    "/legacy-usage", response_model=LegacyUsageResponse, tags=["Legacy Telemetry"]
)
async def get_legacy_usage():
    ...
    try:
        ...
        logger.info(
            f"Legacy usage data requested: {len(endpoints)} endpoints, {usage_data['total']} total calls"
        )
        return response
        ...
        raise BusinessLogicException(
            f"Failed to retrieve legacy usage data: {str(e, status_code=500)}"
        )


@router.get(
    "/migration-readiness",
    response_model=MigrationReadinessResponse,
    tags=["Legacy Telemetry"],
)
async def get_migration_readiness(
    threshold: int = Query(
        50, ge=1, le=1000, description="High usage threshold (calls per hour)"
    )
):
    """
    ...
    try:
        ...
        logger.info(
            f"Migration readiness assessed: score={response.score}, level={response.readiness_level}"
        )
        return response
        ...
        raise BusinessLogicException(
            f"Failed to assess migration readiness: {str(e, status_code=500)}"
        )
...
async def get_legacy_config():
    ...
    try:
        ...
        raise BusinessLogicException(
            f"Failed to get legacy configuration: {str(e, status_code=500)}"
        )
...
async def clear_legacy_usage():
    ...
    try:
        ...
        raise BusinessLogicException(
            f"Failed to clear legacy usage data: {str(e, status_code=500)}"
        )
