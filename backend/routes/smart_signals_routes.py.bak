...
from ..services.smart_signals import SmartSignal, smart_signals_service
...
async def get_smart_signals(
    ...
    try:
        ...
        if not enabled:
            ...
            message = "Smart signals feature is disabled. Enable with ENABLE_SMART_SIGNALS=true"
            payload = {
                "success": False,
                "data": None,
                "error": {
        ...
        if sport != "ALL":
            opportunities = [
                ...
                if opp.get("sport", "").upper() == sport.upper()
            ]
        ...
        for opp in opportunities:
            try:
                ...
                if signal and signal.score >= min_score:
                    ...
                    smart_signals_generated_total.labels(
                        sport=sport, outcome="success"
                    ).inc()

            except Exception as e:
                logger.warning(
                    f"Failed to compute signal for opportunity {opp.get('id', 'unknown')}: {e}"
                )
        ...
        logger.info(
            f"Generated {len(enhanced_opportunities)} smart signals from {total_count} opportunities"
        )
        ...
        smart_signals_generated_total.labels(sport=sport, outcome="error").inc()
        # Raise a BusinessLogicException with an explicit status_code
