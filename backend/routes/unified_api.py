...
def _normalize_payload(payload: Any) -> Any:
    ...
    try:
        ...
        if isinstance(payload, dict) and payload.get("success") is not None and "data" in payload:
        ...
        logger.exception("Failed to normalize payload for response; returning fallback empty list")
        return []
...
async def unified_analysis_stub() -> Dict[str, Any]:
    ...
    try:
        ...
        logger.exception("[unified_analysis_stub] Failed to generate analysis; returning fallback sample.")
        fallback = {
            ...
            "enriched_props": [
                {
                    "player_info": {"name": "Sample Player", "team": "Sample Team", "position": "G", "image_url": None, "score": 0},
...
@router.get("/props/featured", response_model=StandardAPIResponse, tags=["Featured Props"])
@router.get("/unified/props/featured", response_model=StandardAPIResponse, tags=["Featured Props"])
async def get_featured_props(
    ...
    try:
        ...
        logger.exception("[FeaturedProps] Error fetching real props; returning fallback sample.")
        fallback = [
            {
...
async def get_mlb_bet_analysis(
    ...
    try:
        ...
        logger.exception("Error generating MLB BetAnalysisResponse; using fallback enriched_props")
        player_info = PlayerInfo(
    ...
    try:
        result_payload = response.model_dump() if hasattr(response, "model_dump") else (
            response if isinstance(response, dict) else dict(response.__dict__)
        )
    except Exception:
        # Best-effort fallback: attempt to serialize fields manually
        try:
            result_payload = json.loads(json.dumps(response, default=lambda o: getattr(o, "__dict__", str(o))))
        except Exception:
            result_payload = {"analysis": str(response)}

    logger.debug(f"[MLB_BET_ANALYSIS] Raw response payload type: {type(result_payload)}")
...
async def get_ai_insights(
    ...
    try:
        ...
        logger.exception("[get_ai_insights] Failed to generate AI insights; returning fallback empty insights.")
        fallback = {
            "ai_insights": [],
            "summary": {
...
async def get_live_game_context(
    ...
    try:
        ...
        logger.exception("[get_live_game_context] Failed to return live context; returning fallback context.")
        fallback = {
...
async def get_multi_platform_opportunities(
    ...
    return _ok_as_response(response)
        ...
        logger.exception("[get_multi_platform_opportunities] Error computing multi-platform opportunities; returning fallback.")
        fallback = {
            "multi_platform_opportunities": [],
            "arbitrage_opportunities": [],
            "platform_summary": {"total_platforms": 0, "opportunities_found": 0, "arbitrage_count": 0, "recommended_primary": None},
            "recommendations": [],
        }
        return _ok_as_response(fallback)


@router.get("/health", response_model=StandardAPIResponse[Dict[str, Any]], tags=["Unified Intelligence"])
@router.get("/unified/health", response_model=StandardAPIResponse[Dict[str, Any]], tags=["Unified Intelligence"])
...
@router.get("/api/health/comprehensive", response_model=dict, tags=["Legacy Compatibility"])
@router.get("/api/health/database", response_model=dict, tags=["Legacy Compatibility"])
@router.get("/api/health/data-sources", response_model=dict, tags=["Legacy Compatibility"])
...
@router.get("/api/prizepicks/health", response_model=dict, tags=["Legacy Compatibility"])
...
@router.get("/api/predictions/prizepicks", response_model=dict, tags=["Legacy Compatibility"])
...
@router.get("/api/v2/modern/health/async", response_model=dict, tags=["Legacy Compatibility"])
...
@router.get("/predictions/model-performance", response_model=dict, tags=["Legacy Compatibility"])
...
@router.get("/api/production/health/comprehensive", response_model=dict, tags=["Legacy Compatibility"])
@router.get("/api/production/health/background-tasks", response_model=dict, tags=["Legacy Compatibility"])
...
async def api_props(
    ...
    try:
        ...
        if isinstance(featured_list, dict):
            ...
            for candidate_key in ("opportunities", "props", "enriched_props", "items", "results", "predictions", "data"):
                if candidate_key in featured_list and isinstance(featured_list[candidate_key], list):
            ...
            if not isinstance(featured_list, list):
                for v in featured_list.values():
                    if isinstance(v, list) and v and isinstance(v[0], dict):
                        ...
                        keys = set(v[0].keys())
                        if keys & {"player", "player_name", "player_info", "playerInfo"}:
                            featured_list = v
                            break
    ...
    try:
        ...
        featured_list = [
            {"player": "Sample Player", "stat_type": "points", "confidence": 50, "source": "fallback"}
        ]
...
@router.get("/predictions", response_model=StandardAPIResponse[List[Dict[str, Any]]], tags=["Unified Intelligence"])
@router.get("/unified/predictions", response_model=StandardAPIResponse[List[Dict[str, Any]]], tags=["Unified Intelligence"])
async def api_predictions_get() -> Dict[str, Any]:
    ...
    try:
        sample = [
            {"player": "Sample Player", "confidence": 50, "source": "sample"}
        ]
        return ResponseBuilder.success(sample)
    except Exception as e:
        logger.debug("[api_predictions_get] unexpected error, returning fallback: %s", e)
...
@router.get("/analytics", response_model=StandardAPIResponse[Dict[str, Any]], tags=["Unified Intelligence"])
    ...
    try:
        ...
        if isinstance(result, dict) and "success" in result:
            ...
            if isinstance(data, dict) and "summary" not in data:
                try:
                    total = len(data.get("enriched_props", [])) if isinstance(data.get("enriched_props", None), list) else 0
                except Exception:
                    total = 0
        ...
        logger.debug("[api_analytics] get_mlb_bet_analysis failed, returning fallback: %s", e)
        fallback = {
