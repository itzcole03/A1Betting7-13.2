...
from backend.utils.trace_utils import add_span_log, add_span_tag, trace_span
...
class ShadowModeRequest(BaseModel):
    ...
    reason: Optional[str] = Field(
        None, description="Reason for shadow mode change (for audit trail)"
    )
...
class ShadowModeResponse(BaseModel):
    ...
    shadow_version: Optional[str] = Field(
        None, description="Shadow model version (if enabled)"
    )
    runtime_override: bool = Field(
        ..., description="Whether runtime override is active"
    )
    environment_shadow_version: Optional[str] = Field(
        None, description="Shadow version from environment"
    )
    last_change_reason: Optional[str] = Field(
        None, description="Reason for last shadow mode change"
    )
...
class AdminStatusResponse(BaseModel):
    ...
    admin_controls_active: bool = Field(
        ..., description="Whether admin controls are enabled"
    )
...
class RuntimeShadowController:
    ...
    def is_shadow_enabled(self) -> bool:
        ...
        return effective_shadow is not None and effective_shadow != active_version
    ...
    def enable_shadow_mode(
        ...
        with trace_span(
            ...
            # Record an informational span log (message, level)
            add_span_log(
                span_id, f"Shadow mode enabled with version {shadow_version}", "info"
            )
    ...
    def disable_shadow_mode(
        ...
        with trace_span(
            ...
            self._runtime_override_active = (
                True  # Keep override active but set shadow to None
            )
    ...
    try:
        ...
        if not shadow_request.shadow_version:
            raise BusinessLogicException(
                "Shadow version must be specified to enable shadow mode",
                status_code=400,
            )
        ...
        raise BusinessLogicException(
            message=f"Failed to enable shadow mode: {str(e)}", status_code=500
        )
...
async def disable_shadow_mode(request: Request, shadow_request: ShadowModeRequest):
    ...
    try:
        ...
        result = controller.disable_shadow_mode(
            reason=shadow_request.reason, request_info=request_info
        )
        ...
        raise BusinessLogicException(
            message=f"Failed to disable shadow mode: {str(e)}", status_code=500
        )
...
async def clear_shadow_override(request: Request, shadow_request: ShadowModeRequest):
    ...
    try:
        ...
        raise BusinessLogicException(
            message=f"Failed to clear shadow override: {str(e)}", status_code=500
        )
...
async def get_admin_status(request: Request):
    ...
    try:
        ...
        raise BusinessLogicException(
            message=f"Failed to get admin status: {str(e)}", status_code=500
        )
