...
from ..services.edges.edge_service import EdgeData, edge_service
from ..services.valuation.valuation_engine import ValuationResult, valuation_engine
...
async def get_valuation(
    ...
    force_recompute: bool = Query(
        False, description="Force recomputation of valuation"
    ),
) -> ValuationResponse:
    """
    ...
    try:
        logger.info(
            f"Getting valuation for prop {prop_id}, force_recompute={force_recompute}"
        )

        # Run valuation
        valuation = await valuation_engine.valuate(
            prop_id=prop_id, force_recompute=force_recompute
        )

        if not valuation:
            raise BusinessLogicException(
                f"Could not compute valuation for prop {prop_id}", status_code=404
            )
        ...
        logger.info(
            f"Valuation computed for prop {prop_id}: EV={valuation.expected_value:.4f}"
        )
        return response
        ...
        logger.error(f"Error getting valuation for prop {prop_id}: {e}", exc_info=True)
        # Raise a standardized BusinessLogicException with 500 status
        raise BusinessLogicException(
            message=f"Internal error computing valuation: {str(e)}",
            status_code=500,
            details={"prop_id": prop_id},
        )
...
async def recompute_edges(
    ...
    max_props: Optional[int] = Query(
        None, description="Maximum number of props to process"
    ),
    ...
    try:
        ...
        logger.error(f"Error recomputing edges for {sport}: {e}", exc_info=True)
...
async def get_edges(
    ...
    try:
        logger.info(
            f"Getting edges with filters: sport={sport}, prop_type={prop_type}, min_ev={min_ev}"
        )

        # Get edges from service
        edges = await edge_service.get_active_edges(
            sport=sport, prop_type=prop_type, min_ev=min_ev, limit=limit
        )
...
async def test_valuation() -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(
            f"Valuation test completed: EV={valuation.expected_value:.4f}, Edge={edge is not None}"
        )
        return result

    except Exception as e:
        logger.error(f"Error in valuation test: {e}")
        # Return a standardized error via exception so the global handler will format it
        raise BusinessLogicException(
            "Handler status error", status_code=500, details={"error": str(e)}
        )
