...
router = APIRouter(prefix="/api/v1/shap", tags=["SHAP Explanations"])
    ...
    try:
        ...
        try:
            ...
            mock_shap_values = {
                ...
                "home_away_factor": (
                    0.05 if request.features.get("is_home", True) else -0.03
                ),
                "rest_days": 0.02,
                "season_average": 0.12,
                "injury_status": 0.03,
            ...
            feature_importance = {
                k: abs(v) / total_abs_impact for k, v in mock_shap_values.items()
            }
    ...
    try:
        ...
        for request in requests:
            try:
                ...
                logger.error(
                    f"Error explaining prediction for {request.player_name}: {e}"
                )
