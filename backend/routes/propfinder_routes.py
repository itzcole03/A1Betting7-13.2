...
# Use unified data service backed by free league APIs
...
def _resolve_propfinder_service() -> Any:
    """Resolve the active PropFinder service factory at runtime.

    This indirection lets test suites patch ``get_simple_propfinder_service`` without
    requiring the route definitions to be rebuilt. The resolver looks up the current
    factory from the module namespace each time it executes, falling back to the
