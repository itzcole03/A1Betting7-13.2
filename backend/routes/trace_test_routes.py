...
from backend.utils.trace_utils import add_span_tag, get_trace_summary, trace_span
...
async def test_request_correlation(request: Request):
    ...
    with trace_span(
        "correlation_test", service_name="test", operation_name="correlation"
    ) as span_id:
...
async def test_nested_tracing(request: Request):
    ...
    with trace_span(
        "outer_operation", service_name="test", operation_name="nested_test"
    ) as outer_span:
        ...
        with trace_span(
            "middle_operation", service_name="test", operation_name="middle_work"
        ) as middle_span:
            ...
            with trace_span(
                "inner_operation", service_name="test", operation_name="inner_work"
            ) as inner_span:
...
async def test_performance_tracking(request: Request):
    ...
    # Fast operation
    with trace_span(
        "fast_operation", service_name="test", operation_name="fast_work"
    ) as fast_span:
    ...
    # Medium operation
    with trace_span(
        "medium_operation", service_name="test", operation_name="medium_work"
    ) as medium_span:
    ...
    # Slow operation
    with trace_span(
        "slow_operation", service_name="test", operation_name="slow_work"
    ) as slow_span:
...
async def test_error_tracing(request: Request):
    ...
    try:
        with trace_span(
            "error_operation", service_name="test", operation_name="error_test"
        ) as error_span:
