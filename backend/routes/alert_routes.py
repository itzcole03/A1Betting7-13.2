...
async def create_alert_rule(
    request: CreateAlertRequest, user_id: str = Depends(get_current_user_id)
):
    """
...
async def get_alert_rules(user_id: str = Depends(get_current_user_id)):
    """
    ...
    try:
        ...
        return AlertsListResponse(rules=rule_responses, total_count=len(rule_responses))
...
async def delete_alert_rule(rule_id: str, user_id: str = Depends(get_current_user_id)):
    """
...
async def get_fired_alerts(
    limit: int = Query(
        50, ge=1, le=100, description="Number of recent fired alerts to return"
    )
):
    """
    ...
    try:
        ...
        return FiredAlertsResponse(
            fired_alerts=fired_alerts, total_count=len(fired_alerts)
        )
...
async def start_alert_evaluation():
    ...
    try:
        ...
        raise BusinessLogicException(
            "Failed to start alert evaluation", status_code=500
        )
...
async def trigger_manual_evaluation():
    ...
    try:
        ...
        raise BusinessLogicException(
            "Failed to trigger alert evaluation", status_code=500
        )
