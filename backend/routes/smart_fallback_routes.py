...
class SetPrimaryProviderRequest(BaseModel):
    ...
    context: str = Field(..., description="Context identifier (e.g., 'odds_aggregation')")
...
class SelectProviderRequest(BaseModel):
    ...
    available_providers: List[str] = Field(..., description="List of available provider IDs")
    current_provider: Optional[str] = Field(None, description="Currently selected provider ID")
...
async def get_provider_priorities(
    context: str,
    available_providers: str = Query(..., description="Comma-separated list of provider IDs"),
    force_refresh: bool = Query(False, description="Force refresh of cached priorities"),
...
async def select_optimal_provider(
    ...
    try:
        ...
        priorities = await service.get_provider_priorities(request.context, request.available_providers)
...
async def clear_priority_cache(
    context: Optional[str] = Query(None, description="Specific context to clear, or all if not specified"),
    service: SmartFallbackPriorityService = Depends(get_fallback_service)
):
    ...
    try:
        if context:
            ...
            keys_to_remove = [key for key in service.priority_cache.keys() if key.startswith(f"{context}:")]
...
async def get_system_status(
    ...
    try:
        ...
        recent_events = [
            e for e in service.fallback_history 
        ...
        status = {
            ...
            "recent_activity": {
                ...
                "unique_providers_used": len(set(e.fallback_provider for e in recent_events))
            },
            "configuration": {
