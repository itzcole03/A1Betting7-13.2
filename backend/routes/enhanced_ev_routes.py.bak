...
class EnhancedEVRequest(BaseModel):
    ...
    fair_odds: float = Field(
        ..., gt=1.0, description="Our fair odds assessment (decimal format)"
    )
    market_odds: float = Field(
        ..., gt=1.0, description="Market offered odds (decimal format)"
    )
...
class BatchEVRequest(BaseModel):
    ...
    opportunities: List[Dict[str, Any]] = Field(
        ..., description="List of betting opportunities"
    )
...
async def calculate_enhanced_ev(request: EnhancedEVRequest):
    ...
    try:
        ...
        raise BusinessLogicException(
            message=f"EV calculation failed: {e}", status_code=500
        )
...
async def batch_calculate_ev(request: BatchEVRequest):
    ...
    try:
        ...
        original_flag = enhanced_ev_engine.is_feature_enabled(
            FeatureFlag.ENABLE_BATCH_OPTIMIZATION
        )
        enhanced_ev_engine.set_feature_flag(
            FeatureFlag.ENABLE_BATCH_OPTIMIZATION, request.use_optimization
        )
        ...
        try:
            ...
            return EVResponse(
                ...
                data={
                    ...
                    "batch_summary": {
                        ...
                        "positive_ev": len(
                            [r for r in results if r.get("ev_percent", 0) > 0]
                        ),
                    },
                },
            ...
            enhanced_ev_engine.set_feature_flag(
                FeatureFlag.ENABLE_BATCH_OPTIMIZATION, original_flag
            )

    except Exception as e:
        logger.error(f"Batch EV calculation error: {e}")
        raise BusinessLogicException(
            message=f"Batch EV calculation failed: {e}", status_code=500
        )
...
async def get_ev_metrics():
    ...
    try:
        ...
        return EVResponse(
            success=True, data=metrics, timestamp=datetime.now().isoformat()
        )

    except Exception as e:
        logger.error(f"Metrics retrieval error: {e}")
        raise BusinessLogicException(
            message=f"Failed to retrieve metrics: {e}", status_code=500
        )
...
async def get_rolling_metrics(
    window_minutes: int = Query(
        15, ge=1, le=60, description="Time window in minutes (1-60)"
    )
):
    """
    ...
    try:
        ...
        return EVResponse(
            ...
            data={
                ...
                "performance_status": {
                    "status": (
                        "healthy"
                        if rolling_metrics["error_rate"] < 0.05
                        else "degraded"
                    ),
                    "load_level": (
                        "high"
                        if rolling_metrics["calculations_per_minute"] > 10
                        else (
                            "moderate"
                            if rolling_metrics["calculations_per_minute"] > 2
                            else "low"
                        )
                    ),
                    "cache_efficiency": (
                        "excellent"
                        if rolling_metrics["cache_hit_rate"] > 0.8
                        else (
                            "good"
                            if rolling_metrics["cache_hit_rate"] > 0.5
                            else "poor"
                        )
                    ),
                },
                "alerts": {
                    ...
                    "low_cache_hit_rate": rolling_metrics["cache_hit_rate"] < 0.3
                    and rolling_metrics["calculations_total"] > 10,
        ...
        raise BusinessLogicException(
            message=f"Failed to retrieve rolling metrics: {e}", status_code=500
        )
...
async def get_ev_distribution():
    ...
    try:
            ...
            data={
                ...
                "analysis_metadata": {
                    ...
                    "data_quality": (
                        ...
                        else "moderate" if distribution.sample_size > 20 else "low"
                    ),
                },
            },
            timestamp=datetime.now().isoformat(),
        ...
        raise BusinessLogicException(message=str(e), status_code=400)
    except Exception as e:
        logger.error(f"Distribution analysis error: {e}")
        raise BusinessLogicException(
            message=f"Failed to analyze distribution: {e}", status_code=500
        )
...
async def manage_feature_flag(request: FeatureFlagRequest):
    ...
    try:
        ...
        if request.flag not in flag_map:
            ...
            raise BusinessLogicException(
                f"Invalid flag '{request.flag}'. Available flags: {available_flags}",
                status_code=400,
            )
        ...
        raise BusinessLogicException(
            message=f"Failed to manage feature flag: {e}", status_code=500
        )
...
async def get_feature_flags():
    ...
    try:
        ...
        raise BusinessLogicException(
            message=f"Failed to retrieve feature flags: {e}", status_code=500
        )
...
async def invalidate_cache(
    pattern: Optional[str] = Query(
        None, description="Pattern to match cache keys (optional)"
    )
):
    """
    ...
    try:
        ...
        return EVResponse(
            ...
            data={
                ...
                + (f" for pattern '{pattern}'" if pattern else " (all entries)"),
        ...
        raise BusinessLogicException(
            message=f"Failed to invalidate cache: {e}", status_code=500
        )
...
async def reset_metrics():
    ...
    try:
        ...
        raise BusinessLogicException(
            message=f"Failed to reset metrics: {e}", status_code=500
        )
...
async def ev_engine_health():
    ...
    try:
        ...
        if (
            metrics.get("cache_hit_rate", 0) < 0.5
            and metrics.get("total_calculations", 0) > 100
        ):
            health_status = "degraded"
            ...
            data={
                ...
                "engine_info": {
                    ...
                    "features_enabled": sum(
                        1
                        for enabled in enhanced_ev_engine.feature_flags.values()
                        if enabled
                    ),
