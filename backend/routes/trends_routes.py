...
async def get_trends_leaderboard(
    ...
    sport: SportFilter = Query(
        default=SportFilter.ALL,
        description="Sport filter: ALL, MLB, NBA, NFL, NHL"
    ),
    ...
    try:
        ...
        logger.info(f"Fetching trends leaderboard with filters: {filters.dict()}")
        
        response = await trends_service.get_trends_leaderboard(filters)
        
        logger.info(f"Returning {len(response.data)} leaderboard entries")
        return response
        
    except Exception as e:
        logger.error(f"Error fetching trends leaderboard: {str(e)}", exc_info=True)
...
async def clear_trends_cache() -> Dict[str, Any]:
    ...
    try:
        ...
        if success:
            ...
            return {
                "success": False,
                "message": "Failed to clear trends cache"
            }
