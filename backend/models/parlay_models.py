...
class ParlayLegRequest(BaseModel):
    ...
    our_fair_odds: int = Field(..., description="Our calculated fair odds in American format")
    team: Optional[str] = Field(None, description="Player's team (optional)")
    stat_type: Optional[str] = Field(None, description="Statistical category (optional)")
    
    @validator('odds', 'our_fair_odds')
...
class ParlayAnalysisRequest(BaseModel):
    ...
    legs: List[ParlayLegRequest] = Field(..., description="List of parlay legs to analyze")
...
class CorrelationWarningResponse(BaseModel):
    ...
    affected_legs: List[int] = Field(..., description="Indices of affected legs in parlay")
...
class IndividualLegAnalysis(BaseModel):
    ...
    odds: int = Field(..., description="Market odds in American format")
    implied_probability: float = Field(..., description="Implied probability from market odds")
    fair_probability: float = Field(..., description="Fair probability from our odds")
    individual_ev: float = Field(..., description="Expected value percentage for this leg alone")


class ParlayAnalyticsResponse(BaseModel):
    """Complete parlay analysis response"""
    total_payout: float = Field(..., description="Total payout multiplier for the parlay")
    implied_probability: float = Field(..., description="Combined implied probability of all legs hitting")
    fair_probability: float = Field(..., description="Combined fair probability based on our odds")
    expected_value_percent: float = Field(..., description="Expected value percentage (adjusted for correlations)")
    raw_expected_value_percent: float = Field(..., description="Raw EV before correlation adjustment")
    correlation_warnings: List[CorrelationWarningResponse] = Field(..., description="List of correlation warnings")
    risk_assessment: str = Field(..., description="Overall risk assessment for the parlay")
    individual_leg_analysis: List[IndividualLegAnalysis] = Field(..., description="Analysis of each individual leg")
    number_of_legs: int = Field(..., description="Total number of legs in parlay")
    correlation_adjustment_factor: float = Field(..., description="Factor applied to adjust for correlations")
...
class ParlayHealthResponse(BaseModel):
    ...
    correlation_patterns_loaded: bool = Field(..., description="Whether correlation patterns are loaded")
...
class ParlayAnalysisApiResponse(BaseModel):
    ...
    data: Optional[ParlayAnalyticsResponse] = Field(None, description="Analysis results if successful")
    error: Optional[ParlayAnalysisErrorResponse] = Field(None, description="Error details if failed")
...
class ParlayHealthApiResponse(BaseModel):
    ...
    data: Optional[ParlayHealthResponse] = Field(None, description="Health check results")
    message: str = Field(..., description="Human-readable response message")