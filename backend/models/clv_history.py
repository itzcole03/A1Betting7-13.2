...
class CLVHistory(SQLModel, table=True):
    ...
    opportunity_hash: str = Field(
        index=True, description="SHA256 hash of opportunity key fields"
    )
    player: Optional[str] = Field(
        default=None, index=True, max_length=100, description="Player name"
    )
    sport: Optional[str] = Field(
        default=None, index=True, max_length=20, description="Sport name"
    )
    market: Optional[str] = Field(
        default=None, index=True, max_length=50, description="Betting market"
    )
    ...
    source_version: Optional[str] = Field(
        default="v1", max_length=10, description="CLV computation version"
    )

    # Optional fields for enhanced analytics
    initial_line: Optional[float] = Field(
        default=None, description="Initial line value if available"
    )
    initial_odds: Optional[int] = Field(
        default=None, description="Initial odds value if available"
    )
