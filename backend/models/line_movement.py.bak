...
class LineSnapshot(BaseModel):
    ...
    source: Optional[str] = Field(
        None, description="Data source that triggered the snapshot"
    )
...
class MovementStats(BaseModel):
    ...
    movementMagnitude: float = Field(
        ..., description="Absolute difference between earliest and latest line"
    )
    direction: MovementDirection = Field(
        ..., description="Overall direction of movement"
    )
...
class LineMovementResponse(BaseModel):
    ...
    timeline: List[LineSnapshot] = Field(
        ..., description="Chronological list of line snapshots"
    )
    movementMagnitude: float = Field(
        ..., description="Absolute line change from start to end"
    )
    direction: MovementDirection = Field(..., description="Overall movement direction")
    volatilityScore: float = Field(
        ..., description="Volatility measure (standard deviation)"
    )
...
class LineMovementQuery(BaseModel):
    ...
    limit: Optional[int] = Field(
        40, description="Maximum snapshots to return", ge=1, le=100
    )
...
class MovementEvent:
    ...
    def is_significant(
        self, magnitude_threshold: float = 0.5, volatility_threshold: float = 1.0
    ) -> bool:
        """Check if movement is significant enough to broadcast"""
        return (
            self.magnitude >= magnitude_threshold
            or self.volatility_score >= volatility_threshold
        )
...
class MovementMetrics(BaseModel):
    ...
    high_volatility_events: int = Field(
        0, description="High volatility movements detected"
    )
    active_tracked_lines: int = Field(
        0, description="Currently tracked player-market combinations"
    )
    avg_snapshots_per_line: float = Field(
        0.0, description="Average snapshots per tracked line"
    )
...
class MovementConfiguration(BaseModel):
    ...
    max_snapshots_per_line: int = Field(
        40, description="Maximum snapshots to store per line"
    )
    volatility_threshold: float = Field(
        1.0, description="Threshold for high volatility detection"
    )
    magnitude_threshold: float = Field(
        0.5, description="Threshold for significant movement"
    )
    snapshot_ttl_hours: int = Field(
        168, description="TTL for snapshots in hours (default: 7 days)"
    )
    redis_key_prefix: str = Field(
        "line_mv", description="Redis key prefix for line movement data"
    )
