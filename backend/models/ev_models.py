...
class EVOpportunity(BaseModel):
    ...
    market: str = Field(..., description="Market description (e.g., 'Points Over 25.5')")
    ...
    our_fair_odds: float = Field(..., description="Our calculated fair odds", ge=-1000, le=1000)
    market_odds: int = Field(..., description="Market odds from sportsbook", ge=-1000, le=1000)
    ...
    # Additional context
    confidence_score: Optional[float] = Field(None, description="Confidence in fair odds calculation", ge=0, le=1)
    volume_indicator: Optional[str] = Field(None, description="Betting volume indicator")
...
class EVFeedRequest(BaseModel):
    ...
    min_ev: Optional[float] = Field(3.0, description="Minimum EV percentage", ge=0, le=100)
    ...
    limit: Optional[int] = Field(100, description="Maximum number of opportunities", ge=1, le=500)


class EVFeedResponse(BaseModel):
    """Response model for +EV feed endpoint"""
    opportunities: List[EVOpportunity] = Field(..., description="List of +EV opportunities")
...
class EVFeedStats(BaseModel):
    ...
    generation_duration_ms: int = Field(..., description="Feed generation time in milliseconds")
    max_edge: Optional[float] = Field(None, description="Maximum EV percent in current feed (optional)")


class EVForecastItem(BaseModel):
    """Forecast item for an opportunity based on recent EV slope"""
    key: str = Field(..., description="Stable identity key for the opportunity snapshots")
    ...
    slope_per_min: float = Field(..., description="Estimated EV% slope per minute over recent snapshots")
...
class EVForecastResponse(BaseModel):
    ...
    items: List[EVForecastItem] = Field(..., description="Forecast items with positive slope")
    total_count: int = Field(..., description="Total available items before limit")
    filters_applied: Dict[str, Any] = Field(..., description="Applied filters including min_ev and limit")


def calculate_expected_value(market_odds: int, fair_odds: float, stake: float = 100.0) -> EVCalculationResult:
    """
