...
class TrendLeaderboardEntry(BaseModel):
    ...
    high_confidence_rate: float = Field(..., description="Rate of high confidence (>70%) bets")
    ...
    rank: Optional[int] = Field(default=None, description="Rank in leaderboard for selected metric")


class TrendLeaderboardFilters(BaseModel):
    """Filters for trend leaderboard queries"""
    metric: TrendMetric = Field(default=TrendMetric.OVER_HIT_RATE, description="Primary metric to rank by")
    sport: SportFilter = Field(default=SportFilter.ALL, description="Sport filter")
    market_type: MarketTypeFilter = Field(default=MarketTypeFilter.ALL, description="Market type filter")
    min_samples: int = Field(default=5, ge=1, le=100, description="Minimum number of props for inclusion")
...
class TrendLeaderboardResponse(BaseModel):
    ...
    cache_timestamp: Optional[datetime] = Field(default=None, description="When data was cached")
