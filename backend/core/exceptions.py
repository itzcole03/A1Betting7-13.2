...
class BusinessLogicException(HTTPException):
    ...
    def __init__(
        ...
        # Backwards-compatible alias used in some callsites/tests
        ...
        """
        # Support legacy 'detail' kwarg by merging it into details and accept
        # message being optional for backwards compatibility with older callsites
        self.error_code = error_code
        # If a single-detail blob was passed via `detail=...`, normalize to details dict
        if detail is not None and not details:
            if isinstance(detail, dict):
                self.details = detail
            else:
                self.details = {"detail": detail}
        else:
            self.details = details or {}

        # Determine effective message to pass to HTTPException
        effective_message = (
            message
            or (detail if isinstance(detail, str) else None)
            or "Business logic error"
        )
        self.user_message = user_message or effective_message

        # Call parent with the technical message and provided status_code
        super().__init__(status_code=status_code, detail=effective_message)
