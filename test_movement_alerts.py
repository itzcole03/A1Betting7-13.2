...
from backend.services.movement_alert_service import MovementAlertService, MovementAlertType
from backend.services.simple_propfinder_service import SimplePropFinderService, PropOpportunity, Sport, Market, Pick, Trend, Venue, Direction, SharpMoney, MatchupHistory, LineMovement

def create_test_opportunity(player: str, market: str, line: float, odds: int, clv_percent: float = 0.0):
...
async def test_movement_alert_service():
    ...
    print(f"✅ Service initialized with {len(alert_service.movement_thresholds)} alert types")
    ...
    for alert_type, config in status['thresholds'].items():
        print(f"   {alert_type}: threshold={config['threshold_value']}, enabled={config['enabled']}")
    ...
    opportunities = [
        ...
        create_test_opportunity("Stephen Curry", "3-Pointers Made", 4.5, -105, -3.2),  # Negative CLV
    ...
    for opportunity in opportunities:
        try:
            ...
            opportunity_alerts = await alert_service._analyze_opportunity_movement(opportunity)
            alerts.extend(opportunity_alerts)
            print(f"   Analyzed {opportunity.player} {opportunity.market}: {len(opportunity_alerts)} alerts")
    ...
    original_threshold = alert_service.movement_thresholds[MovementAlertType.LINE_MOVEMENT].threshold_value
    ...
    new_threshold = alert_service.movement_thresholds[MovementAlertType.LINE_MOVEMENT].threshold_value
    ...
    clv_enabled = alert_service.movement_thresholds[MovementAlertType.CLV_DEGRADATION].enabled
    ...
    in_cooldown = alert_service._is_in_cooldown(prop_id, MovementAlertType.LINE_MOVEMENT, 15)
    ...
    print("✅ Ready for integration with CLV foundation and existing alerting infrastructure")
