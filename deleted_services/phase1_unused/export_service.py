...
class DataExportService:
    ...
    async def create_export(
        ...
        asyncio.create_task(
            self._process_export(export_id, data, available_fields, options)
        )
    ...
    async def _process_export(
        ...
        try:
            ...
            prepared_data = await self._prepare_data(
                data, available_fields, options, export_id
            )

            # Generate export file
            file_content = await self._generate_export_file(
                prepared_data, available_fields, options, export_id
            )
    ...
    async def _prepare_data(
        ...
        for i, record in enumerate(data):
            ...
            if options.filters and not self._record_matches_filters(
                record, options.filters
            ):
                continue
            ...
            for field in selected_fields:
                ...
                if value is not None:
                    prepared_record[field.key] = self._format_field_value(
                        value, field, options
                    )
                else:
        ...
        if options.sorting["field"] in [f.key for f in selected_fields]:
            ...
            prepared_data.sort(
                key=lambda x: x.get(options.sorting["field"], ""), reverse=reverse
            )
    ...
    def _format_field_value(
        self, value: Any, field: ExportField, options: ExportOptions
    ) -> str:
            ...
            "metadata": (
                {
                    ...
                    "options": (
                        (
                            ...
                            and callable(getattr(options, "model_dump"))
                            else None
                        )
                        or (
                            options.dict()
                            if hasattr(options, "dict")
                            and callable(getattr(options, "dict"))
                            else None
                        )
                        or dict(getattr(options, "__dict__", {}) or {})
                    ),
                }
    ...
    def _generate_pdf(
        ...
        if options.formatting.get("include_metadata", True):
            ...
            metadata_style = ParagraphStyle(
                "Metadata", parent=styles["Normal"], fontSize=8, textColor=colors.grey
            )
        ...
        for export_id in expired_exports:
            ...
            if progress and progress.file_path:
                try:
                    ...
                    logger.error(
                        f"Failed to delete export file {progress.file_path}: {e}"
                    )
