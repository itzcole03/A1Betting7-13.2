...
async def test_propollama_unified():
    ...
    try:
        ...
        print(
            f"✅ Generated {len(best_bets_result.get('best_bets', []))} best bets in {generation_time:.2f}s"
        )
        ...
        for i, bet in enumerate(best_bets_result.get("best_bets", [])[:3], 1):
            print(
                f"   {i}. {bet['player_name']} - {bet['stat_type']} {bet['recommendation']} {bet['line']} ({bet['confidence']}%)"
            )
        ...
        if hasattr(engine, "process_chat_message_with_web_research"):
            chat_result = await engine.process_chat_message_with_web_research(
                chat_request
            )
        else:
        ...
        try:
            ...
            # Get projections
            projections = (
                await comprehensive_prizepicks_service.get_comprehensive_projections()
            )
            ...
            if props_count > 0:
                ...
                print(
                    f"   Sample: {sample_prop.get('player_name', 'Unknown')} - {sample_prop.get('stat_type', 'Unknown')}"
                )
        ...
        try:
            real_manager = RealDataModelManager(
                projections if "projections" in locals() else {"props": []}
            )
            real_engine = EnhancedPropOllamaEngine(real_manager)

            real_best_bets = await real_engine.generate_best_bets(3)
            print(
                f"✅ Generated {len(real_best_bets.get('best_bets', []))} best bets with real data"
            )
        ...
        async with httpx.AsyncClient() as client:
            ...
            try:
                response = await client.get(
                    "http://localhost:8001/api/health/status", timeout=10.0
                )
            ...
            try:
                response = await client.get(
                    "http://localhost:8001/api/propollama/best-bets-unified?limit=3",
                    timeout=30.0,
                )
                if response.status_code == 200:
                    data = response.json()
                    print(
                        f"✅ Best bets endpoint working: {len(data.get('best_bets', []))} bets returned"
                    )
                else:
            ...
            try:
                ...
                response = await client.post(
                    "http://localhost:8001/api/propollama/chat-unified",
                    json=chat_data,
                    timeout=30.0,
                )
                if response.status_code == 200:
                    data = response.json()
                    print(
                        f"✅ Chat endpoint working: {data.get('status', 'unknown')} response"
                    )
                else:
