...
class TestCacheMetricsHook:
    ...
    def test_cache_hit_rate_calculation(self):
        ...
        expected_hit_rate = (
            cache_metrics["hits"] / total_operations if total_operations > 0 else 0.0
        )
    ...
    def test_auto_hook_unified_cache_not_available(self):
        ...
        with patch(
            "backend.services.metrics.cache_metrics_hook.unified_cache_service",
    ...
    @patch(
        "backend.services.metrics.cache_metrics_hook.intelligent_cache_service", None
    )
    def test_auto_hook_intelligent_cache_not_available(self):
        """Test auto-hooking when intelligent cache service is not available."""
        with patch(
            "backend.services.metrics.cache_metrics_hook.intelligent_cache_service",
            side_effect=ImportError,
        ):
            result = auto_hook_intelligent_cache_service()
            assert result is False  # Should fail gracefully
    ...
    def test_initialize_cache_metrics_hooks(self):
        ...
        with patch(
            "backend.services.metrics.cache_metrics_hook.auto_hook_unified_cache_service",
            return_value=True,
        ), patch(
            "backend.services.metrics.cache_metrics_hook.auto_hook_intelligent_cache_service",
            return_value=False,
        ):
    ...
    def test_cache_hook_performance_overhead(self):
        ...
        # Overhead should be minimal (allow some headroom for test environments)
