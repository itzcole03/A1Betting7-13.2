...
class TestCacheInstrumentation:
    ...
    def instrumentation(self):
        ...
        Some production code starts background coroutines for periodic cleanup
        when an event loop is present; in sync unit tests there may be no
        running loop which can lead to 'coroutine was never awaited' warnings.

        To keep tests synchronous and avoid scheduling cleanup coroutines,
        temporarily stub the periodic cleanup implementation during the
        fixture lifetime.
        """
        import asyncio

        # Ensure an event loop exists for any async primitives used by the class
        try:
        ...
        # Patch CacheInstrumentation._periodic_cleanup to avoid creating an
        # un-awaited coroutine during instantiation in sync tests.
        orig_cleanup = CacheInstrumentation._periodic_cleanup
        CacheInstrumentation._periodic_cleanup = lambda self: None
        try:
            inst = CacheInstrumentation()
            yield inst
        finally:
            # Restore original implementation
            CacheInstrumentation._periodic_cleanup = orig_cleanup
    ...
    def test_basic_metrics_tracking(self, instrumentation):
        ...
        assert (
            len(instrumentation._recent_latencies) == 4
        )  # hits + miss + set recorded latency    def test_latency_percentiles(self, instrumentation):
...
class TestCacheKeyBuilder:
    ...
    def test_basic_key_building(self, builder):
        ...
        key = builder.build_key(
            tier=CacheTier.RAW_PROVIDER, entity=CacheEntity.USER, identifier="user_123"
        )
    ...
    def test_key_building_functionality(self, builder):
        ...
        key = builder.build_key(
            CacheTier.RAW_PROVIDER, CacheEntity.USER, "123", sub_keys=["profile"]
        )
        assert "user" in key
        assert "123" in key
...
class TestCacheServiceExt:
    ...
    def cache_ext(self, mock_cache_service, mock_instrumentation):
        ...
        with patch(
            "backend.services.cache_service_ext.unified_cache", mock_cache_service
        ), patch(
            "backend.services.cache_service_ext.cache_instrumentation",
            mock_instrumentation,
        ):
            return CacheServiceExt()

    @pytest.mark.asyncio
    async def test_basic_get_operation(
        self, cache_ext, mock_cache_service, mock_instrumentation
    ):
        ...
        assert (
            "test:key" in call_args[0]
        )  # Original key should be part of versioned key
    ...
    @pytest.mark.asyncio
    async def test_get_or_build_cache_hit(
        self, cache_ext, mock_cache_service, mock_instrumentation
    ):
    ...
    @pytest.mark.asyncio
    async def test_get_or_build_cache_miss(
        self, cache_ext, mock_cache_service, mock_instrumentation
    ):
    ...
    @pytest.mark.asyncio
    async def test_error_handling_in_get_or_build(
        self, cache_ext, mock_cache_service, mock_instrumentation
    ):
...
class TestCachePerformance:
    ...
    async def test_concurrent_get_or_build_performance(self):
        ...
        logger.debug(
            f"Concurrent operations completed in {end_time - start_time:.2f}s with stampede protection"
        )
