...
from backend.services.health_service import get_health_status, health_service
...
class TestHealthEndpoints:
    ...
    def test_legacy_health_endpoint_deprecated(self, client):
        ...
        # Enable legacy deprecation hints for this test only so it can assert
        # on the presence of `deprecated` and `forward` fields without
        # impacting other tests that expect canonical envelopes.
    ...
    def test_health_endpoint_error_handling(self, client):
        ...
        with patch.object(health_service, "compute_health") as mock_compute:
            ...
            assert response.status_code in [
                200,
                503,
            ]  # Either OK or Service Unavailable
...
class TestHealthServiceIntegration:
    ...
    async def test_health_service_with_mock_websocket(self):
        ...
        with patch(
            "backend.services.health_service.asyncio.sleep", new_callable=AsyncMock
        ):
            health = await health_service.check_websocket_health()
    ...
    async def test_health_service_resilience(self):
        ...
        with patch.object(
            health_service, "check_cache_health", side_effect=Exception("Cache error")
        ):
