...
class TestProviderConfidenceIntegration:
    ...
    def test_selection_priority_calculation(self, integration):
        ...
        priority = integration._calculate_selection_priority(0.9, CircuitBreakerState.CLOSED)
        ...
        priority = integration._calculate_selection_priority(0.3, CircuitBreakerState.OPEN)
        assert priority == 270  # (1.0 - 0.3) * 100 + 200 = 270
        
        # Medium confidence, half-open circuit
        priority = integration._calculate_selection_priority(0.6, CircuitBreakerState.HALF_OPEN)
    ...
    async def test_get_provider_confidence_score_with_mocks(self, integration):
        ...
        with patch.object(mock_enhanced_metrics, 'get_comprehensive_confidence_score', return_value=0.82):
            integration.statistics_manager.provider_metrics[provider_id] = mock_enhanced_metrics
        ...
        integration.resilience_manager.provider_metrics[provider_id] = mock_resilience_metrics
        ...
        assert score.adjusted_confidence == pytest.approx(0.9425, rel=1e-3)  # No penalty applied
    ...
    async def test_confidence_score_with_circuit_breaker_penalty(self, integration):
        ...
        with patch.object(mock_enhanced_metrics, 'get_comprehensive_confidence_score', return_value=0.65):
            integration.statistics_manager.provider_metrics[provider_id] = mock_enhanced_metrics
        ...
        assert score.adjusted_confidence == pytest.approx(0.1605, rel=1e-3)  # 0.8025 * (1 - 0.8) ≈ 0.1605
    ...
    async def test_select_optimal_provider_success(self, integration):
        ...
        with patch.object(integration, 'get_provider_confidence_score') as mock_get_score:
            mock_get_score.side_effect = mock_scores
            
            result = await integration.select_optimal_provider(providers, confidence_threshold=0.7)
    ...
    async def test_should_trigger_circuit_breaker_enhanced(self, integration):
        ...
        with patch.object(integration, 'get_provider_confidence_score', return_value=low_confidence_score):
            should_trigger = await integration.should_trigger_circuit_breaker(provider_id)
    ...
    async def test_should_trigger_circuit_breaker_high_confidence(self, integration):
        ...
        with patch.object(integration, 'get_provider_confidence_score', return_value=high_confidence_score):
            should_trigger = await integration.should_trigger_circuit_breaker(provider_id)
    ...
    async def test_get_provider_rankings(self, integration):
        ...
        for pid, confidence in provider_data.items():
            mock_scores.append(ProviderConfidenceScore(
                provider_id=pid,
        ...
        for provider_id, confidence, level in rankings:
            ...
            assert level in [ConfidenceLevel.EXCELLENT, ConfidenceLevel.GOOD, 
                           ConfidenceLevel.FAIR, ConfidenceLevel.POOR, ConfidenceLevel.CRITICAL]
    ...
    async def test_update_provider_confidence_on_request(self, integration):
        ...
        # Mock the managers
        integration.statistics_manager.provider_metrics[provider_id] = EnhancedProviderMetrics(provider_id)
        integration.resilience_manager.provider_metrics[provider_id] = ProviderMetrics()
        
        # Mock record_provider_request
        with patch.object(integration.statistics_manager, 'record_provider_request') as mock_record:
        ...
        resilience_metrics = integration.resilience_manager.provider_metrics[provider_id]
    ...
    def test_integration_status(self, integration):
        ...
        # Add some mock data
        integration.provider_confidence_cache["test_provider"] = ProviderConfidenceScore(
            provider_id="test_provider",
            confidence_score=0.8,
            confidence_level=ConfidenceLevel.GOOD,
            circuit_state=CircuitBreakerState.CLOSED,
...
class TestProviderConfidenceAPIRoutes:
    ...
    @patch('backend.routes.provider_confidence_routes.get_provider_confidence_integration')
    def test_get_provider_confidence_score_endpoint(self, mock_get_integration, test_client):
        ...
        mock_integration.get_provider_confidence_score = AsyncMock(return_value=mock_score)
    ...
    def test_select_provider_endpoint(self, mock_get_integration, test_client):
        ...
        response = test_client.post(
            "/api/odds/provider-confidence/select",
            json=["primary_provider"]
        )
        assert response.status_code == 200
