...
class TestEVOpportunityPersistence:
    ...
    async def test_persist_ev_opportunity_above_threshold(self, analytics_service):
        ...
        result = await analytics_service.persist_ev_opportunity(
            EVOpportunityData(
                sport="MLB",
                player="Aaron Judge",
                market="Home Runs",
    ...
    @pytest.mark.asyncio
    async def test_ev_opportunity_deduplication(
        self, analytics_service, sample_ev_data
    ):
    ...
    def test_ev_hash_calculation(self):
        ...
        hash1 = EVOpportunityHistory.calculate_hash(
            "MLB", "Player A", "Hits", 2.5, -110
        )
        hash2 = EVOpportunityHistory.calculate_hash(
            "MLB", "Player A", "Hits", 2.5, -110
        )
        hash3 = EVOpportunityHistory.calculate_hash(
            "MLB", "Player A", "Hits", 2.5, -120
        )
...
class TestArbitrageOpportunityPersistence:
    ...
    @pytest.mark.asyncio
    async def test_persist_arbitrage_opportunity_above_threshold(
        self, analytics_service, sample_arbitrage_data
    ):
        """Test persisting arbitrage opportunity above threshold"""
        result = await analytics_service.persist_arbitrage_opportunity(
    ...
    @pytest.mark.asyncio
    async def test_persist_arbitrage_opportunity_below_threshold(
...
class TestDailyAggregation:
    ...
    async def test_daily_ev_stats_with_data(self, analytics_service, async_session):
        ...
        yesterday_stats = next(
            s for s in stats if s.date == yesterday.date().isoformat()
        )
        assert yesterday_stats.total_opportunities == 1
        assert yesterday_stats.avg_ev_percent == 3.5

    @pytest.mark.asyncio
    async def test_daily_arbitrage_stats_with_data(
        self, analytics_service, async_session
    ):
...
class TestHelperFunctions:
    ...
    async def test_persist_ev_opportunity_helper_above_threshold(self):
        ...
        with patch(
            "backend.services.analytics_scheduler.AnalyticsPersistenceService"
