...
class TestEnhancedSchemaValidator:
    ...
    def test_validation_result_properties(self, validator):
        ...
        assert result.warning_count_by_category == {"schema_structure": 1, "data_range": 1}
    ...
    def test_valid_sportradar_data_validation(self, validator, sample_sportradar_data):
        ...
        result = validator.validate_aggregated_odds(
            raw_data=sample_sportradar_data,
            provider="SportRadar"
        )
        ...
        critical_warnings = [w for w in result.warnings if w.level == ValidationLevel.CRITICAL]
        assert len(critical_warnings) == 0
    
    def test_valid_theodds_data_validation(self, validator, sample_theodds_data):
        """Test validation of valid TheOdds data"""
        result = validator.validate_aggregated_odds(
            raw_data=sample_theodds_data,
            provider="TheOdds"
        )
        
        assert result.is_valid is True
        assert result.processed_data is not None
        assert result.processed_data.sportsbook == "DraftKings"  # Normalized from TheOdds data
    ...
    def test_invalid_data_type_validation(self, validator):
        ...
        result = validator.validate_aggregated_odds(
            raw_data="not_a_dict",  # Invalid type
            provider="SportRadar"
        )
    ...
    def test_extreme_odds_validation(self, validator):
        ...
        assert any(w.level in [ValidationLevel.CRITICAL, ValidationLevel.WARNING] for w in odds_warnings)
    ...
    def test_temporal_validation_future_timestamp(self, validator):
        ...
        with patch('backend.api_integration.OddsNormalizer.normalize_odds_data') as mock_normalize:
            ...
            temporal_warnings = [w for w in result.warnings 
    ...
    def test_business_logic_validation_with_context(self, validator):
        ...
        result = validator.validate_aggregated_odds(
            raw_data=high_hits_data,
            provider="Internal",
            context=context
        )
        
        # Should have business logic warning about high hits line
        business_warnings = [w for w in result.warnings 
    ...
    def test_data_sanitization(self, validator):
        ...
        with patch('backend.api_integration.OddsNormalizer.normalize_odds_data') as mock_normalize:
            ...
            if result.processed_data:
                ...
                sportsbook_warnings = [w for w in result.warnings 
                                     if w.field == "sportsbook" and 
                                     w.category == ValidationCategory.DATA_CONSISTENCY]
    ...
    def test_exception_handling(self, validator):
        ...
        with patch('backend.api_integration.OddsNormalizer.normalize_odds_data') as mock_normalize:
            ...
            error_warnings = [w for w in result.warnings if "normalization failed" in w.message.lower()]
...
class TestSchemaValidationAPIRoutes:
    ...
    def test_validate_raw_data_endpoint(self, test_client):
        ...
        response = test_client.post(
            "/api/odds/validation/validate-raw",
            json=test_payload
        )
        assert response.status_code == 200
    ...
    def test_provider_statistics_specific_endpoint(self, test_client):
        ...
        response = test_client.get("/api/odds/validation/statistics/NonExistentProvider")
