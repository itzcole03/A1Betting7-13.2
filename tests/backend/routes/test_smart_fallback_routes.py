...
from backend.routes.smart_fallback_routes import (
...
class TestSmartFallbackAPIRoutes:
    ...
    def test_health_check_success(self, client, mock_fallback_service):
    ...
    def test_health_check_failure(self, client):
        ...
        with patch('backend.routes.smart_fallback_routes.get_fallback_service', side_effect=Exception("Service unavailable")):
    ...
    def test_get_provider_priorities_success(self, client, mock_fallback_service, sample_provider_priorities):
        """Test successful provider priorities retrieval"""
        mock_fallback_service.get_provider_priorities.return_value = sample_provider_priorities
        
        response = client.get("/api/fallback/priorities/odds_aggregation?available_providers=draftkings,fanduel")
    ...
    def test_get_provider_priorities_force_refresh(self, client, mock_fallback_service, sample_provider_priorities):
        """Test provider priorities with force refresh"""
        mock_fallback_service.get_provider_priorities.return_value = sample_provider_priorities
        
        response = client.get("/api/fallback/priorities/odds_aggregation?available_providers=draftkings&force_refresh=true")
    ...
    def test_get_provider_priorities_no_providers(self, client):
        ...
        response = client.get("/api/fallback/priorities/odds_aggregation?available_providers=")
    ...
    def test_get_provider_priorities_service_error(self, client, mock_fallback_service):
        ...
        mock_fallback_service.get_provider_priorities.side_effect = Exception("Database error")
        
        response = client.get("/api/fallback/priorities/odds_aggregation?available_providers=draftkings")
    ...
    def test_set_primary_provider_success(self, client, mock_fallback_service):
        ...
        response = client.post("/api/fallback/primary-provider", json={
        ...
        mock_fallback_service.set_primary_provider.assert_called_once_with("odds_aggregation", "fanduel")
    ...
    def test_set_primary_provider_service_error(self, client, mock_fallback_service):
        ...
        mock_fallback_service.set_primary_provider.side_effect = Exception("Provider not found")
    ...
    def test_select_optimal_provider_success(self, client, mock_fallback_service, sample_provider_priorities):
        """Test successful optimal provider selection"""
        mock_fallback_service.select_optimal_provider.return_value = ("fanduel", FallbackReason.LOW_CONFIDENCE)
        mock_fallback_service.get_provider_priorities.return_value = sample_provider_priorities
        
        response = client.post("/api/fallback/select-provider", json={
            "context": "odds_aggregation",
    ...
    def test_select_optimal_provider_no_fallback(self, client, mock_fallback_service, sample_provider_priorities):
        """Test optimal provider selection when no fallback is needed"""
        mock_fallback_service.select_optimal_provider.return_value = ("draftkings", None)
        mock_fallback_service.get_provider_priorities.return_value = sample_provider_priorities
    ...
    def test_get_fallback_configuration_success(self, client, mock_fallback_service):
    ...
    def test_get_active_contexts_success(self, client, mock_fallback_service):
    ...
    def test_get_system_status_success(self, client, mock_fallback_service):
        ...
        mock_fallback_service.fallback_history = [
            Mock(timestamp=current_time - 1800, success=True, fallback_provider="fanduel"),  # 30 min ago
            Mock(timestamp=current_time - 3600, success=False, fallback_provider="betmgm"),  # 1 hour ago
            Mock(timestamp=current_time - 7200, success=True, fallback_provider="caesars")   # 2 hours ago (should be excluded)
        ]
        ...
        assert data["recent_activity"]["fallback_events_last_hour"] == 1  # Only events within last hour
    ...
    @pytest.mark.asyncio
    async def test_concurrent_requests(self, client, mock_fallback_service, sample_provider_priorities):
        ...
        async def make_request():
            return client.get("/api/fallback/priorities/odds_aggregation?available_providers=draftkings,fanduel")
    ...
    def test_response_model_validation(self, client, mock_fallback_service, sample_provider_priorities):
