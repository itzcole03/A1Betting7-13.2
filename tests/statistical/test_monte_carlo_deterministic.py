...
class DeterministicSeedPipeline:
    ...
    def generate_simulation_seeds(self, n_simulations: int) -> List[int]:
        ...
        for i in range(n_simulations):
            ...
            seed = (self.baseline.mc_seed + i * 1009) % (
                2**31 - 1
            )  # Large prime for spacing
            seeds.append(seed)
...
class KSTestAutoRunner:
    ...
    def compare_distributions(
        ...
        try:
            ...
            # Perform KS test
            ks_statistic, p_value = stats.kstest(sorted_samples, analytic_cdf_func)
            ...
            try:
                if (
                    hasattr(self, "parent_suite")
                    and hasattr(self.parent_suite, "all_results")
                    and self.test_results
                ):
...
class CorrelationPSDDriftMonitor:
    ...
    def __init__(self, baseline: StatisticalBaseline):
        ...
        self.drift_state = DriftMonitorState(
            last_baseline_update=datetime.now(timezone.utc)
        )

    def validate_correlation_matrix(
        self, correlation_matrix: np.ndarray, matrix_name: str = "correlation_matrix"
    ) -> StatisticalTestResult:
        """
        ...
        try:
            ...
            if (
                ...
                raise ValueError(
                    f"Matrix must be square, got shape {correlation_matrix.shape}"
                )

            # Check if matrix is symmetric
            if not np.allclose(correlation_matrix, correlation_matrix.T, atol=1e-10):
                logger.warning(
                    f"Matrix {matrix_name} is not symmetric, forcing symmetry"
                )
            ...
            condition_number = max_eigenvalue / max(
            ...
            condition_valid = (
                condition_number <= self.baseline.condition_number_threshold
            )
            ...
            try:
                self.drift_state.correlation_matrix_history.append(
                    correlation_matrix.copy()
                )
                if (
                    len(self.drift_state.correlation_matrix_history) > 100
                ):  # Limit memory usage
    ...
    def detect_drift(
        self, current_matrix: np.ndarray, reference_matrix: Optional[np.ndarray] = None
    ) -> StatisticalTestResult:
        """
        ...
        try:
            ...
            drift_detected = (
                normalized_drift > self.baseline.correlation_drift_threshold
            )
            ...
            try:
                if (
                    hasattr(self, "parent_suite")
                    and hasattr(self.parent_suite, "all_results")
                    and self.drift_state is not None
                ):
                    latest = (
                        self.parent_suite.all_results
                        and self.parent_suite.all_results[-1]
                        if self.parent_suite.all_results
                        else None
                    )
                    if (
                        "result" in locals()
                        and result not in self.parent_suite.all_results
                    ):
    ...
    def check_ci_width_regression(
        ...
        try:
            ...
            if test_name not in self._baseline_ci_widths:
                ...
                regression_detected = (
                    relative_change > self.baseline.ci_width_regression_threshold
                )
            ...
            try:
                if hasattr(self, "parent_suite") and hasattr(
                    self.parent_suite, "all_results"
                ):
...
class StatisticalTestSuite:
    ...
    def run_full_suite(
        ...
        try:
            ...
            self.suite_end_time = datetime.now(timezone.utc)
            ...
            return {
                ...
                "error": str(e),
                "duration_seconds": (
                    self.suite_end_time - self.suite_start_time
                ).total_seconds(),
            }
    ...
    def _run_ks_comparison_tests(self):
        ...
        betting_probs = sample_truncated_beta(
            1.2, 1.8, self.baseline.mc_sample_size, low=0.1, high=0.9
        )
    ...
    def _run_correlation_matrix_tests(self):
        ...
        valid_corr_matrix = create_test_correlation_matrix(
            n_assets, condition_number=10.0
        )
        ...
        drift_matrix = valid_corr_matrix + np.random.normal(
            0, 0.01, valid_corr_matrix.shape
        )
    ...
    def _run_ci_width_regression_tests(self):
        ...
        dispersed_probs = np.random.beta(
            1, 1, self.baseline.mc_sample_size
        )  # More uniform
        self.ci_guard.check_ci_width_regression(
    ...
    def _generate_suite_summary(self) -> Dict[str, Any]:
        ...
        for result in self.all_results:
            if result.error_message:
                ...
                "mc_vs_analytic" in result.test_name
                or "distribution" in result.test_name
            ):
    ...
    def _check_alert_conditions(self) -> List[str]:
        ...
        try:
            combined_results.extend(
                getattr(self.drift_monitor.drift_state, "recent_results", []) or []
            )
        except Exception:
            pass
    ...
    def save_results(self, output_path: Optional[str] = None) -> str:
        ...
        if output_path is None:
            output_path = f"statistical_test_results_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}.json"
...
def create_test_correlation_matrix(
    size: int, condition_number: float = 10.0
) -> np.ndarray:
    ...
    eigenvalues = np.logspace(np.log10(min_eigenvalue), np.log10(max_eigenvalue), size)
