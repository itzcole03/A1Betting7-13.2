...
class CIStatisticalRunner:
    ...
    def generate_ci_report(self, test_results: Dict[str, Any]) -> str:
        ...
        with open(report_path, "w") as f:
            ...
            f.write(
                f"**Status:** {'✅ PASS' if test_results['success'] else '❌ FAIL'}\n\n"
            )
            ...
            if "test_details" in test_results:
                ...
                if failed_tests:
                    ...
                    for test in failed_tests:
                        ...
                        f.write(
                            f"**Error:** {test.get('call', {}).get('longrepr', 'N/A')}\n\n"
                        )
            ...
            if not test_results["success"]:
                ...
                f.write(
                    "The statistical verification suite has detected issues that require attention:\n"
                )
                f.write("- Monte Carlo simulations may not be converging properly\n")
                f.write(
                    "- Statistical distributions may have shifted from expected baselines\n"
                )
                f.write(
                    "- Correlation matrices may be exhibiting drift or numerical instability\n"
                )
                f.write("- Confidence intervals may have regressed in quality\n\n")
                f.write(
                    "Please review the test output and address any statistical anomalies before merging.\n"
                )
            else:
                f.write("## Build Recommendation\n\n")
    ...
    def set_github_outputs(self, test_results: Dict[str, Any]):
        ...
        if "GITHUB_OUTPUT" in os.environ:
            with open(os.environ["GITHUB_OUTPUT"], "a") as f:
                f.write(
                    f"statistical_tests_passed={str(test_results['success']).lower()}\n"
                )
                f.write(
                    f"statistical_tests_timestamp={test_results.get('timestamp', '')}\n"
                )
                ...
                f.write(
                    f"should_fail_build={str(not test_results['success']).lower()}\n"
                )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run statistical verification suite in CI"
    )
    parser.add_argument("--workspace", default=".", help="Workspace directory")
    parser.add_argument(
        "--setup-only", action="store_true", help="Only setup environment"
    )
