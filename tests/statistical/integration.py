...
class StatisticalVerificationIntegration:
    ...
    def run_quick_validation(self) -> Dict[str, Any]:
        ...
        cmd = [
            ...
            str(
                self.tests_dir
                / "test_statistical_verification_suite.py::TestDeterministicMonteCarlo::test_seed_reproducibility"
            ),
            str(
                self.tests_dir
                / "test_statistical_verification_suite.py::TestKSTestRunner::test_ks_normal_distribution"
            ),
            "-v",
            "--tb=short",
        ]
    ...
    def run_full_verification(self) -> Dict[str, Any]:
        ...
        if not ci_runner_path.exists():
            return {"success": False, "error": "CI runner not found", "return_code": -1}
    ...
    def run_drift_injection_tests(self) -> Dict[str, Any]:
        ...
        env = os.environ.copy()
        env.update({"PYTEST_STATISTICAL_SEED": "42", "STATISTICAL_SAMPLE_SIZE": "5000"})
    ...
    def generate_integration_report(self, results: Dict[str, Dict[str, Any]]) -> str:
        ...
        with open(report_path, "w") as f:
            ...
            if all_passed:
                ...
                f.write(
                    "The statistical verification suite is working correctly and ready for:\n"
                )
                ...
                f.write(
                    "2. Configure alert thresholds for your specific requirements\n"
                )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Statistical Verification Suite Integration"
    )
    ...
    parser.add_argument(
        "--full", action="store_true", help="Run full verification only"
    )
    parser.add_argument(
        "--drift", action="store_true", help="Run drift injection tests only"
    )
    parser.add_argument(
