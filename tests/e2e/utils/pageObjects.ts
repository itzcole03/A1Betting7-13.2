...
export class BasePage {
  ...
  async goto(path: string = "/") {
    ...
    if (lastError) {
      // If the navigation failed, attempt a client-side route change when
      // possible (useful for SPAs where a direct navigation sometimes fails
      // due to dev server behavior). This only works if we're already on the
      // same origin (usually true in tests after initial load).
      try {
        ...
        // keep lastError for reporting
        lastError = lastError || e;
      }
    }

    if (lastError) {
      // Final attempt: force a top-level navigation by assigning location.href
      try {
        const base = new URL(this.page.url()).origin;
        const target = new URL(path, base).href;
    ...
    if (lastError) {
      ...
      // Re-throw the last error so Playwright records it as a test failure
      throw lastError;
    }
  }
  ...
  async waitForPageLoad() {
    ...
    try {
      // Try several selectors that commonly dismiss onboarding.
      const skipSelectors = [
