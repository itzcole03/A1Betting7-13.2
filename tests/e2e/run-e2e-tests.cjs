...
// End-to-End Test Runner for Phase 4.3 (CommonJS copy)
...
class E2ETestRunner {
  ...
  async checkEnvironment() {
    ...
    try {
      ...
      try {
        const res = execSync(`node -e "(async ()=>{const r=await fetch('${baseURL}',{method:'HEAD'});console.log(r.status)})().catch(e=>{console.error(e.message); process.exit(2)})"`, { stdio: 'pipe' });
    ...
    try {
      ...
      for (const dir of artifactDirs) {
        if (fs.existsSync(dir)) {
          ...
          const screenshots = files.filter(file => file.toString().endsWith('.png') || file.toString().endsWith('.jpg'));
          this.results.screenshots.push(...screenshots.map(file => path.join(dir, file.toString())));
          const videos = files.filter(file => file.toString().endsWith('.webm') || file.toString().endsWith('.mp4'));
  ...
  generateReport() {
    ...
    const successRate = this.results.summary.total > 0 ? (this.results.summary.passed / this.results.summary.total * 100).toFixed(1) : 0;
...
Usage: node tests/e2e/run-e2e-tests.cjs [options]
...
Examples:
  node tests/e2e/run-e2e-tests.cjs                           # Run all tests
  node tests/e2e/run-e2e-tests.cjs --browser=chromium       # Run on Chrome only
  node tests/e2e/run-e2e-tests.cjs --headed --debug         # Run with visible browser and debug
  node tests/e2e/run-e2e-tests.cjs --workers=1 --retries=2  # Single worker with retries
`);
  process.exit(0);
}
