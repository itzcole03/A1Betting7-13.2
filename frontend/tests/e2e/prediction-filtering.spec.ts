...
test.describe('Prediction Filtering', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.evaluate(() => {
      fetch('/api/sports/activate/MLB', { method: 'POST' }).catch(() => {}); // Ignore errors if already active
    });
  });
  ...
  test('should filter props by player name', async ({ page }) => {
    ...
    if (initialCount > 0) {
      ...
      const playerSearch = page
        .locator('input[placeholder*="player" i], input[placeholder*="search" i]')
        .first();
  ...
  test('should filter props by sport type', async ({ page }) => {
    ...
    const sportFilter = page
      .locator(
        '[data-testid="sport-filter"], select[name*="sport" i], button[role="button"]:has-text("MLB")'
      )
      .first();
  ...
  test('should filter props by confidence level', async ({ page }) => {
    ...
    const confidenceFilter = page
      .locator(
        '[data-testid="confidence-filter"], input[type="range"], select[name*="confidence" i]'
      )
      .first();
  ...
  test('should sort props by different criteria', async ({ page }) => {
    ...
    if (initialProps > 1) {
      ...
      const sortControl = page
        .locator('[data-testid="sort-control"], select[name*="sort" i]')
        .first();
  ...
  test('should handle empty filter results gracefully', async ({ page }) => {
    ...
    const searchInput = page
      .locator('input[placeholder*="search" i], input[placeholder*="player" i]')
      .first();
    ...
    if (await searchInput.isVisible()) {
      ...
      const noResults = await page
        .locator('[data-testid="no-results"], [data-testid="empty-state"]')
        .isVisible();
  ...
  test('should provide clear all filters functionality', async ({ page }) => {
    ...
    if (await playerFilter.isVisible()) {
      ...
      const clearButton = page
        .locator('[data-testid="clear-filters"], button:has-text("Clear")')
        .first();
