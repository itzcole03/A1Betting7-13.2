...
export const KellyCalculator: React.FC = () => {
  ...
  const updateSessionOutcome = useCallback(
    (sessionId: string, actualBet: number, outcome: 'win' | 'loss' | 'push') => {
      const updatedSessions = sessions.map(session => {
        if (session.id === sessionId) {
          let profit = 0;
          if (outcome === 'win') {
            const decimalOdds =
              session.inputs.odds > 0
          ...
          <div className='bg-slate-800/50 backdrop-blur rounded-lg border border-slate-700 p-6'>
            ...
            <div className='bg-slate-700/50 rounded-lg p-6 mb-6'>
              <div className='text-center'>
                ...
                <div
                  className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-sm mt-3 ${
                    kellyResult.riskLevel === 'low'
                      ? 'bg-green-900/50 text-green-300'
                      : kellyResult.riskLevel === 'medium'
                      ? 'bg-yellow-900/50 text-yellow-300'
                      : kellyResult.riskLevel === 'high'
            ...
            <div className='grid grid-cols-2 gap-4 mb-6'>
              ...
              <div className='bg-slate-700/30 rounded-lg p-4'>
                ...
                <div className='text-xl font-bold text-white'>
                  {kellyResult.expectedReturn >= 0 ? '+' : ''}
                  {kellyResult.expectedReturn.toFixed(2)}%
                </div>
              </div>
              ...
              <div className='grid grid-cols-2 md:grid-cols-5 gap-4 mb-6'>
                ...
                <div className='bg-slate-700/30 rounded-lg p-3 text-center'>
                  ...
                  >
                    ...
                    {sessionStats.totalProfit.toFixed(2)}
                  </div>
                ...
                <div className='bg-slate-700/30 rounded-lg p-3 text-center'>
                  <div
                    className={`text-lg font-bold ${
                      sessionStats.averageProfit >= 0 ? 'text-green-400' : 'text-red-400'
                    }`}
                  >
                    ${sessionStats.averageProfit >= 0 ? '+' : ''}
                    {sessionStats.averageProfit.toFixed(2)}
                  </div>
                ...
                <div className='bg-slate-700/30 rounded-lg p-3 text-center'>
                  <div
                    className={`text-lg font-bold ${
                      sessionStats.roiPercentage >= 0 ? 'text-green-400' : 'text-red-400'
                    }`}
                  >
                    {sessionStats.roiPercentage >= 0 ? '+' : ''}
                    {sessionStats.roiPercentage.toFixed(1)}%
                  </div>
            ...
            {sessions.length === 0 ? (
              <div className='text-center py-8'>
                ...
                <p className='text-sm text-slate-500 mt-2'>
                  Click "Save" to track your Kelly calculations
                </p>
              </div>
            ) : (
                    ...
                    {sessions.slice(0, 10).map(session => (
                      <tr key={session.id} className='hover:bg-slate-700/30 transition-colors'>
                        ...
                        <td className='px-4 py-4 whitespace-nowrap text-sm text-green-400'>
                          ${session.result.expectedValue >= 0 ? '+' : ''}
                          {session.result.expectedValue}
                        </td>
                        ...
                        <td className='px-4 py-4 whitespace-nowrap text-sm'>
                          {session.profit !== undefined ? (
                            <span
                              className={session.profit >= 0 ? 'text-green-400' : 'text-red-400'}
                            >
                              ${session.profit >= 0 ? '+' : ''}
