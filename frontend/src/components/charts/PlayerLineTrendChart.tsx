...
const PlayerLineTrendChart: React.FC<PlayerLineTrendChartProps> = ({
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className='bg-white p-3 border rounded-lg shadow-lg'>
          ...
          {gameData.confidence && (
            <p className='text-gray-600'>{`Confidence: ${(gameData.confidence * 100).toFixed(
              1
            )}%`}</p>
          )}
  ...
  if (loading) {
    return (
      <Card padded={false} variant='outline' className='bg-white/95 border-gray-200 text-slate-900'>
        <div className='p-6'>
