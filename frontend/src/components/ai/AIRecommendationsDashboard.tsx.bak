...
export const AIRecommendationsDashboard: React.FC = () => {
  ...
  const [selectedRecommendation, setSelectedRecommendation] = useState<SmartRecommendation | null>(
    null
  );
  ...
  const fetchRecommendations = useCallback(async () => {
    ...
    try {
      ...
      try {
        response = await fetch('/v1/ai-recommendations/generate', {
          method: 'POST',
          headers: {
      ...
      if (response.ok) {
        ...
        console.log(
          '[AIRecommendations] Successfully loaded',
  ...
  const generateMockRecommendations = (): SmartRecommendation[] => {
    ...
    return Array.from({ length: 8 }, (_, i) => ({
      ...
      confidence_interval: [0.45 + Math.random() * 0.1, 0.55 + Math.random() * 0.1] as [
        number,
        number
      ],
      reasoning: `AI model identifies strong value based on recent performance trends and matchup analysis. Player showing ${Math.floor(
        10 + Math.random() * 20
      )}% improvement over last 10 games.`,
          ...
          <div className='flex items-center justify-between'>
            <div>
              ...
              <p className='text-slate-300'>
                Smart prop recommendations powered by advanced ML models and real-time market
                analysis
              </p>
            </div>
            ...
            <div className='flex items-center gap-6'>
              ...
              <div className='text-sm'>
                ...
                <span className='text-white font-semibold ml-2'>
                  {recommendations.length > 0
                    ? (
                        recommendations.reduce((sum, rec) => sum + rec.ai_score, 0) /
              ...
              <div className='text-sm'>
                ...
                <span className='text-white font-semibold ml-2'>
                  {recommendations.length > 0
                    ? (
                        recommendations.reduce((sum, rec) => sum + rec.edge_percentage, 0) /
                        recommendations.length
                      ).toFixed(1)
                    : '0.0'}
                  %
            ...
            <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6'>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={e =>
                    setFilters(prev => ({ ...prev, min_edge: parseFloat(e.target.value) }))
                  }
              ...
              <div>
                ...
                <input
                  ...
                  onChange={e =>
                    setFilters(prev => ({ ...prev, min_ai_score: parseInt(e.target.value) }))
                  }
                  className='w-full accent-blue-500'
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={e =>
                    setFilters(prev => ({ ...prev, max_recommendations: parseInt(e.target.value) }))
                  }
                  className='w-full accent-blue-500'
                />
              </div>
          ...
          {loading ? (
            ...
            <div className='col-span-full text-center py-12'>
              ...
              <p className='text-slate-400 text-lg'>
                No AI recommendations match your current criteria
              </p>
            ...
            recommendations.map(rec => (
              ...
              >
                ...
                <div className='flex items-start justify-between mb-4'>
                  <div>
                    <h3 className='text-lg font-semibold text-white mb-1'>{rec.player_name}</h3>
                    <p className='text-slate-300 text-sm'>
                      {rec.stat_type.replace('_', ' ').toUpperCase()}{' '}
                      {rec.recommended_side.toUpperCase()} {rec.line}
                    </p>
                  </div>
                  <div className='flex items-center gap-2'>
                    <div
                    ...
                    <div className='bg-slate-700 rounded-lg p-3'>
                      <div className='flex justify-between text-sm'>
                        ...
                        <span className='text-white'>
                          {(selectedRecommendation.confidence_interval[0] * 100).toFixed(1)}%
                        </span>
                      </div>
                      <div className='flex justify-between text-sm'>
                        <span className='text-slate-400'>Upper Bound:</span>
                        <span className='text-white'>
                          {(selectedRecommendation.confidence_interval[1] * 100).toFixed(1)}%
                        </span>
                      </div>
                    </div>
                  </div>
                      ...
                      <div className='grid grid-cols-2 gap-4 text-sm'>
                        <div>
                          ...
                          <span className='text-white ml-2'>
                            {(selectedRecommendation.fair_probability * 100).toFixed(1)}%
                          </span>
                        </div>
                        <div>
                          <span className='text-slate-400'>Implied Probability:</span>
                          <span className='text-white ml-2'>
                            {(selectedRecommendation.implied_probability * 100).toFixed(1)}%
                          </span>
                        </div>
                        <div>
                          <span className='text-slate-400'>Market Efficiency:</span>
                          <span className='text-white ml-2'>
                            {(selectedRecommendation.market_efficiency * 100).toFixed(1)}%
                          </span>
                        </div>
                        <div>
                          ...
                          <span className='text-white ml-2'>
                            {(
                              selectedRecommendation.kelly_fraction * userProfile.bankroll_size
                            ).toFixed(0)}{' '}
                            units
                          </span>
                        </div>
                  ...
                  <div>
                    ...
                    <div className='bg-slate-700 rounded-lg p-3'>
                      <div className='flex items-center gap-2 text-sm'>
                        ...
                        <span className='text-white'>
                          {new Date(selectedRecommendation.expires_at).toLocaleString()}
                        </span>
                      </div>
                    </div>
                  </div>
