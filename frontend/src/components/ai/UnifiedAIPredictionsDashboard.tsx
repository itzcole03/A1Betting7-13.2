...
const UnifiedAIPredictionsDashboard: React.FC = () => {
  ...
  const [selectedStrength, setSelectedStrength] = useState<'ALL' | 'STRONG' | 'MODERATE' | 'WEAK'>('ALL');
  ...
  return (
    <div className="unified-ai-predictions min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 text-white p-6">
      {/* Header */}
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-8 gap-4">
        <div>
          ...
          <p className="text-slate-400">Advanced AI-powered sports betting predictions with confidence scoring</p>
        </div>
              ...
              <div className="flex flex-col lg:flex-row justify-between items-start gap-6">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-slate-400">{prediction.team} vs {prediction.opponent}</p>
                ...
                <div className="flex-1 max-w-md">
                  <div className="flex justify-between items-center mb-2">
                    ...
                    <div className={`px-3 py-1 rounded-full text-sm font-medium ${
                      prediction.recommendationStrength === 'STRONG' ? 'bg-green-500/20 text-green-300' :
                      prediction.recommendationStrength === 'MODERATE' ? 'bg-yellow-500/20 text-yellow-300' :
                  ...
                  <div className="grid grid-cols-2 gap-2 text-sm mb-3">
                    <div>
                      ...
                      <span className="text-white ml-2">{prediction.odds > 0 ? '+' : ''}{prediction.odds}</span>
                    </div>
                    <div>
                ...
                <div className="flex flex-col items-center gap-4">
                    ...
                    <div className="relative w-24 h-24">
                      <svg className="w-24 h-24 transform -rotate-90" viewBox="0 0 100 100">
                        ...
                        <circle
                          ...
                          strokeDashoffset={`${2 * Math.PI * 40 * (1 - prediction.aiConfidence / 100)}`}
                          className="text-cyan-400"
                          strokeLinecap="round"
                        />
                      </svg>
                      <div className="absolute inset-0 flex items-center justify-center">
                        <span className="text-2xl font-bold text-cyan-400">{prediction.aiConfidence}%</span>
                      </div>
                    </div>
                  ...
                  <div className="flex gap-4 text-center">
                    <div>
                      <div className="text-lg font-semibold text-purple-300">{prediction.modelConfidence}%</div>
                      <div className="text-xs text-slate-400">Model</div>
                    </div>
                    <div>
                      <div className="text-lg font-semibold text-green-300">{prediction.expectedValue}</div>
      ...
      {filteredPredictions.length === 0 && (
        <div className="text-center py-12">
          ...
          <p className="text-slate-400">Try adjusting your filters or check back soon for new AI analysis</p>
        </div>
      )}
    </div>
  );
  ...
  <div className="p-6 bg-slate-900/30">
          ...
          {prediction.keyFactors.map((factor, index) => (
            <div key={index} className="space-y-1">
              <div className="flex justify-between items-center">
                ...
                <span className={`text-sm font-medium ${factor.positive ? 'text-green-300' : 'text-red-300'}`}>
                  {factor.impact}%
                </span>
              </div>
    ...
    <div className="mt-6 pt-6 border-t border-slate-700">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="text-center">
          <div className="text-2xl font-bold text-cyan-300">{prediction.volume.toLocaleString()}</div>
        ...
        <div className="text-center">
          <div className={`text-2xl font-bold ${
            prediction.riskLevel === 'LOW' ? 'text-green-300' :
            prediction.riskLevel === 'MEDIUM' ? 'text-yellow-300' : 'text-red-300'
          }`}>
