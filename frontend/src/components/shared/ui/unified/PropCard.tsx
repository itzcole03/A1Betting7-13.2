...
interface UnifiedPropCardProps {
  ...
  fetchEnhancedAnalysis?: (proj: UnifiedProp) => Promise<unknown>;
  enhancedAnalysisCache?: Map<string, unknown>;
  loadingAnalysis?: Set<string>;
  statcastData?: Record<string, unknown> | null;
  alternativeProps?: {
    stat: string;
    line: number;
    confidence: number;
          ...
          {showAnalysis && (
            ...
            >
              {isAnalysisLoading ? (
                ...
                <div className='text-gray-200 text-sm leading-relaxed'>{analysis.analysis}</div>
              ) : (
              ...
              {analysis.stats.slice(0, 10).map((stat, idx) => {
                ...
                return (
                  <motion.div key={idx} className='text-center' whileHover={{ scale: 1.05 }}>
                    ...
                    />
                    <div className='text-xs text-gray-400 mt-1'>{stat.label.replace(/^0/, '')}</div>
                  </motion.div>
                );
              })}
            </div>
