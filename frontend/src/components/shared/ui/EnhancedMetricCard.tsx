...
export const EnhancedMetricCard: React.FC<EnhancedMetricCardProps> = ({
  ...
  const displayValueRef = useRef<number>(numericValue);
    ...
    ${
      variant === 'cyber'
        ? `bg-black border-cyan-400/30 ${
            isHovered ? 'shadow-lg shadow-cyan-400/20 border-cyan-400/50' : ''
          }`
        ...
        {trend && (
          <motion.div
            className={`flex items-center space-x-2 ${
              onTrendClick
                ? 'cursor-pointer hover:opacity-80 transition-transform hover:scale-[1.03] active:scale-[0.97] focus-visible:scale-[1.03]'
                : ''
            }`}
