...
const LaunchReadinessChecker: React.FC = () => {
  ...
  const [readinessChecks, setReadinessChecks] = useState<ReadinessCheck[]>([
    ...
    },
    {
      ...
      details:
        'All critical metrics monitored. Alert thresholds configured. On-call rotation active.',
    },
    {
  ...
  const runReadinessCheck = async () => {
    ...
    for (let i = 0; i < readinessChecks.length; i++) {
      ...
      setReadinessChecks(prev =>
        prev.map((check, index) => (index === i ? { ...check, status: 'checking' } : check))
      );
    }
  ...
  const criticalIssues = readinessChecks.filter(
  ...
  return (
    <div className='min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 text-white p-6'>
      ...
      <div className='mb-8'>
            ...
            <div className='flex items-center space-x-2 bg-slate-800 px-4 py-2 rounded-lg'>
              <div
                className={`w-3 h-3 rounded-full ${
                  isRunningChecks ? 'bg-blue-400 animate-pulse' : 'bg-green-400'
                }`}
              ></div>
        ...
        <div className='text-sm text-slate-500 mt-4'>Last scan: {lastScan.toLocaleString()}</div>
      </div>
                ...
                >
                  <div className='flex items-start justify-between'>
                    <div className='flex items-start space-x-3 flex-1'>
                      <StatusIcon
                        className={`w-6 h-6 mt-1 ${getStatusColor(check.status).split(' ')[0]}`}
                      />
                      <div className='flex-1'>
                        ...
                        <p className='text-xs text-slate-500'>Last checked: {check.lastChecked}</p>
                      </div>
                    </div>
