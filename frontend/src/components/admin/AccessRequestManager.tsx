...
import { _accessRequestService, AccessRequest } from '../../services/AccessRequestService';
...
const _AccessRequestManager: React.FC<AccessRequestManagerProps> = ({ authToken, adminEmail }) => {
  ...
  const _loadRequests = async () => {
    try {
      setLoading(true);
      setError(null);
      const _data = await _accessRequestService.getAllAccessRequests(authToken);
      setRequests(_data);
    } catch (err) {
  ...
  const _handleApprove = async (requestId: string) => {
    ...
    try {
      const _response = await _accessRequestService.approveAccessRequest(
        requestId,
        authToken,
        adminEmail
      );
      ...
      if (_response.success) {
        await _loadRequests(); // Reload to get updated data
      }
    } catch (err) {
  ...
  const _handleDeny = async (requestId: string) => {
    ...
    setProcessingRequest(requestId);
    try {
      const _response = await _accessRequestService.denyAccessRequest(
        requestId,
        authToken,
        adminEmail,
      ...
      <div className='flex flex-col sm:flex-row gap-4'>
        ...
        <div className='flex items-center space-x-2'>
          ...
          <select
            value={statusFilter}
            onChange={e =>
              setStatusFilter(e.target.value as 'all' | 'pending' | 'approved' | 'denied')
            }
                ...
                <div className='flex items-start justify-between'>
                  ...
                  {request.status === 'pending' && (
                    ...
                    <div className='flex items-center space-x-2 ml-4'>
                      ...
                      provided... Remove this comment to see the full error message
                      <button
                        onClick={() => _handleApprove(request.id)}
        ...
        <div className='fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4'>
          ...
          >
            ...
            <div className='flex items-center space-x-3'>
              ...
              Remove this comment to see the full error message
              <button
                onClick={() => _handleDeny(showDenyModal!)}
...
export default _AccessRequestManager;
