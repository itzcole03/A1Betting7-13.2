...
const _AdminPanel: React.FC = () => {
  ...
  useEffect(() => {
    _loadSystemData();
    const _interval = setInterval(_loadSystemData, 30000); // Refresh every 30 seconds
  ...
  const _loadSystemData = async () => {
    ...
    try {
      // Simulate API calls
      await Promise.all([_loadUsers(), _loadSystemMetrics(), _loadAPIEndpoints()]);
    } catch (error) {
  ...
  const _loadAPIEndpoints = async () => {
    ...
    setApiEndpoints(_mockEndpoints);
  };
  ...
  const _bulkUpdateUsers = async (action: string) => {
    ...
    switch (action) {
      case 'activate':
        selectedUsers.forEach(userId => _updateUserStatus(userId, 'active'));
        break;
      case 'suspend':
        selectedUsers.forEach(userId => _updateUserStatus(userId, 'suspended'));
        break;
      case 'ban':
        selectedUsers.forEach(userId => _updateUserStatus(userId, 'banned'));
        break;
    }
  ...
  const _filteredUsers = users.filter(user => {
    ...
    return _matchesSearch && _matchesStatus && _matchesPlan;
  });
          ...
          <div className='flex items-center justify-between'>
            <div>
              ...
              <p className='text-green-400 text-sm mt-1'>
                {_formatUptime(systemMetrics?.uptime || 0)}
              </p>
            </div>
  ...
  const _renderTabContent = () => {
    switch (activeTab) {
      ...
      case 'users':
        return _renderUserManagement();
      case 'system':
        return _renderSystemHealth();
      case 'api':
        return _renderAPIStatus();
      case 'settings':
        return (
          // @ts-expect-error TS(17004): Cannot use JSX unless the '--jsx' flag is provided... Remove this comment to see the full error message
          <div className='space-y-6'>
