...
export class RealTimePerformanceTracker {
  ...
  startTracking(): void {
    ...
    console.log('[RealTimePerformanceTracker] Tracking started for session:', this.currentSession.id);
  }
  ...
  private setupEventListeners(): void {
    ...
    this.eventListeners.set('navigation', () => window.removeEventListener('popstate', navigationHandler));
    ...
    this.eventListeners.set('unhandledrejection', () => window.removeEventListener('unhandledrejection', rejectionHandler));
  }
  ...
  private recordFirstInput(entry: any): void {
  ...
  private async gatherPerformanceMetrics(): Promise<PerformanceSnapshot['metrics']> {
    ...
    try {
      ...
      webVitals.getFCP((metric) => { metrics.fcp = metric.value; });
      webVitals.getFID((metric) => { metrics.fid = metric.value; });
      webVitals.getLCP((metric) => { metrics.lcp = metric.value; });
      webVitals.getTTFB((metric) => { metrics.ttfb = metric.value; });
    } catch (error) {
    ...
    if (this.currentSession) {
      ...
      if (recentInteractions.length > 0) {
        metrics.interactionLatency = recentInteractions.reduce((sum, i) => sum + i.performance.responseTime, 0) / recentInteractions.length;
      }
    }
    ...
    Object.entries(thresholds).forEach(([metric, threshold]) => {
      ...
      if (value > threshold) {
        console.warn(`[Performance] Regression detected in ${metric}:`, {
          current: value,
          threshold,
          average: (averages as any)[metric]
        });
      }
    });
  }

  private calculateAverages(snapshots: PerformanceSnapshot[]): Partial<PerformanceSnapshot['metrics']> {
    const sums: any = {};
  ...
  private calculateSessionMetrics(): void {
    ...
    if (interactions.length > 0) {
      ...
      session.metrics.averageResponseTime = interactions.reduce(
        (sum, i) => sum + i.performance.responseTime, 0
      ) / interactions.length;
  ...
  getPerformanceTrends(hours = 1): PerformanceTrend[] {
    ...
    const recentSnapshots = this.performanceSnapshots.filter(
