...
describe('UnifiedDataService Extensions', () => {
  ...
  describe('Stale While Revalidate', () => {
    test('should handle network errors with allowStale option', async () => {
      ...
      // Test that allowStale option is passed through (even though no stale data exists)
      try {
        await dataService.searchData('test', {}, { allowStale: true });
      ...
      const metrics = dataService.getMetrics();
      expect(metrics.errors).toBe(1);
      expect(metrics.staleServed).toBe(0); // No stale data was available
    });
