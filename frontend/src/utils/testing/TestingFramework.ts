...
export class MockDataGenerator {
  static generatePlayer(overrides: Partial<any> = {}): any {
    return {
      ...
      sport: 'MLB',
      stats: {
        avg: this.generateNumber(0.200, 0.400, 3),
        hr: this.generateNumber(0, 50),
        rbi: this.generateNumber(0, 150),
        ops: this.generateNumber(0.600, 1.200, 3)
      },
      ...overrides
    };
  }
...
export class TestUtils {
  ...
  static mockLocalStorage(): void {
    const localStorage = {
      ...
      getItem: jest.fn((key: string) => localStorage.store[key] || null),
    ...
    return {
      ...
      testFn: async () => {
        ...
        if (duration > maxDuration) {
          throw new Error(
            `Operation took ${duration.toFixed(2)}ms, expected < ${maxDuration}ms`
          );
        }
      }
    };
...
export class IntegrationTestUtils {
  ...
  static createE2ETest(name: string, steps: Array<{
    description: string;
